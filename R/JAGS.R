BUGSeta <- function(x, id = NULL, ...) {
 ...
 for (j in 1:k) {
   setup$param <- c(setup$param, paste(if (k < 2) paste("beta", id2, sep = "") else paste("beta", id2, "[", j, "]", sep = ""), "*X", id2, "[i, ", j, "]", sep = ""))
 }
...
BUGSlinks <- function(x) {
  switch(x, identity = "eta", log = "exp(eta)", exp = "log(eta)", inverse = "1 / (eta^2)", logit = "1 / (1 + exp(-(eta)))", probit = "phi(eta)", cloglog = "log(-log(1 - eta))", pow = "pow(eta, -2)")
}
BUGSmodel <- function(x, family, is.stan = FALSE, reference = NULL, ...) {
 ...
 stop(paste("more parameters specified than existing in family ", family$family, ".bamlss()!", sep = ""), call. = FALSE)
  }
  model <- "model {"
  for (j in 1:k) {
 ...
 pn <- pn[family$bugs$order]
  ...
  model2 <- c(paste("  y[i] ~ ", family$bugs$dist, "(", paste(if (!is.null(reference)) paste("probs[i, 1:", k + 1, "]", sep = "") else pn, collapse = ", "), ")", sep = ""))
  if (!is.null(reference)) {
 for (j in 1:k) {
   model2 <- c(model2, paste("  probs[i, ", j, "] <- ", pn[j], sep = ""))
 }
 model2 <- c(model2, paste("  probs[i, ", k + 1, "] <- ", 1, sep = ""))
  }
  if (!is.null(family$bugs$reparam)) {
 reparam <- NULL
 for (j in seq_along(family$bugs$reparam)) reparam <- c(reparam, paste("  rp", j, "[i] <- ", family$bugs$reparam[j], sep = ""))
 for (j in family$names) reparam <- gsub(j, paste(j, "[i]", sep = ""), reparam)
 model2 <- c(model2, reparam)
 pn[repi] <- paste(family$names[repi], "[i]", sep = "")
  }
  if (!is.null(family$bugs$addparam)) {
 for (j in family$bugs$addparam) model2 <- c(model2, paste("   ", j))
  }
  if (!is.null(family$bugs$addvalues)) {
 for (j in names(family$bugs$addvalues)) model2 <- gsub(j, family$bugs$addvalues[[j]], model2)
  }
  for (j in 1:k) {
 model2 <- c(model2, paste("  ", if (is.null(on)) pn[j] else paste(on, "[i, ", j, "]", sep = ""), " <- ", gsub("eta", x[[j]]$eta, links[[j]]), sep = ""))
  }
  for (j in 1:k) model2 <- c(model2, x[[j]]$adds)
  for (j in 1:k) model2 <- c(model2, x[[j]]$param, x[[j]]$smooth.construct)
  model2 <- rev(model2)
  model <- c(model, "  for(i in 1:n) {", model2)
  model <- c(model, "  }")
  for (i in 1:k) model <- c(model, x[[i]]$close1)
  for (i in 1:k) {
 lp <- list()
 if (!is.null(x[[i]]$loops)) {
   for (j in 1:length(x[[i]]$loops)) lp[[paste(x[[i]]$loops[j])]] <- c(lp[[paste(x[[i]]$loops[j])]], x[[i]]$priors.coef[j])
 }
 if (length(lp)) {
   for (j in names(lp)) {
  if (j != 1) 
...
setupJAGS <- function(x, y, family, is.stan = FALSE) {
 ...
 psave <- c(psave, rval[[j]]$psave)
  ...
  data$y <- y
  rval <- list(model = model, data = data, inits = inits, psave = psave)
  return(rval)
}
buildBUGS.smooth <- function(smooth, setup, i) {
  fall <- NULL
  kr <- if (is.null(smooth$rand$Xr)) 
 0
  else ncol(smooth$rand$Xr)
  kx <- if (is.null(smooth$Xf)) 
 0
  else ncol(smooth$Xf)
  if (kr < 1 & kx < 1) {
 if (!is.null(smooth$X)) {
  ...
  hcauchy <- if (is.null(smooth$xt$hcauchy)) 
 FALSE
  else smooth$xt$hcauchy
  ig <- itau2 <- NULL
 ...
 itau2 <- get.par(smooth$state$parameters, "tau2")
  ...
  if (kx > 0) {
 fall <- c(fall, paste("b", i, if (kx > 1) paste("[", 1:kx, "]", sep = ""), "*Xf", i, "[i, ", 1:kx, "]", sep = ""))
 ...
 setup$psave <- c(setup$psave, paste("b", i, sep = ""))
  ...
  if (kr > 0) {
 fall <- c(fall, paste("g", i, if (kr > 1) paste("[", 1:kr, "]", sep = ""), "*Xr", i, "[i, ", 1:kr, "]", sep = ""))
 setup$data[[paste("Xr", i, sep = "")]] <- smooth$rand$Xr
 taug <- paste("taug", if (is.null(smooth$id)) 
   i
 else smooth$id, sep = "")
 tmp <- if (kr > 1) {
 ...
 if (is.null(setup$priors.scale) || !any(grepl(taug, setup$priors.scale))) {
   if (!hcauchy) {
  setup$priors.scale <- c(setup$priors.scale, paste("  ", taug, " ~ dgamma(1.0E-4, 1.0E-4)", sep = ""))
   }
   else {
  setup$priors.scale <- c(setup$priors.scale, paste("  ", taug, " <- abs(", taug, 0, ")", sep = ""), paste("  ", taug, 0, "~ dt(0, 10, 1)", sep = ""))
   }
 ...
 setup$psave <- c(setup$psave, paste("g", i, sep = ""))
  ...
  setup$smooth.construct <- c(setup$smooth.construct, paste("  sm", i, "[i] <- ", paste(fall, collapse = " + ", sep = ""), sep = ""))
  setup$eta <- paste(setup$eta, paste("sm", i, "[i]", sep = ""), sep = if (length(setup$eta)) 
 " + "
  else "")
  setup
}
...
buildBUGS.smooth.special.gc.smooth <- function(smooth, setup, i, zero, ...) {
  center <- if (is.null(smooth$xt$center)) 
 TRUE
  else smooth$xt$center
  pn <- paste("g", i, sep = "")
  setup$data[[paste("X", pn, sep = "")]] <- as.numeric(smooth$X)
  setup$inits[[paste(pn, sep = "")]] <- runif(3, 0.1, 0.2)
  setup$psave <- c(setup$psave, pn)
  setup$close2 <- c(setup$close2, "  for(j in 1:3) {", paste("  ", pn, "[j] ~ dnorm(0, 1.0E-6)", sep = ""), "  }")
  fall <- paste(pn, "[1] / (1 + exp(", pn, "[2]) * (exp(", pn, "[3]) / (1 + exp(", pn, "[3])))^(X", pn, "[i]))", sep = "")
  if (!center) {
 setup$smooth.construct <- c(setup$smooth.construct, paste("  sm", i, "[i] <- ", paste(fall, collapse = " + ", sep = ""), sep = ""))
  }
  else {
 setup$close1 <- c(setup$close1, paste("  sm", i, 1, " <- sm", i, 0, " - mean(sm", i, 0, ")", sep = ""))
 setup$close1 <- c(setup$close1, paste("  for(i in 1:n) {", sep = ""), paste("  sm", i, 0, "[i] <- ", paste(fall, collapse = " + ", sep = ""), sep = ""), "  }")
  }
  setup$eta <- paste(setup$eta, paste("sm", i, if (center) 
 1
  else NULL, "[i]", sep = ""), sep = if (length(setup$eta)) 
 " + "
  else "")
  setup
}
samplerJAGS <- function(x, tdir = NULL, n.chains = 1, n.adapt = 100, n.iter = 4000, thin = 2, burnin = 1000, seed = NULL, verbose = TRUE, set.inits = TRUE, save.all = FALSE, modules = NULL) {
  if (is.null(tdir)) {
 ...
 inits[[j]][[".RNG.seed"]] <- seed[j]
  ...
  if (!is.null(modules)) {
 for (m in modules) rjags::load.module(m)
  }
  if (verbose) 
 writeLines(x$model)
  if (save.all) {
 ...
 writeLines(c("library(\"rjags\")", "load(\"msetup.rda\")", "m <- jags.model(\"model.txt\", data = mdata, inits = inits)", paste("samples <- coda.samples(m, variable.names = vnames, n.iter = ", n.iter, ", thin = ", thin, ")", sep = "")), con = file.path(tdir, "model.R"))
  }
  if (set.inits) {
 jmodel <- rjags::jags.model(mfile, data = x$data, inits = inits, n.chains = n.chains, n.adapt = n.adapt)
  }
  else {
 jmodel <- rjags::jags.model(mfile, data = x$data, n.chains = n.chains, n.adapt = n.adapt)
  }
  jsamples <- rjags::coda.samples(jmodel, variable.names = c(x$psave, "deviance"), n.iter = n.iter, thin = thin)
  if (is.null(burnin)) 
 burnin <- floor(n.iter * 0.2)
  jsamples <- window(jsamples, start = burnin)
  jsamples
}
sam_JAGS <- JAGS <- function(x, y, family, start = NULL, tdir = NULL, n.chains = 1, n.adapt = 100, n.iter = 4000, thin = 2, burnin = 1000, seed = NULL, verbose = TRUE, set.inits = TRUE, save.all = FALSE, modules = NULL, ...) {
 ...
 x <- set.starting.values(x, start)
  ...
  samps <- samplerJAGS(ms, tdir, n.chains, n.adapt, n.iter, thin, burnin, seed, verbose, set.inits, save.all, modules)
...
transform.bugs.samples <- function(x, samples) {
 ...
 for (i in seq_along(x)) {
  ...
  kr <- if (is.null(x[[i]]$smooth.construct[[ii]]$rand$Xr)) 
    0
  else ncol(x[[i]]$smooth.construct[[ii]]$rand$Xr)
  kx <- if (is.null(x[[i]]$smooth.construct[[ii]]$Xf)) 
    0
  else ncol(x[[i]]$smooth.construct[[ii]]$Xf)
  if (kx) {
  ...
  colnames(psamples) <- paste(nx[i], "s", x[[i]]$smooth.construct[[ii]]$label, paste("b", 1:ncol(x[[i]]$smooth.construct[[ii]]$X), sep = ""), sep = ".")
  taug <- paste("taug", ii, i, sep = "")
  if (taug %in% snames) {
    vsamples <- as.matrix(samples[[j]][, snames %in% taug])
    colnames(vsamples) <- paste(nx[i], "s", x[[i]]$smooth.construct[[ii]]$label, paste("tau2", 1:length(x[[i]]$smooth.construct[[ii]]$S), sep = ""), sep = ".")
