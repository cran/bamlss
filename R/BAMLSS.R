bamlss.frame <- function(formula, data = NULL, family = "gaussian", weights = NULL, subset = NULL, offset = NULL, na.action = na.omit, contrasts = NULL, knots = NULL, specials = NULL, reference = NULL, model.matrix = TRUE, smooth.construct = TRUE, ytype = c("matrix", "vector", "integer"), scale.x = FALSE, scale.d = FALSE, ...) {
 ...
 formula <- attr(formula, "orig.formula")
  bf <- list()
  bf$call <- match.call()
  bf$model.frame <- bamlss.model.frame(formula, data, family, weights, subset, offset, na.action, specials, contrasts, nthres = list(...)$nthres)
 ...
 if (!is.null(cf) & (ytype != "integer")) {
  ...
  if (is.factor(bf$model.frame[[rn[1]]])) {
  if (ytype == "integer") 
    bf$y[[1]] <- as.integer(bf$y[[1]]) - if (nlevels(bf$model.frame[[rn[1]]]) < 3) 
    1
    else 0
  }
   }
  ...
  if (is.factor(bf$y[[j]]) & (ytype == "integer")) {
    bf$y[[j]] <- as.integer(bf$y[[j]]) - if (nlevels(bf$y[[j]]) < 3) 
    1
    else 0
  }
  }
 ...
 bf$model.frame <- scale_model.frame(bf$model.frame, not = rn)
  bf$x <- design.construct(bf$terms, data = bf$model.frame, knots = knots, model.matrix = model.matrix, smooth.construct = smooth.construct, model = NULL, scale.x = scale.x, specials = specials, envir_formula = envir_formula, ...)
  bf$knots <- knots
...
design.construct <- function(formula, data = NULL, knots = NULL, model.matrix = TRUE, smooth.construct = TRUE, binning = FALSE, before = TRUE, gam.side = NULL, model = NULL, drop = NULL, scale.x = TRUE, absorb.cons = NULL, sparse.cons = 0, specials = NULL, ...) {
 ...
 data <- ff::read.table.ffdf(file = data, na.strings = "", header = TRUE, sep = ",")
  }
 ...
 if (model.matrix) {
  ...
  obj$model.matrix <- try(model.matrix(drop.terms.bamlss(obj$terms, sterms = FALSE, keep.response = FALSE, data = data, specials = specials), data = drop_terms_attr(data)), silent = TRUE)
  if (inherits(obj$model.matrix, "try-error")) {
  lmt <- drop.terms.bamlss(obj$terms, sterms = FALSE, keep.response = FALSE, data = data, specials = specials)
  ...
  mm_terms <- drop.terms.bamlss(obj$terms, sterms = FALSE, keep.response = FALSE, data = NULL, specials = specials)
  ...
  if (ncol(mm_test) > 0) {
  nobs <- nrow(data)
  obj$model.matrix <- ff::ff(0, length = nobs * ncol(mm_test), dim = c(nobs, ncol(mm_test)))
  k <- 1
  np <- 0
 ...
 if (smooth.construct) {
   tx <- drop.terms.bamlss(obj$terms, pterms = FALSE, keep.response = FALSE, data = data, specials = specials)
  ...
  sterms <- lapply(sterms, function(x) {
  eval(parse(text = x), envir = envir_formula)
  })
  nst <- NULL
  ...
  if (!special) {
   ...
   stop("binning not allowd using data of class \"ffdf\"!")
    ...
    if (!inherits(data, "ffdf")) {
   if (!doCmat) {
   smt <- smoothCon(tsm, if (before) 
     data[tsm$binning$nodups, term.names, drop = FALSE]
   else data, knots, absorb.cons = if (is.null(absorb.cons)) 
     acons
   else absorb.cons, sparse.cons = sparse.cons, scale.penalty = TRUE)
   }
   else {
   smt <- smooth.construct(tsm, if (before) 
     data[tsm$binning$nodups, term.names, drop = FALSE]
   else data, knots)
   smt$C <- Cmat(smt)
   smt$doCmat <- TRUE
   ...
   smt <- smooth.construct_ff(tsm, data, knots, absorb.cons = if (is.null(absorb.cons)) 
   acons
   else absorb.cons, ff_name = ff_name, nthres = nthres)
   ...
   smt <- smoothCon(tsm, data, knots, absorb.cons = if (is.null(absorb.cons)) 
   acons
   else absorb.cons, sparse.cons = sparse.cons, scale.penalty = scale.pen)
    }
    smooth <- c(smooth, smt)
    }
  }
   ...
   class(smt2) <- c(class(smt2), "mgcv.smooth")
   smt <- if (!inherits(smt2, "smooth.list")) 
   list(smt2)
   else smt2
   smooth <- c(smooth, smt)
    }
  ...
  for (j in seq_along(fterms)) {
    nenv <- new.env()
    for (nd in names(data)) assign(nd, data[[nd]], envir = nenv)
   ...
   assign(vj, tmp, envir = nenv)
    ...
    smt <- smoothCon(tfme, data = tfm$data, n = nrow(tfm$data[[1]]), knots = knots, absorb.cons = TRUE, scale.penalty = TRUE)
  ...
  if (gam.side) {
    ...
    Xp <- model.matrix(drop.terms.bamlss(obj$terms, sterms = FALSE, keep.response = FALSE, data = data, specials = specials), data = data)
    smooth <- try(gam.side(smooth, Xp, tol = .Machine$double.eps^0.5), silent = TRUE)
    }
    else {
    smooth <- try(gam.side(smooth, obj$model.matrix, tol = .Machine$double.eps^0.5), silent = TRUE)
    }
    if (inherits(smooth, "try-error")) {
    cat("---\n", smooth, "---\n")
    if (binning) 
   stop("gam.side() produces an error when binning, try to set before = FALSE or set gam.side = FALSE!")
    else stop("gam.side() produces an error, try to set gam.side = FALSE!")
    }
  }
  sme <- NULL
  if (smooth.construct) 
  ...
  for (j in seq_along(smooth)) smooth[[j]][["X.dim"]] <- ncol(smooth[[j]]$X)
  if (!is.null(no.mgcv)) 
 ...
 if (!is.null(obj$smooth.construct)) {
  ...
  if (obj$smooth.construct[[j]]$by != "NA") {
    ...
    if (!grepl(paste0("by=", obj$smooth.construct[[j]]$by), obj$smooth.construct[[j]]$label, fixed = TRUE)) {
 ...
 for (j in seq_along(formula)) {
  ...
  formula[[j]][[i]] <- assign.design(formula[[j]][[i]], if (i > 1) 
    duplicated(data[, all.vars(formula[[j]][[i]]$fake.formula), drop = FALSE])
  else NULL)
  }
   }
...
coerce_to_highest_vmode <- function(x, y, onlytest = TRUE) {
  test <- data.frame(x.vmode = ff::vmode(x), y.vmode = ff::vmode(y), stringsAsFactors = FALSE)
  test$maxffmode <- apply(test[, , drop = FALSE], MARGIN = 1, FUN = function(x) names(ff::maxffmode(x)))
...
smooth.construct_ff.default <- function(object, data, knots, ff_name, nthres = NULL, ...) {
 ...
 if ((length(terms) > 1) & !any(is_f) & FALSE) {
  ...
  cat("\n  .. .. unique obs. ", j, " = ", length(nd[[j]]), ", digits = ", object$xt$digits, "\n", sep = "")
  }
  else {
  nd[[j]] <- sample(rep(unique(data[[j]]), length.out = 1000))
  }
   }
 }
 nmax <- max(sapply(nd, length))
 for (j in 1:length(nd)) nd[[j]] <- rep(nd[[j]], length.out = nmax)
 nd <- as.data.frame(nd)
  }
  object <- smoothCon(object, data = if (nrow(data) > nthres) 
 nd
  else as.data.frame(data), knots = knots, absorb.cons = FALSE, scale.penalty = FALSE)[[1]]
  rm(nd)
  nobs <- nrow(data)
 ...
 object[["X"]] <- ff::ff(0, length = nrow(data) * ncol(object[["X"]]), dim = c(nrow(data), ncol(object[["X"]])), filename = paste0(xfile, ".ff"))
 sX <- function(x) {
...
sparse.setup <- function(x, S = NULL, ...) {
 ...
 for (j in seq_along(S)) {
  ...
  index.crossprod <- if (!symmetric) 
 sparse.matrix.index(x, ...)
  else NULL
  setup <- list(matrix = index.matrix, crossprod = index.crossprod)
...
sparse.chol <- function(x, index, ...) {
 ...
 for (i in imat[c, ][imat[c, ] > c]) {
   l[i, c] <- (x[p[i], p[c]] - sum(l[i, 1:(c - 1)] * l[c, 1:(c - 1)]))/l[c, c]
 }
  }
...
sparse.solve <- function(a, b, index, ...) {
  if (all(dim(a) < 2)) 
 return(b/a)
  id <- if (!("crossprod" %in% names(index))) 
 "matrix"
  else "crossprod"
...
make.prior <- function(x, sigma = 0.1) {
 ...
 if (is.null(theta)) {
   theta <- switch(prior, sd = 0.00877812, hc = 0.01034553, hn = 0.1457644, u = 0.2723532)
 }
 fixed <- if (is.null(x$fixed)) 
   FALSE
 else x$fixed
 igs <- log((b^a)) - log(gamma(a))
 var_prior_fun <- switch(prior, ig = function(tau2) {
   igs + (-a - 1) * log(tau2) - b/tau2
 }, hc = function(tau2) {
   -log(1 + tau2/(theta^2)) - 0.5 * log(tau2) - log(theta^2)
 }, sd = function(tau2) {
   -0.5 * log(tau2) + 0.5 * log(theta) - (tau2/theta)^(0.5)
 }, hn = function(tau2) {
   -0.5 * log(tau2) - tau2/(2 * theta^2)
 }, hn.lasso0 = function(tau2) {
   -0.2257913 - log(sigma) - tau2^2/(2 * sigma^2)
 ...
 rval$prior <- function(parameters) {
  ...
  if (length(tau2) < 2) {
  K <- if (is.function(x$S[[1]])) 
    x$S[[1]](c(parameters, x$fixed.hyper))
  else x$S[[1]]
  if (is.null(x$rank)) 
    x$rank <- qr(K)$rank
  if (!is.null(x$xt[["pS"]])) 
  ...
  ld <- 0
  P <- if (inherits(x$X, "Matrix")) 
    Matrix(0, ncol(x$X), ncol(x$X))
  else 0
  for (j in seq_along(tau2)) {
    P <- P + 1/tau2[j] * if (is.function(x$S[[j]])) 
    x$S[[j]](c(parameters, x$fixed.hyper))
    else x$S[[j]]
 ...
 rval$grad <- function(score = NULL, parameters, full = TRUE) {
  ...
  grad2 <- NULL
  for (j in seq_along(tau2)) {
  tauS <- -1/tau2[j] * if (is.function(x$S[[j]])) 
    x$S[[j]](c(parameters, x$fixed.hyper))
  else x$S[[j]]
 ...
 rval$hess <- function(score = NULL, parameters, full = FALSE) {
  ...
  hx <- 0
  for (j in seq_along(tau2)) {
  hx <- hx + (1/tau2[j]) * if (is.function(x$S[[j]])) 
    x$S[[j]](c(parameters, x$fixed.hyper))
  else x$S[[j]]
  }
   }
...
model.frame.bamlss <- model.frame.bamlss.frame <- function(formula, ...) {
 ...
 fcall <- formula$call
 fcall[[1]] <- quote(bamlss.model.frame)
...
model.search <- function(x, what, model = NULL, part = c("x", "formula", "terms"), extract = FALSE, drop = FALSE) {
 ...
 if (!all(elmts %in% names(x[[i]]))) {
   rval[[i]] <- list()
   for (j in names(x[[i]])) {
  rval[[i]][[j]] <- if (is.null(x[[i]][[j]][[what]])) 
  FALSE
  else TRUE
  if (extract & rval[[i]][[j]]) 
  rval[[i]][[j]] <- x[[i]][[j]][[what]]
...
extract.design.construct <- function(object, data = NULL, knots = NULL, model = NULL, drop = TRUE, what = c("model.matrix", "smooth.construct"), specials = NULL, ...) {
 ...
 if (!is.null(data)) {
   ...
   object <- design.construct(object, data = data, knots = knots, model.matrix = model.matrix, smooth.construct = smooth.construct, model = model, drop = drop, specials = specials, ...)
 }
 else {
   if (!all(model.search(object, what, model, part = "x"))) {
  object <- design.construct(object, model.matrix = model.matrix, smooth.construct = smooth.construct, model = model, drop = TRUE, specials = specials, ...)
   }
   else {
...
model.matrix.bamlss.frame <- model.matrix.bamlss.formula <- model.matrix.bamlss.terms <- function(object, data = NULL, model = NULL, drop = TRUE, scale.x = FALSE, ...) {
  extract.design.construct(object, data = data, knots = NULL, model = model, drop = drop, what = "model.matrix", scale.x = scale.x)
}
...
smooth.construct.bamlss.frame <- smooth.construct.bamlss.formula <- smooth.construct.bamlss.terms <- function(object, data = NULL, knots = NULL, model = NULL, drop = TRUE, ...) {
  extract.design.construct(object, data = data, knots = knots, model = model, drop = drop, what = "smooth.construct")
}
parameters <- function(x, model = NULL, start = NULL, fill = c(0, 1e-04), list = FALSE, simple.list = FALSE, extract = FALSE, ...) {
 ...
 if (!is.null(x$parameters)) {
  ...
  if (list) 
    return(x$parameters[model])
  else return(unlist(x$parameters[model]))
  }
  else {
 ...
 if (is.null(x$x)) {
   x <- design.construct(x, data = x$model.frame, knots = x$knots, model.matrix = TRUE, smooth.construct = TRUE, model = NULL)
 }
 else x <- x$x
  }
 ...
 if (!all(c("formula", "fake.formula") %in% names(x[[i]]))) {
  ...
  if (re.effect) {
   ...
   x[[i]]$smooth.construct[[k]]$X.dim
    else ncol(x[[i]]$smooth.construct[[k]]$X)
    }
    else x[[i]]$smooth.construct[[k]]$special.npar
    if (inherits(x[[i]]$smooth.construct[[k]], "nnet0.smooth")) {
    nfill <- x[[i]]$smooth.construct[[k]]$nodes * ncol(x[[i]]$smooth.construct[[k]]$X) + x[[i]]$smooth.construct[[k]]$nodes
    }
    tpar <- rep(fill[1], nfill)
    if (inherits(x[[i]]$smooth.construct[[k]], "nnet0.smooth")) {
    cn <- c(paste0("bb", 1:x[[i]]$smooth.construct[[k]]$nodes), names(unlist(x[[i]]$smooth.construct[[k]]$n.weights)))
    }
    else {
...
par2list <- function(x) {
  ...
  np <- unique(sapply(npl, function(x) {
 x[1]
  }))
  for (j in np) {
 xl[[j]] <- list()
 tmp <- grep(paste(j, ".", sep = ""), nx, value = TRUE)
 tmp2 <- unique(sapply(strsplit(tmp, ".", fixed = TRUE), function(x) {
   x[2]
 }))
 for (jj in tmp2) {
  ...
  tmp4 <- unique(sapply(strsplit(tmp4, ".", fixed = TRUE), function(x) {
  x[3]
  }))
  xl[[j]][[jj]] <- list()
  for (jjj in tmp4) {
...
bamlss <- function(formula, family = "gaussian", data = NULL, start = NULL, knots = NULL, weights = NULL, subset = NULL, offset = NULL, na.action = na.omit, contrasts = NULL, reference = NULL, transform = NULL, optimizer = NULL, sampler = NULL, samplestats = NULL, results = NULL, cores = NULL, sleep = NULL, combine = TRUE, model = TRUE, x = TRUE, light = FALSE, ...) {
  family <- bamlss.family(family, ...)
 ...
 samplestats <- FALSE
  ...
  foo <- list(transform = transform, optimizer = optimizer, sampler = sampler, samplestats = samplestats, results = results)
  nf <- names(foo)
 ...
 if (!is.function(functions[[nf[j]]])) {
  ...
  bf[c("transform", "optimizer", "sampler", "samplestats", "results", "cores", "sleep", "combine", "model", "x")] <- NULL
 ...
 opt <- functions$optimizer(x = bf$x, y = bf$y, family = bf$family, start = start, weights = model.weights(bf$model.frame), offset = bf$model.frame[["(offset)"]], ...)
 if (!is.list(opt)) {
 ...
 ptm <- proc.time()
 if (is.null(cores)) {
   bf$samples <- functions$sampler(x = bf$x, y = bf$y, family = bf$family, weights = model.weights(bf$model.frame), offset = model.offset(bf$model.frame), start = if (is.null(bf$parameters)) 
  start
   else unlist(bf$parameters), hessian = bf$hessian, ...)
 }
 else {
   parallel_fun <- function(j) {
  if (j > 1 & !is.null(sleep)) 
  Sys.sleep(sleep)
  functions$sampler(x = bf$x, y = bf$y, family = bf$family, weights = model.weights(bf$model.frame), offset = model.offset(bf$model.frame), start = if (is.null(bf$parameters)) 
  start
  else unlist(bf$parameters), hessian = bf$hessian, ...)
   }
...
formula.bamlss.formula.character <- function(x, ...) {
 ...
 return(x)
  ...
  env <- if (grepl("globalenv", env, ignore.case = TRUE)) 
 .GlobalEnv
  else NULL
  for (i in seq_along(x)) {
 if (!is.null(names(x[[i]]))) {
...
samplestats <- function(samples, x = NULL, y = NULL, family = NULL, logLik = FALSE, ...) {
 ...
 x <- if (is.null(samples$x)) 
   smooth.construct(samples)
 else samples$x
 y <- samples$y
 ...
 if (length(pn) & ("DIC" %in% what)) {
  ...
  for (i in nx) mpar[[i]] <- make.link2(family$links[i])$linkinv(.fitted.bamlss(i, x[[i]], msamples))
  tpar <- mpar
...
bamlss.model.frame <- function(formula, data, family = gaussian_bamlss(), weights = NULL, subset = NULL, offset = NULL, na.action = na.omit, specials = NULL, contrasts.arg = NULL, drop.unused.levels = TRUE, ...) {
  if (!missing(data)) {
 if (is.character(data)) {
   if (!file.exists(data)) 
  stop("data path is not existing!")
   data_ff <- ff::read.table.ffdf(file = data, na.strings = "", header = TRUE, sep = ",")
   if (!is.null(weights)) 
 ...
 data <- as.data.frame(data)
  ...
  data <- list(formula = fF, data = data, subset = subset, na.action = na.action, drop.unused.levels = drop.unused.levels, ...)
 ...
 for (j in 1:ncol(weights)) weights[weights[, j] == 0, j] <- .Machine$double.eps
...
bamlss.family <- function(family, type = "bamlss", ...) {
 ...
 if (!is.character(family)) {
  ...
  stop("family is specified wrong, no family name available!")
   family <- family$family
 }
 txt <- paste(tolower(family), type, sep = if (!is.null(type)) 
   "_"
 else "")
...
complete.bamlss.family <- function(family) {
 ...
 family$map2par <- function(eta) {
   if (inherits(eta[[1]], "ff")) {
  for (j in family$names) {
  eta[[j]] <- ff_eval(eta[[j]], FUN = function(x) {
    linkinv[[j]](x)
 ...
 if (is.null(family$score[[i]]) & !is.null(family$d)) {
   fun <- c("function(y, par, ...) {", paste("  eta <- linkfun[['", i, "']](par[['", i, "']]);", sep = ""), paste("  par[['", i, "']] <- linkinv[['", i, "']](eta + err01);", sep = ""), "  d1 <- family$d(y, par, log = TRUE);", paste("  par[['", i, "']] <- linkinv[['", i, "']](eta - err01);", sep = ""), "  d2 <- family$d(y, par, log = TRUE);", "  return((d1 - d2) / err02)", "}")
 ...
 if (is.null(family$hess[[i]]) & !is.null(family$d)) {
  ...
  c("function(y, par, ...) {", paste("  eta <- linkfun[['", i, "']](par[['", i, "']]);", sep = ""), paste("  par[['", i, "']] <- linkinv[['", i, "']](eta + err11);", sep = ""), paste("  d1 <- family$score[['", i, "']](y, par, ...);", sep = ""), paste("  par[['", i, "']] <- linkinv[['", i, "']](eta - err11);", sep = ""), paste("  d2 <- family$score[['", i, "']](y, par, ...);", sep = ""), "  return(-1 * (d1 - d2) / err12)", "}")
   }
   else {
  c("function(y, par, ...) {", paste("  eta <- linkfun[['", i, "']](par[['", i, "']]);", sep = ""), paste("  par[['", i, "']] <- linkinv[['", i, "']](eta + err01);", sep = ""), paste("  d1 <- family$score[['", i, "']](y, par, ...);", sep = ""), paste("  par[['", i, "']] <- linkinv[['", i, "']](eta - err01);", sep = ""), paste("  d2 <- family$score[['", i, "']](y, par, ...);", sep = ""), "  return(-1 * (d1 - d2) / err02)", "}")
   }
...
bamlss.formula <- function(formula, family = NULL, specials = NULL, env = NULL, ...) {
 ...
 fn <- NULL
 for (j in seq_along(formula)) {
   ft <- if (!inherits(formula[[j]], "formula")) 
  formula[[j]][[1]]
   else formula[[j]]
   if (!is.null(ft)) {
 ...
 if (!is.null(family)) {
  ...
  fn <- family$names[1:length(fn)]
   else family$names[1:length(fn)] <- fn
 }
 ...
 if (any(j <- is.na(names(formula)))) {
  ...
  formula <- formula[!j]
   else names(formula) <- paste0(names(formula)[1], 1:length(formula))
 }
 formula
  }
...
get_formula_envir <- function(formula) {
 ...
 get_env <- function(x) {
   if (inherits(x, "list")) {
  env <- NULL
  for (j in x) env <- c(env, get_env(j))
  return(env)
   }
...
make_fFormula <- function(formula) {
  ...
  for (j in seq_along(formula)) {
 if (!all(c("formula", "fake.formula") %in% names(formula[[j]]))) {
   for (i in seq_along(formula[[j]])) fF <- c(fF, formula[[j]][[i]]$fake.formula)
 }
 else {
...
all_labels_formula <- function(formula, specials = NULL, full.names = FALSE) {
 ...
 for (j in sid) {
  ...
  tl[j] <- paste0(c("af", "lf.vd", "re", "peer", "fpc")[i], "(", all.vars(as.formula(paste("~", tl[j])))[1], ")")
   }
   else {
...
fake.formula <- function(formula, lhs = TRUE, rhs = TRUE, specials = NULL) {
 ...
 if (f == "") 
  ...
  if (all(lhs)) 
 f <- paste(all_vars_formula(formula, lhs = TRUE, rhs = FALSE, type = 2), "~", if (!is.null(f)) 
   f
 else 0)
  else f <- paste("~", if (!is.null(f)) 
 f
  else 0)
...
formula_extend <- function(formula, family, specials = NULL) {
  if (is.list(formula)) {
 for (j in seq_along(formula)) formula[[j]] <- formula_extend(formula[[j]], family, specials)
 return(formula)
  }
  else {
...
formula_insert <- function(from, to, formula) {
 ...
 formula <- formula0
 if (length(formula) > 1) {
   for (j in 2:length(formula)) formula[[j]] <- update(formula[[j]], NULL ~ .)
 }
  }
  formula
}
...
trans_random <- randomize <- function(x) {
 ...
 if (!all(elmts %in% names(x[[j]]))) {
   for (i in seq_along(x[[j]])) x[[j]][[i]] <- rand_fun(x[[j]][[i]])
 }
...
trans_AR1 <- AR1 <- function(rho = 0.1) {
 ...
 for (j in seq_along(x$x)) {
  ...
  for (i in seq_along(x$x[[j]])) x$x[[j]][[i]] <- trans_fun(x$x[[j]][[i]])
   }
...
c.bamlss <- function(...) {
  objects <- list(...)
  x <- NULL
  for (i in 1:length(objects)) x <- c(x, objects[i])
  Call <- match.call()
  names(x) <- as.character(Call[-1])
...
compute_s.effect <- function(x, get.X, fit.fun, psamples, FUN = NULL, snames, data, grid = -1, rug = TRUE) {
  nt <- length(x$term)
  for (j in seq_along(data)) {
 if (is.matrix(data[[j]])) {
   if (ncol(data[[j]]) < 2) 
  data[[j]] <- if (is.numeric(data[[j]])) 
  as.numeric(data[[j]])
  else drop(data[[j]])
 }
  }
 ...
 data[[x$term]]
  ...
  if (!is.na(grid)) {
 if (!any_f & !any(grepl("mrf", class(x))) & !any(grepl("re.", class(x), fixed = TRUE)) & !any(grepl("random", class(x)))) {
   xsmall <- TRUE
   nd <- list()
   for (j in tterms) {
 ...
 if (is.data.frame(data)) {
  ...
  if (!is.factor(data[[x$by]])) 
   ...
   xsmall <- if ((nrow(data) != nrow(data0)) & (nt < 2)) 
  TRUE
   else FALSE
 }
 else xsmall <- FALSE
 ...
 if (x$special) {
  ...
  if ((inherits(x, "mgcv.smooth") | inherits(x, "deriv.smooth")) & (nrow(psamples) > 39) & is.null(FUN)) {
 ...
 if (is.matrix(data[[tterms[l]]])) {
  ...
  smf <- cbind(as.numeric(data[[tterms[l]]]), smf)
   else smf <- cbind(data[[tterms[l]]], smf)
 }
 else {
 ...
 smf <- unique(smf)
  ...
  if (!is.list(data)) {
 attr(smf, "x") <- if (xsmall & nt < 2) 
   data0[, tterms]
 else data[, tterms]
  }
  else {
 for (t in tterms) data[[t]] <- if (is.matrix(data[[t]])) 
   as.numeric(data[[t]])
 else drop(data[[t]])
...
predict.bamlss <- function(object, newdata, model = NULL, term = NULL, match.names = TRUE, intercept = TRUE, type = c("link", "parameter"), FUN = function(x) {
  mean(x, na.rm = TRUE)
}, trans = NULL, what = c("samples", "parameters"), nsamps = NULL, verbose = FALSE, drop = TRUE, cores = NULL, chunks = 1, ...) {
 ...
 if (is.function(family$predict)) {
   return(family$predict(object = object, newdata = newdata, model = model, term = term, intercept = intercept, type = type, FUN = FUN2, trans = trans, what = what, nsamps = nsamps, verbose = verbose, drop = drop, cores = cores, chunks = chunks, ...))
 }
  }
  if (is.null(object$x)) {
 ...
 names(enames) <- nx
  ...
  if (intercept) {
 intcpt <- unlist(lapply(enames, function(x) {
   any(grepl("intercept", tolower(x)))
 }))
 if (any(!intcpt)) {
 ...
 samps <- as.mcmc(samps)
  ...
  cn <- colnames(samps)
  drop2 <- grep2(c(".tau2", ".alpha", ".edf", ".accepted", ".dic", ".loglik", ".logpost"), tolower(cn), fixed = TRUE)
  if (length(drop2)) 
 cn <- cn[-drop2]
 ...
 return(list(samps = samps, enames = enames, intercept = intercept, FUN = FUN2, trans = trans, type = type, nsamps = nsamps, env = env))
  }
 ...
 if (type != "link") {
  ...
  warning(paste("could not compute predictions on the scale of parameter", ", predictions on the scale of the linear predictor are returned!", sep = ""))
   }
 }
 if (!is.null(trans)) {
   if (!is.list(trans)) {
 ...
 if (chunks > 1) {
  ...
  if (j < 2) {
    pred[[i]] <- pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, newdata[[j]]), id = i)
  }
  else {
    if (is.null(dim(pred[[i]]))) {
    pred[[i]] <- c(pred[[i]], pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, newdata[[j]]), id = i))
    }
    else {
    pred[[i]] <- rbind(pred[[i]], pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, newdata[[j]]), id = i))
    }
  }
  }
  if (verbose) 
  cat("\n")
   ...
   for (i in nx) {
  pred[[i]] <- pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, newdata), id = i)
   }
 }
  }
  else {
 parallel_fun <- function(k) {
  ...
  for (j in 1:chunks) {
    ...
    if (j < 2) {
    pred[[i]] <- pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, nd[[j]]), id = i)
    }
    else {
    if (is.null(dim(pred[[i]]))) {
   pred[[i]] <- c(pred[[i]], pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, nd[[j]]), id = i))
    }
    else {
   pred[[i]] <- rbind(pred[[i]], pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, nd[[j]]), id = i))
    }
    }
  }
  }
  ...
  for (i in nx) {
  pred[[i]] <- pred_fun(.predict.bamlss(i, object$x[[i]], samps, enames[[i]], intercept, nsamps, newdata[[k]]), id = i)
  }
   }
   return(pred)
 }
...
.predict.bamlss <- function(id, x, samps, enames, intercept, nsamps, data) {
 ...
 for (j in enames2[i]) {
   if (j != "(Intercept)") {
  f <- as.formula(paste("~", if (has_intercept) 
  "1"
  else "-1", "+", j))
  ...
  ok <- NULL
  for (jjt in jt) ok <- cbind(ok, grepl(jjt, snames, fixed = TRUE), grepl(paste(id, "p", sep = "."), snames, fixed = TRUE) | grepl(paste(id, "p.model.matrix", sep = "."), snames, fixed = TRUE))
  ok <- apply(ok, 1, all)
  sn <- snames[ok]
  }
  else {
  ...
  if (any(grepl("model.matrix.", sn2))) 
    sn2 <- gsub("model.matrix.", "", sn2, fixed = TRUE)
  X <- X[, sn2, drop = FALSE]
  }
  if (ncol(X) < length(sn)) {
  if (any(grepl("model.matrix", sn))) 
    sn <- paste(id, "p.model.matrix", colnames(X), sep = ".")
  else sn <- paste(id, "p", colnames(X), sep = ".")
  }
 ...
 if (length(m) > 1) {
   ...
   y2 <- sapply(strsplit(y, "(", fixed = TRUE), function(x) {
  paste0(x[1], "(")
   })
   m <- y[y2 %in% x]
 }
 ...
 for (j in enames2[i]) {
  ...
  if (!is.null(x[[jj]]$is.refund)) {
    ...
    tfme <- eval(tfm$call, envir = tfm$data)
    X <- smoothCon(tfme, data = tfm$data, n = nrow(tfm$data[[1]]), knots = NULL, absorb.cons = TRUE, scale.penalty = TRUE)[[1]]$X
    rm(tfm)
    rm(tfme)
  }
  else {
...
.fitted.bamlss <- function(id, x, samps) {
 ...
 for (j in names(x$smooth.construct)) {
   ptxt <- paste(id, if (j != "model.matrix") 
  "s"
   else "p", j, sep = ".")
   if (j != "model.matrix") 
  ptxt <- paste0(ptxt, ".")
...
smooth.construct.rsc.smooth.spec <- function(object, data, knots, ...) {
 ...
 if (length(vars)) {
   ...
   for (j in vars) {
  rs.by[[j]] <- data[[j]]
  if (!is.factor(rs.by[[j]])) 
  stop("random scaling by variables must be factors!")
   }
   n <- length(vars)
   g <- paste("g[", 1:n, "]", sep = "", collapse = " + ")
   fun <- paste("function(X, g) { ", "(", if (attr(ft, "intercept")) 
  "1 + ", g, ") * (X %*% ", if (n > 1) 
  paste("g[-(1:", n, ")]", sep = "")
   else "g[-1]", ") }", sep = "")
...
rs <- function(formula, link = "log", ...) {
 ...
 nd <- FALSE
  label <- if (nd) 
 paste("rs(", fnl, "|", fdl, ")", sep = "")
  else paste("rs(", fnl, ")", sep = "")
 ...
 xt$do.optim <- TRUE
  rval <- list(formula = formula, term = unique(c(vn, vd)), label = label, special = TRUE, link = link, xt = xt, by = "NA", nspecials = kn + kd)
...
smooth.construct.rs.smooth.spec <- function(object, data, knots, ...) {
 ...
 object$xt$center
  ...
  object$X <- bamlss.engine.setup(design.construct(object$formula, data = data, knots = knots), df = rep(object$xt$df, object$nspecials))
 ...
 if (ncol(object$X[[2]]$smooth.construct$model.matrix$X) < 1) {
   ...
   object$X[[2]]$smooth.construct$model.matrix$term <- gsub("(Intercept)+", "", object$X[[2]]$smooth.construct$model.matrix$term, fixed = TRUE)
 ...
 for (sj in seq_along(object$X[[j]]$smooth.construct)) {
  ...
  "n"
   ...
   names(object$X[[j]]$smooth.construct[[sj]]$state$parameters) <- paste(paste(pn, sj, sep = ""), names(object$X[[j]]$smooth.construct[[sj]]$state$parameters), sep = ".")
  ...
  object$xmat <- cbind(object$xmat, object$X[[j]]$smooth.construct[[sj]]$X)
   else object$zmat <- cbind(object$zmat, object$X[[j]]$smooth.construct[[sj]]$X)
 }
  }
 ...
 hessfun <- function(b) {
   ...
   Hn <- if (!is.null(x$zmat)) 
  crossprod(x$zmat * (w_scale^2 * hess), x$zmat)
   else NULL
 ...
 if (length(start <- get.par(x$state$parameters, "tau2")) & x$xt$do.optim) {
  ...
  if ((ic < env$ic_val) & (ic < env$ic00_val)) {
    opt_state <- list(parameters = par2, fitted.values = fit, edf = edf3, nu = nu)
 ...
 for (i in 1:2) {
  ...
  for (jj in seq_along(x$X[[i]]$smooth.construct[[j]]$S)) S <- S + 1/tau2[jj] * x$X[[i]]$smooth.construct[[j]]$S[[jj]]
 ...
 hessfun <- function(b, score, hess) {
   ...
   Hn <- if (!is.null(data$zmat)) 
  crossprod(data$zmat * (w_scale^2 * hess), data$zmat)
   else NULL
   return(if (!is.null(Hn)) as.matrix(do.call("bdiag", list(Hd, Hn))) else as.matrix(Hd))
 }
 peta <- family$map2par(eta)
 ...
 if (length(i)) {
   for (j in i) {
  theta2 <- uni.slice(theta2, data, NULL, NULL, NULL, id[1], j, logPost = gmcmc_logPost, lower = 0, ll = pibetaprop)
   }
 }
 ...
 return(list(parameters = theta2, alpha = alpha, extra = c(edf = edf)))
  ...
  object$state <- list(parameters = parameters, fitted.values = rep(0, nrow(object$X[[1]]$smooth.construct[[1]]$X)), edf = edf)
...
rs.plot <- function(x, model = NULL, term = NULL, what = c("numerator", "denominator"), type = "link", ...) {
 ...
 for (w in what) {
  ...
  pl[[k]] <- compute_s.effect(x$x[[pn]]$smooth.construct[[i]]$X[[w]]$smooth.construct[[ii]], get.X, x$x[[pn]]$smooth.construct[[i]]$X[[w]]$smooth.construct[[ii]]$fit.fun, samps[, sn, drop = FALSE], FUN = FUN, sn, model.frame(x), grid = -1, rug = TRUE)
...
la <- function(formula, type = c("single", "multiple"), ...) {
 ...
 if (any(grepl(":", formula, fixed = TRUE))) {
  ...
  rval <- list(formula = formula, term = vars, label = if (is.null(label)) paste("la(~", paste(vars, collapse = "+"), ")", sep = "") else label, type = match.arg(type), by = "NA")
  rval$dim <- length(rval$term)
  rval$special <- TRUE
  rval$xt <- list(...)
...
smooth.construct.la.smooth.spec <- function(object, data, knots, ...) {
  ridge <- if (is.null(object$xt[["ridge"]])) 
 FALSE
  else object$xt[["ridge"]]
  enet <- if (is.null(object$xt[["enet"]])) 
 FALSE
  else object$xt[["enet"]]
  fuse <- if (is.null(object$xt[["fuse"]])) 
 FALSE
  else object$xt[["fuse"]]
  standardize <- if (is.null(object$xt[["standardize"]])) 
 FALSE
  else object$xt[["standardize"]]
  standardize01 <- if (is.null(object$xt[["standardize01"]])) 
 FALSE
  else object$xt[["standardize01"]]
  fuse_type <- "nominal"
  if (is.logical(fuse)) {
 if (fuse) 
   fuse <- "nominal"
 ...
 if (is.character(fuse)) {
   ...
   fuse_type <- switch(as.integer(fuse), `1` = "nominal", `2` = "ordered")
 }
 fuse <- TRUE
  }
  object$fuse <- fuse
  object$fuse_type <- fuse_type
  object$standardize <- if (!fuse) 
 TRUE
  else standardize
 ...
 object$X[[j]] <- as.matrix(model.matrix(as.formula(paste("~", j)), data = data, contrasts = contr.list))
 ...
 if (!fuse | standardize) {
  ...
  if (standardize01) {
   ...
   object$X[[j]][, jj] <- object$X[[j]][, jj] * 2 - 1
    ...
    object$lasso$trans[[j]] <- list(center = attr(object$X[[j]], "scaled:center"), scale = attr(object$X[[j]], "scaled:scale"))
  }
  }
   }
   else {
 ...
 if (!fuse | standardize) {
   ...
   object$lasso$trans[[j]] <- list(center = 0, scale = 1)
   if (is.factor(data[[j]])) {
 ...
 if (object$type == "single") {
  ...
  f <- c("function(parameters) {", "  b <- get.par(parameters, \"b\")", "  A <- diag(0, length(b))", paste("  A[", j, ",", j, "] <- df[", j, "] / sqrt(b[", j, "]^2 + const)", sep = ""), "  A", "}")
  ...
  object$xt$gfx <- if (is.null(object$xt$gfx)) 
 FALSE
  else object$xt$gfx
  if (fuse & !ridge) {
 k <- ncol(object$X)
 if (fuse_type == "nominal") {
  ...
  for (ii in 1:length(ucn1)) {
    tAf <- rep(0, k)
    tAf[cn[, 2] == ucn2[combis[1, ff]] & cn[, 1] == ucn1[ii]] <- 1
    tAf[cn[, 2] == ucn2[combis[2, ff]] & cn[, 1] == ucn1[ii]] <- -1
    Af <- cbind(Af, tAf)
  }
  }
 ...
 object$S <- list(diag(1, ncol(object$X)))
  if (enet) 
 object$S[[length(object$S) + 1]] <- diag(1, ncol(object$X))
  object$xt[["prior"]] <- "ig"
  object$xt[["a"]] <- 1e-10
  object$xt[["b"]] <- 10000
  object$fixed <- if (is.null(object$xt[["fx"]])) 
 FALSE
  else object$xt[["fx"]]
 ...
 object$xt$lambda <- if (is.null(object$xt[["sp"]])) 
   1e-04
 else object$xt[["sp"]]
  }
  else {
 ...
 object$xt[["df"]] <- if (!ridge) 
   ceiling(ncol(object$X) * 0.9)
 else ceiling(ncol(object$X) * 0.3)
  object$ctype <- switch(object$type, single = 0, multiple = 1)
...
Predict.matrix.lasso.smooth <- function(object, data) {
 ...
 if (!is_f) {
  ...
  for (jj in 1:ncol(X[[j]])) X[[j]][, jj] <- (X[[j]][, jj] - xmin[jj])/(xmax[jj] - xmin[jj])
  if (!is.null(object$xt$m1p1)) 
    X[[j]][, jj] <- X[[j]][, jj] * 2 - 1
  }
  else {
...
smooth.construct.linear.smooth.spec <- function(object, data, knots, ...) {
 ...
 TRUE
  ...
  neigh <- if (is.null(object$xt$neigh)) 
 FALSE
  else object$xt$neigh
  if (ridge | neigh) {
 if (ridge) 
   object$S <- list(diag(ncol(object$X)))
 else object$S <- list(crossprod(diff(diag(ncol(object$X)))))
 object$xt$df <- if (is.null(object$xt$df)) 
   min(c(floor(length(object$scale$center)/2), 2))
 else object$xt$df
  }
  else {
 object$fixed <- TRUE
...
n.weights <- function(nodes, k, r = NULL, s = NULL, type = c("sigmoid", "gauss", "softplus", "cos", "sin"), x = NULL, ...) {
 ...
 wts <- t.weights(x, y, k = nodes, n = list(...)$tntake, dropout = list(...)$dropout, weights = list(...)$weights, wts = list(...)$wts, maxit = list(...)$maxit)
 return(wts)
  }
  type <- match.arg(type)
 ...
 if (!is.null(nodes$parameters)) {
  ...
  terms <- unique(paste0(sapply(strsplit(names(cb1), ").b", fixed = TRUE), function(x) {
    x[1]
  }), ")"))
  if (length(terms) > 1) {
    rval[[j]] <- list()
    ...
    id <- as.integer(sapply(strsplit(names(cb2), ").b", fixed = TRUE), function(x) {
   x[2]
    }))
    id <- id[abs(cb2) > 1e-10]
    rval[[j]][[tj]] <- id
    ...
    id <- as.integer(sapply(strsplit(names(cb1), ").b", fixed = TRUE), function(x) {
    x[2]
    }))
    id <- id[abs(cb1) > 1e-10]
    rval[[j]] <- list()
 ...
 weights <- lapply(1:nodes, function(i) {
   sw <- switch(type, sigmoid = runif(1, log((1 - r[i])/r[i]), s[i] * log((1 - r[i])/r[i])), gauss = runif(1, sqrt(-log(r[i])), s[i] * sqrt(-log(r[i]))), softplus = runif(1, -log(exp(r[i]) - 1), s[i] * -log(exp(r[i]) - 1)), cos = runif(1, acos(r[i]), s[i] * acos(r[i])), sin = runif(1, acos(r[i]), s[i] * acos(r[i])))
   w <- runif(k - 1, -1, 1)
   if (!is.null(dropout)) {
...
nnet2Zmat <- function(X, weights, afun) {
 ...
 afun <- switch(i, relu = function(x) {
   x[x < 0] <- 0
   x
 }, sigmoid = function(x) {
   1/(1 + exp2(-x))
 }, tanh = tanh, sin = sin, cos = sin, gauss = function(x) {
   exp2(-x^2)
 }, identity = function(x) {
   x
 }, softplus = function(x) {
...
smooth.construct.nnet0.smooth.spec <- function(object, data, knots, ...) {
 ...
 if (nodes < 0) 
  ...
  if (is.null(object$xt$weights)) {
 object$xt$afun <- if (is.null(object$xt$afun)) 
   "sigmoid"
 else object$xt$afun
 type <- object$xt$afun
  }
 ...
 names(object$xt$sint) <- type
  ...
  if (is.null(object$xt$weights)) {
 nobs <- nrow(object$X)
 object$xt[["tx"]] <- object$X[sample(1:nobs, size = nodes, replace = if (nodes >= nobs) 
   TRUE
 else FALSE), -1, drop = FALSE]
 ...
 object$sample_weights <- function(x = NULL, y = NULL, weights = NULL, wts = NULL) {
  ...
  y <- NULL
   ...
   n.weights(nodes, ncol(object$X) - 1, rint = object$xt$rint[[1]], sint = object$xt$sint[[1]], type = type[1], x = x, dropout = object$xt[["dropout"]], y = y, tntake = object$xt[["nobs"]], wm = object$xt[["wm"]], weights = weights, wts = wts, maxit = object$xt[["maxit"]])
 }
 ...
 object$S[[k]] <- A
  ...
  object$xt$center <- if (is.null(object$xt$center)) 
 FALSE
  else object$xt$center
  object$by <- "NA"
  ...
  object$activ_fun <- switch(type[1], relu = function(x) {
 x[x < 0] <- 0
 return(x - mean(x))
  }, sigmoid = function(x) {
 f <- 1/(1 + exp2(-x))
 ...
 return(f - mean(f))
  ...
  object$activ_grad <- switch(type[1], relu = function(x) {
 x[x > 0] <- 1
 x[x <= 0] <- 0
 return(x)
  }, sigmoid = function(x) {
 ...
 1 - tanh(x)^2
  }, sin = cos, cos = sin, gauss = function(x) -(exp2(-x^2) * (2 * x)), softplus = function(x) exp2(x)/(1 + exp2(x)))
  object$activ_hess <- switch(type[1], relu = function(x) {
 return(rep(0, length(x)))
  }, sigmoid = function(x) {
 exp2(-x)/(1 + exp2(-x))^2 * (1 - 1/(1 + exp2(-x))) - 1/(1 + exp2(-x)) * (exp2(-x)/(1 + exp2(-x))^2)
  }, tanh = function(x) {
 -(2 * (1/cosh(x)^2 * tanh(x)))
  }, sin = sin, cos = cos, gauss = function(x) -(exp2(-x^2) * 2 - exp2(-x^2) * (2 * x) * (2 * x)), softplus = function(x) exp2(x)/(1 + exp2(x)) - exp2(x) * exp2(x)/(1 + exp2(x))^2)
  nc <- ncol(object$X) - 1
  if (oc) {
 object$oc <- list()
...
nnet0_update <- function(x, family, y, eta, id, weights, criterion, ...) {
 ...
 gr <- score * cbind(Z[, j], w[1] * x$activ_grad(Xw) * x$X)
 gr <- colSums(gr) - drop(I %*% w)
 return(-gr)
  }
  objfun <- function(w, i, j, fit) {
 Z[, j] <- x$activ_fun(drop(x$X %*% w[-1]))
 fit <- fit + Z[, j] * w[1]
 fit <- fit - mean(fit)
 eta[[id]] <- eta[[id]] + fit
 ...
 fit <- fit - Z[, j] * par[j]
 opt <- try(optim(c(par[j], par[i]), fn = objfun, gr = gradfun, method = "BFGS", i = i, j = j, fit = fit), silent = TRUE)
 if (!inherits(opt, "try-error")) {
   if (((-1 * opt$value) > ll0) & (-1 * opt$value > lls)) {
  par[i] <- opt$par[-1]
  par[j] <- opt$par[1]
  Z[, j] <- x$activ_fun(drop(x$X %*% opt$par[-1]))
   }
 }
 fit <- fit + Z[, j] * par[j]
 fit <- fit - mean(fit)
...
nnet0_propose <- function(family, theta, id, eta, y, data, weights = NULL, offset = NULL, ...) {
  ...
  id <- id[1]
  if (!is.null(offset)) {
 for (j in names(offset)) eta[[j]] <- eta[[j]] + offset[[j]]
  }
  if (is.null(attr(theta, "fitted.values"))) 
 attr(theta, "fitted.values") <- data$fit.fun(data$X, theta)
 ...
 eta[[id]] <- eta[[id]] + Zj * w[1]
 score <- family$score[[id]](y, family$map2par(eta))
 gr <- score * cbind(Zj, w[1] * data$activ_grad(Xw) * data$X)
 gr <- colSums(gr) - drop(I %*% w)
 return(gr)
  }
 ...
 h0 <- sum(Zj^2 * hess)
 dh <- w[1]^2 * data$activ_grad(Xw)^2 * hess + w[1] * data$activ_hess(Xw) * score
 h1 <- crossprod(data$X * dh, data$X)
 h2 <- data$X * w[1] * Zj * data$activ_grad(Xw) * hess + data$X * data$activ_grad(Xw) * score
 h2 <- colSums(h2)
 ...
 a <- b <- 1e-04
 igs <- log((b^a)) - log(gamma(a))
 lp <- -log(tau2) * (ncol(data$X) + 1)/2 + drop(-0.5/tau2 * t(gamma) %*% I %*% gamma) + (igs + (-a - 1) * log(tau2) - b/tau2)
 lp
  }
 ...
 Xw <- drop(data$X %*% theta2[-1])
 fj1 <- data$activ_fun(Xw) * theta2[1]
 eta[[id]] <- eta[[id]] + fj1
 ...
 if (!is.finite(alpha)) 
   alpha <- NA
 accepted <- if (is.na(alpha)) 
   FALSE
 else log(runif(1)) <= alpha
 if (accepted) {
   theta[j] <- theta2[1]
   theta[i] <- theta2[-1]
   fit <- fit - fj0 + fj1
  ...
  return(list(parameters = theta, alpha = log(1), extra = c(edf = data$nodes * ncol(data$X))))
}
nnet00_update <- function(x, family, y, eta, id, weights, criterion, ...) {
 ...
 for (l in 1:nobs) {
  ...
  for (jj in 1:nc) {
  if (ii <= jj) {
    h[ii, jj] <- h[ii, jj] + x$X[l, ii] * x$X[l, jj] * x$activ_hess(Xw[l]) * score[l] + x$X[l, ii] * x$X[l, jj] * x$activ_grad(Xw[l])^2 * hess[l]
    h[jj, ii] <- h[ii, jj]
  }
  }
   }
 ...
 return(h)
  ...
  for (j in 1:x$nodes) {
 i <- paste0("bw", j, "_w", 0:(nc - 1))
 opt <- optim(par[i], fn = objfun, gr = gradfun, method = "L-BFGS-B", i = i, j = j)
 if (inherits(opt, "try-error")) 
   next
...
smooth.construct.nnet2.smooth.spec <- function(object, data, knots, ...) {
 ...
 object$xt$k <- object$bs.dim
  ...
  if (!is.null(object$xt$weights)) 
 nodes <- length(object$xt$weights)
  npen <- if (is.null(object$xt$npen)) 
 1
  else object$xt$npen
  dotake <- FALSE
  if (length(nodes) < 2) {
 ...
 for (j in type) {
   object$n.weights[[j]] <- n.weights(nodes, ncol(object$X) - 1, rint = object$xt$rint[[j]], sint = object$xt$sint[[j]], type = j, x = object$xt[["tx"]], dropout = object$xt[["dropout"]])
 }
  }
  else {
 if (length(object$xt$weights) != nodes) 
 ...
 object$n.weights <- object$xt$weights
  ...
  object$Xkeep <- which(apply(object$X, 2, function(x) {
 abs(diff(range(x)))
  }) > 1e-05)
 ...
 for (j in object$term) {
  ...
  object$sm[[j]] <- smoothCon(sj, data, knots = NULL, absorb.cons = TRUE, sparse.cons = 0, scale.penalty = TRUE)[[1]]
  }
   }
 }
 sm <- do.call("cbind", lapply(object$sm, function(x) {
   x$X
 }))
 smL <- qr(sm)
 ...
 object$xt$ndf <- 4
  else object$xt$ndf <- ceiling(object$xt$ndf)
  if (object$xt$ndf < 2) 
 object$xt$ndf <- NULL
...
nn.fit <- function(x, y, k = 50, size = min(c(floor(ncol(x)/2), 50))) {
 ...
 for (j in sample(id, size = size, replace = FALSE)) {
  ...
  rval <- list(coefficients = c(my, apply(coefs, 2, mean)))
...
subset_features <- function(x, eps = 0.01) {
 ...
 rss <- NULL
 for (j in cols) {
   xs <- x[, if (is.null(drop)) 
  -j
   else c(-drop, -j), drop = FALSE]
   P <- xs %*% matrix_inv(crossprod(xs) + diag(1e-04, ncol(xs))) %*% t(xs)
...
forward_reg <- function(x, y, n = 4, ...) {
  ...
  while (k < n) {
 rss <- NULL
 for (j in cols) rss <- c(rss, sum(lm.fit(cbind(1, x[, c(take, j), drop = FALSE]), y)$residuals^2) + 2 * length(c(take, j)))
...
Predict.matrix.nnet0.smooth <- function(object, data) {
  object[["standardize"]] <- standardize <- if (is.null(object$xt[["standardize"]])) 
 TRUE
  else object$xt[["standardize"]]
  object[["standardize01"]] <- if (standardize) 
 TRUE
  else FALSE
...
if (FALSE) {
  n <- 600
  d <- data.frame(x1 = runif(n, -3, 3), x2 = runif(n, -3, 3))
  ...
  abline(0, 1)
  mse <- c(GAM = mean((dtest$y - p0)^2), `GAM+NET` = mean((dtest$y - p1)^2))
  print(mse)
}
nnet.fit <- function(X, y, nodes = 20, ..., random = FALSE, w = NULL, lambda = 0.001, optim = FALSE, maxit = 100, nu = 0.1) {
  nc <- ncol(X)
  if (is.null(w)) 
 w <- n.weights(nodes, k = nc, type = "sigmoid", x = X, ...)
  ...
  for (j in 1:nodes) Z[, j] <- 1/(1 + exp(-1 * drop(X %*% par[paste0("bw", j, "_w", 0:nc)])))
 ...
 return(nm)
  ...
  if (random) {
 rval <- list(fitted.values = ffn(X, par), coefficients = par, nodes = nodes, converged = TRUE)
 class(rval) <- "nnet.fit"
 return(rval)
  }
 ...
 return(sum((y - fit)^2))
  ...
  i <- grep("bw", names(par))
  if (optim) {
 opt <- optim(par = par[i], fn = objfun, gr = gradfun, method = "BFGS", X = X, y = y)
 par[i] <- opt$par
 Z <- ffm(X, opt$par)
 beta <- chol2inv(chol(crossprod(Z) + diag(nodes) * lambda)) %*% t(Z) %*% y
 fit <- drop(Z %*% beta)
 ...
 while (iter < maxit) {
  ...
  rval <- list(fitted.values = fit, coefficients = par[-i], weights = par[i], initial = unlist(w0), nodes = nodes, converged = if (optim) opt$convergence == 1 else NA)
  class(rval) <- "nnet.fit"
  return(rval)
}
...
boost.fit.nnet <- function(x, y, nu, hatmatrix = FALSE, weights = NULL, ...) {
 ...
 stop("weights is not supported!")
  ...
  b <- nnet.fit(x$X[i, , drop = FALSE], y[i], x$xt$k, rint = x$xt$rint, sint = x$xt$sint, w = if (x$xt$passw) 
 x$state$parameters[grep("bw", names(x$state$parameters))]
  else NULL)
...
smooth.construct.nnet.smooth.spec <- function(object, data, knots, ...) {
 ...
 object$xt$fx <- FALSE
  ...
  tp <- if (is.null(object$xt$tp)) 
 TRUE
  else object$xt$tp
  object[["standardize01"]] <- object$xt[["standardize01"]] <- if (tp) 
 FALSE
  else TRUE
 ...
 if (!object$xt[["standardize"]]) {
  ...
  w <- unlist(n.weights(object$xt$k, k = ncol(object$X), type = "sigmoid", rint = object$xt$rint, sint = object$xt$sint, x = object$X))
 ...
 stop("no nnet updating function for bfit() implemented yet!")
  }
  object$propose <- function(...) {
 stop("no nnet proposal function for GMCMC() implemented yet!")
  }
...
predictn <- function(object, newdata, model = NULL, mstop = NULL, type = c("link", "parameter")) {
 ...
 if (any(j <- (grepl("n(", tl[[i]], fixed = TRUE)) & !grepl("lin(", tl[[i]], fixed = TRUE))) {
  ...
  pt <- predict(object, newdata = newdata, model = i, term = tj, FUN = function(x) {
    x
  }, intercept = FALSE)
 ...
 p[[i]] <- if (is.null(mstop)) 
   fit
 else fit[, if (is.list(mstop)) 
   mstop$mstop
 else mstop]
 if (type != "link") {
   link <- family$links[i]
...
smooth.construct.randombits.smooth.spec <- function(object, data, knots, ...) {
 ...
 object$sample_weights <- function(...) {
  ...
  attr(weights[[i]], "thres") <- sample(1:nrow(object$X), size = 1)
   }
   return(weights)
 }
 ...
 object$X <- tXw
  ...
  object$Xkeep <- which((d > 1e-05) & !duplicated(t(object$X)))
  object$X <- object$X[, object$Xkeep, drop = FALSE]
  if (ncol(object$X) < 1) 
...
make_weights <- function(object, data, dropout = 0.2) {
 ...
 return(rval[[i]])
  else return(rval$xt[[grep("weights", names(rval$xt))]])
}
...
if (FALSE) {
  set.seed(123)
  nobs <- 2000
  d <- data.frame(x1 = runif(nobs, 0, pi), lon = runif(nobs, -3, 3), lat = runif(nobs, -3, 3), z1 = runif(nobs, 0, 1), z2 = runif(nobs, 0, 1), z3 = runif(nobs, 0, 1), z4 = runif(nobs, 0, 1), z5 = runif(nobs, 0, 1))
...
smooth.construct.ha.smooth.spec <- function(object, data, knots, ...) {
  x <- data[[object$term]]
  freq <- if (is.null(object$xt$frequency)) 
 ...
 else object$p.order <- c(object$p.order, 2)
  }
 ...
 X <- X[, -(2 * order)]
  object$X <- X
  gsin1 <- function(x) {
 cos(2 * pi * order * x) * 2 * pi * order
  }
  gsin2 <- function(x) {
 4 * pi^2 * order^2 * -sin(2 * pi * order * x)
  }
  gcos1 <- function(x) {
 -sin(2 * pi * order * x) * 2 * pi * order
  }
  gcos2 <- function(x) {
 -4 * pi^2 * order^2 * cos(2 * pi * order * x)
  }
  if (!object$fixed) {
...
krDesign1D <- function(z, knots = NULL, rho = NULL, phi = NULL, v = NULL, c = NULL, ...) {
  rho <- if (is.null(rho)) {
 matern
  }
  else rho
...
krDesign2D <- function(z1, z2, knots = 10, rho = NULL, phi = NULL, v = NULL, c = NULL, psi = NULL, delta = 1, isotropic = TRUE, ...) {
  rho <- if (is.null(rho)) {
 matern
  }
  else rho
 ...
 isotropic <- TRUE
  if (is.null(knots)) 
 knots <- min(c(10, nrow(unique(cbind(z1, z2)))), na.rm = TRUE)
 ...
 2.5
  ...
  c <- if (is.null(c)) {
 optim(1, rho, phi = 1, kappa = v, method = "L-BFGS-B", lower = 1e-10)$par
  }
  else c
  z <- cbind(z1, z2)
 ...
 R <- matrix(c(cos(psi), -1 * sin(psi), sin(psi), cos(psi)), 2, 2)
 ...
 for (i in 1:nrow(z)) {
  ...
  return(list(B = B, K = K, knots = knots, phi = phi, v = v, c = c, psi = psi, delta = delta))
}
smooth.construct.kr.smooth.spec <- function(object, data, knots, ...) {
  if (object$dim > 2) 
 stop("more than 2 covariates not supported using kriging terms!")
  if (object$bs.dim < 0) 
 object$bs.dim <- 10
  if (object$dim < 2) {
 k <- knots[[object$term]]
 ...
 D <- krDesign1D(x, knots = k, rho = object$xt$rho, phi = object$xt$phi, v = object$xt[["v"]], c = object$xt[["c"]])
  }
  else {
 knots <- if (is.null(object$xt$knots)) 
   object$bs.dim
 else object$xt$knots
 D <- krDesign2D(data[[object$term[1]]], data[[object$term[2]]], knots = knots, phi = object$xt$phi, v = object$xt[["v"]], c = object$xt[["c"]], psi = object$xt$psi, delta = object$xt$delta, isotropic = object$xt$isotropic)
  }
  X <- D$B
  object$X <- X
  object$S <- list(D$K)
...
Predict.matrix.kriging.smooth <- function(object, data) {
  if (object$dim < 2) {
 X <- krDesign1D(data[[object$term]], knots = object$knots, rho = object$xt$rho, phi = object$xt$phi, v = object$xt$v, c = object$xt$c)$B
  }
  else {
 X <- krDesign2D(data[[object$term[1]]], data[[object$term[2]]], knots = object$knots, phi = object$xt$phi, v = object$xt$v, c = object$xt$c, psi = object$xt$psi, delta = object$xt$delta, isotropic = object$xt$isotropic)$B
  }
  X
}
smooth.construct.str.smooth.spec <- function(object, data, knots, ...) {
  if (object$dim < 3) 
 stop("need at least 3 variables!")
  if (object$bs.dim < 0) 
 object$bs.dim <- 10
  knots <- if (is.null(object$xt$knots)) 
 object$bs.dim
 ...
 trend <- trend[[1]] + scale2(trend[[2]], 0, 1)
  ...
  D <- krDesign2D(co1[, 1], co1[, 2], knots = knots, phi = object$xt$phi, v = object$xt$v, c = object$xt$c, psi = object$xt$psi, delta = object$xt$delta, isotropic = object$xt$isotropic)
 ...
 stop()
  ...
  object$knots <- D$knots
  object$state <- list(parameters = list(b = b, tau2 = c(0.001, 0.001)), fitted.values = rep(0, length(trend)))
  class(object) <- c("strandom.smooth", "no.mgcv", "special")
  object
}
Predict.matrix.strandom.smooth <- function(object, data) {
  D <- krDesign2D(data[[object$term[1]]], data[[object$term[2]]], knots = object$knots, phi = object$xt$phi, v = object$xt$v, c = object$xt$c, psi = object$xt$psi, delta = object$xt$delta, isotropic = object$xt$isotropic)
...
traceplot2 <- function(theta, n.plot = 100, ylab = "", ...) {
 ...
 as.numeric(quantile(x[1:n], c(0.025, 0.5, 0.975), na.rm = TRUE))
  }, vectorize.args = "n")
  n.rep <- length(theta)
  plot(1:n.rep, theta, col = "lightgrey", xlab = "Iteration", ylab = ylab, type = "l", ...)
...
plot.bamlss <- function(x, model = NULL, term = NULL, which = "effects", parameters = FALSE, ask = dev.interactive(), spar = TRUE, ...) {
  if (spar) {
 op <- par(no.readonly = TRUE)
 on.exit(par(op))
  }
 ...
 ask <- FALSE
  ...
  which.match <- c("effects", "samples", "hist-resid", "qq-resid", "wp", "scatter-resid", "max-acf", "param-samples", "boost_summary", "results", "max-acf")
 ...
 if (length(which2)) {
  ...
  mean(x, na.rm = TRUE)
   ...
   for (w in which) {
  plot.bamlss(x, model = model, term = term, which = w, parameters = parameters, ask = ask, spar = spar, ...)
   }
 }
  }
  else {
 if (which %in% c("samples", "max-acf")) {
  ...
  acf(if (nu < 2) 
    jitter(samps[, j, drop = FALSE])
  else samps[, j, drop = FALSE], main = "", ..., na.action = na.pass)
  ...
  samps <- samps[, snames, drop = FALSE]
  macf <- apply(samps, 2, function(x) {
  acf(x, plot = FALSE, ...)
  })
  acfx <- macf[[1]]
  acfx$acf <- array(apply(do.call("rbind", lapply(macf, function(x) {
  x$acf
  })), 2, max, na.rm = TRUE), dim = c(length(acfx$acf), 1, 1))
  args <- list(...)
  ...
  args$ylab <- "ACF"
  getS3method("plot", class = "acf")(acfx, main = args$main, xlab = args$xlab, ylab = args$ylab, xlim = args$xlim, ylim = args$ylim)
 ...
 if (which == "effects") {
  ...
  any_s <- any(sapply(names(xres), function(i) {
  !is.null(xres[[i]]$s.effects)
  }))
  if (!any_s) {
  ...
  any_s <- any(sapply(names(x$results), function(i) {
  !is.null(x$results[[i]]$s.effects)
  }))
  if (!any_s) {
  plot.bamlss(x, which = c("hist-resid", "qq-resid"), ...)
  }
...
plot.bamlss.results <- function(x, model = NULL, term = NULL, ask = dev.interactive(), scale = 1, spar = TRUE, ...) {
  args <- list(...)
  cx <- class(x)
  if (spar) {
 op <- par(no.readonly = TRUE)
 ...
 if (any(which %in% c("scatter-resid", "scale-resid"))) {
   fit0 <- fitted.bamlss(x, type = "parameter", samples = TRUE, model = if (ny < 2) 
  1
   else NULL, nsamps = args$nsamps)
 }
 rtype <- args$type
 if (is.null(rtype)) 
   rtype <- "quantile"
 ...
 for (j in 1:ny) {
  ...
  if (is.null(args$xlab)) 
    args2$xlab <- paste(if (rtype == "quantile") {
    "Quantile"
    }
  ...
  if (w == "qq-resid") {
  args2$y <- if (rtype == "quantile") 
    (res)
  else (res - mean(res))/sd(res)
  ...
  if (is.null(args$xlab)) 
    args2$ylab <- paste(if (rtype == "quantile") {
    "Quantile"
    }
    else "Ordinary", "residuals")
  if (is.null(args$xlab)) {
 ...
 get_k_n <- function(x) {
  ...
  ne[[i]] <- if (!is.null(names(x[[i]]$s.effects))) 
    names(x[[i]]$s.effects)
  else NA
  if (is.null(term)) 
    ...
    tterm <- NULL
    for (j in term) tterm <- c(tterm, grep(j, ne[[i]], fixed = TRUE))
    pterms[[i]] <- if (length(tterm)) 
   tterm
    else NA
    }
 ...
 kn <- get_k_n(x)
 if (kn[1] < 1) 
   on.exit(warning("no terms to plot!"), add = TRUE)
 if (spar & (kn[1] > 0)) {
   if (!ask) {
  if ("cbamlss" %in% cx) {
...
.plot.bamlss.results <- function(x, model = NULL, term = NULL, ask = FALSE, scale = 1, spar = TRUE, ...) {
  n <- length(x)
  args <- list(...)
  k <- 0
  ylim <- NULL
  ylim <- args$ylim
  args$residuals <- if (is.null(args$residuals)) 
 FALSE
  else args$residuals
...
plot.bamlss.effect.default <- function(x, ...) {
 ...
 if (inherits(x, "random.effect") | inherits(x, "re.smooth.spec") | inherits(x, "mrf.smooth.spec") | inherits(x, "mrf.smooth") | is.factor(x[[1]])) {
   if (if (!is.null(args$density)) 
  args$density
   else FALSE) {
  args$density <- NULL
  if (is.null(args$main)) 
  ...
  args$x <- data.frame(x = as.numeric(x[, grepl("50%", colnames(x), fixed = TRUE)]), ID = as.character(x[, 1]), stringsAsFactors = FALSE)
  idvar <- NULL
  ...
  args$id <- if (!is.null(idvar)) 
    idvar
  else as.character(x[, 1])
  args$xlim <- args$ylim <- NULL
  do.call("plotmap", delete.args("plotmap", args, not = c("border", "lwd", "lty", names(formals("colorlegend")), "main", "names", "names_id", "shift")))
  }
  else {
  if (is.null(args$ylab)) 
    args$ylab <- attr(x, "specs")$label
  args$xlab <- attr(x, "specs")$term
  do.call("plotblock", delete.args("plotblock", args, c("xlim", "ylim", "pch", "main", "xlab", "ylab", "lwd", "axes", "add", "scheme")))
  }
   }
 }
 else {
   do.call("plot2d", delete.args("plot2d", args, c("xlim", "ylim", "pch", "main", "xlab", "ylab", "lwd", "axes", "add")))
 }
  }
  else {
 ...
 if (!is.null(args$slice)) {
   do.call("sliceplot", delete.args("sliceplot", args, c("xlim", "ylim", "zlim", "main", "xlab", "ylab", "col", "lwd", "lty")))
 }
 else {
  ...
  do.call("plot3d", delete.args("plot3d", args, c("xlim", "ylim", "zlim", "pch", "main", "xlab", "ylab", "ticktype", "zlab", "phi", "theta", "r", "d", "scale", "range", "lrange", "pos", "image.map", "symmetric", "border", "lwd")))
  }
   }
 }
  }
...
bamlss_factor2d_plot <- function(x, ids = NULL, add = FALSE, rug = FALSE, ...) {
  ...
  specs <- attr(x, "specs")
  if (is.null(specs)) {
 specs <- list(term = colnames(x)[1:2], label = paste("f(", colnames(x)[1], ",", colnames(x)[2], ")", sep = ""))
  }
 ...
 fx <- unlist(x[, grepl("50", colnames(x), fixed = TRUE)])
  ...
  xlab <- if (is.null(args$xlab)) 
 colnames(xd)[!isf]
  else args$xlab
  ylab <- if (is.null(args$ylab)) 
 specs$label
  else args$ylab
  id <- xd[, isf]
  xd <- xd[, !isf]
 ...
 range(fx)
  ...
  if (!add) {
 plot(1, 1, type = "n", xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, main = args$main)
  }
  col <- if (is.null(args$col)) 
 rainbow_hcl(nlevels(id))
  else args$col
  if (is.function(col)) 
 col <- col(nlevels(id))
  lwd <- if (is.null(args$lwd)) 
 1
  else args$lwd
  lty <- if (is.null(args$lty)) 
 1
  else args$lty
...
summary.bamlss <- function(object, model = NULL, FUN = NULL, parameters = TRUE, ...) {
 ...
 if (!inherits(sfun, "try-error")) 
  ...
  if (is.null(FUN)) {
 FUN <- function(x) {
   c(Mean = mean(x, na.rm = TRUE), quantile(x, probs = c(0.025, 0.5, 0.975)))
 }
  }
  rval$model.matrix <- .coef.bamlss(object, model = model, FUN = FUN, sterms = FALSE, full.names = FALSE, list = TRUE, parameters = parameters, summary = TRUE, mm = TRUE, ...)
 ...
 x
  ...
  rval$smooth.construct <- .coef.bamlss(object, model = model, FUN = FUN, sterms = TRUE, full.names = FALSE, list = TRUE, parameters = parameters, hyper.parameters = TRUE, summary = TRUE, ...)
...
print.summary.bamlss <- function(x, digits = max(3, getOption("digits") - 3), ...) {
 ...
 if (!is.null(x$model.stats$sampler)) {
  ...
  cat(if (k > 1) 
    " "
  else "", j, " = ", if (is.numeric(x$model.stats$sampler[[j]])) {
 ...
 if (!is.null(x$model.stats$optimizer)) {
  ...
  if (length(x$model.stats$optimizer[[j]]) < 2) {
    ...
    cat(if (k > 1) 
   " "
    else "", j, " = ", round(x$model.stats$optimizer[[j]], digits), sep = "")
    k <- k + 1
    if (k == 4) {
   k <- 1
   cat("\n")
...
DIC.bamlss <- function(object, ..., samples = TRUE, nsamps = NULL, newdata = NULL) {
 ...
 for (i in 1:length(object)) {
   ...
   if (n < 2) 
  xs <- list(xs)
   rval <- rbind(rval, data.frame(DIC = if (is.null(xs[[n]]$DIC)) 
  NA
   else xs[[n]]$DIC, pd = if (is.null(xs[[n]]$DIC)) 
  NA
   else xs[[n]]$pd))
 }
  }
 ...
 for (i in 1:length(object)) {
  ...
  newdata[, response.name(object[[i]])]
   ...
   mpar <- predict.bamlss(object[[i]], newdata = newdata, type = "parameter", FUN = mean, nsamps = nsamps, drop = FALSE)
   mdev <- -2 * sum(family$d(y, mpar, log = TRUE), na.rm = TRUE)
   object[[i]]$samples <- samps
   rm("samps")
   par <- predict.bamlss(object[[i]], newdata = newdata, type = "parameter", FUN = function(x) {
  x
   }, nsamps = nsamps, drop = FALSE)
   iter <- if (is.list(par)) 
  ncol(par[[1]])
   else ncol(par)
   dev <- rep(NA, iter)
   tpar <- mpar
   for (i in 1:iter) {
  for (j in seq_along(tpar)) tpar[[j]] <- par[[j]][, i]
  dev[i] <- -2 * sum(family$d(y, tpar, log = TRUE), na.rm = TRUE)
   }
   pd <- mean(dev, na.rm = TRUE) - mdev
   DIC <- mdev + 2 * pd
   rval <- rbind(rval, data.frame(DIC = DIC, pd = pd))
 }
  }
  Call <- match.call()
  row.names(rval) <- if (nrow(rval) > 1) 
 as.character(Call[-1])
  else ""
  rval
}
logLik.bamlss <- function(object, ..., optimizer = FALSE, samples = FALSE) {
 ...
 if (samples) {
  ...
  ll[[j]] <- mcmc(samplestats(object[[j]], logLik = TRUE), start = start(object[[j]]$samples), end = end(object[[j]]$samples), thin = thin(object[[j]]$samples))
  samps <- process.chains(object[[j]]$samples, combine = TRUE, drop = TRUE)
  sn <- sapply(strsplit(colnames(samps), ".", fixed = TRUE), function(x) {
  x[length(x)]
  })
  if (any(i <- (sn == "edf"))) {
  edf <- mcmc(apply(samps[, i, drop = FALSE], 1, sum, na.rm = TRUE), start = start(object[[j]]$samples), end = end(object[[j]]$samples), thin = thin(object[[j]]$samples))
  ll[[j]] <- mcmc(cbind(logLik = ll[[j]], df = edf), start = start(object[[j]]$samples), end = end(object[[j]]$samples), thin = thin(object[[j]]$samples))
  }
   }
 }
 else {
...
formula.bamlss.terms <- function(x, model, ...) {
 ...
 if (!inherits(x[[i]], "terms")) {
  ...
  f[[i]][[j]]$fake.formula <- as.formula(paste(response, "~1", if (length(vars)) 
  "+"
  else NULL, paste(vars, collapse = "+")), env = environment(x[[i]][[j]]))
  f[[i]][[j]]$terms <- x[[i]][[j]]
   }
 }
 else {
  ...
  vars <- vars[-1]
   ...
   f[[i]]$fake.formula <- as.formula(paste(response, "~1", if (length(vars)) 
  "+"
   else NULL, paste(vars, collapse = "+")), env = environment(x[[i]]))
   f[[i]]$terms <- x[[i]]
 }
  }
...
print.bamlss.formula <- function(x, ...) {
 ...
 for (i in seq_along(x)) {
  ...
  if (is.character(x[[i]]$formula)) 
    cat(x[[i]]$formula, "\n")
  else print(x[[i]]$formula, showEnv = FALSE)
  }
  else print(x[[i]])
   }
   if (i < length(x)) 
...
drop.terms.bamlss <- function(f, pterms = TRUE, sterms = TRUE, specials = NULL, keep.response = TRUE, keep.intercept = TRUE, data = NULL) {
  specials <- unique(c(specials, "s", "te", "t2", "sx", "s2", "rs", "ti", "tx", "tx2", "tx3", "tx4", "la", "n", "h", "lf", "af", "lf.vd", "re", "peer", "fpc", "lin", "rb", "tree"))
...
terms.bamlss <- terms.bamlss.frame <- terms.bamlss.formula <- function(x, specials = NULL, data = NULL, model = NULL, pterms = TRUE, sterms = TRUE, drop = TRUE, ...) {
  if (inherits(x, "bamlss.frame")) 
 x <- formula(x)
  if (!inherits(x, "bamlss.formula")) 
 x <- bamlss.formula(x, ...)
 ...
 if (!any(names(x[[nx[i]]]) %in% elmts) & !inherits(x[[nx[i]]], "formula")) {
  ...
  rval[[nx[i]]][[nx2[j]]] <- drop.terms.bamlss(x[[nx[i]]][[nx2[j]]], pterms = pterms, sterms = sterms, specials = specials, data = data)
   }
 }
 else {
   rval[[nx[i]]] <- drop.terms.bamlss(x[[nx[i]]], pterms = pterms, sterms = sterms, specials = specials, data = data)
 }
  }
  if (drop & (length(rval) < 2)) {
 rval <- rval[[1]]
...
get_pterms_labels <- function(x, specials = NULL) {
 ...
 x <- drop.terms.bamlss(x, pterms = TRUE, sterms = FALSE, keep.response = FALSE, specials = specials)
...
get_sterms_labels <- function(x, specials = NULL) {
 ...
 x <- drop.terms.bamlss(x, pterms = FALSE, sterms = TRUE, keep.response = FALSE, specials = specials)
 tl <- all_labels_formula(x)
  }
  else tl <- character(0)
  tl
}
results.bamlss.default <- function(x, what = c("samples", "parameters"), grid = -1, nsamps = NULL, burnin = NULL, thin = NULL, ...) {
 ...
 if (has_pterms(obj$terms)) {
  ...
  nas <- apply(psamples, 1, function(x) {
  any(is.na(x))
  })
 ...
 if (has_sterms(obj$terms)) {
  ...
  if (!is.null(obj$smooth.construct[[j]]$Xf) & FALSE) {
    ...
    if (kx) {
    pn <- paste(paste(id, ":h1:linear.", paste(paste(obj$smooth.construct[[j]]$term, collapse = "."), "Xf", sep = "."), sep = ""), 1:kx, sep = ".")
  ...
  if (is.null(obj$smooth.construct[[j]][["X"]])) {
    ...
    b <- paste(id, "s", j, if (is.null(colnames(obj$smooth.construct[[j]]$X))) {
  ...
  s.effects[[obj$smooth.construct[[j]]$label]] <- compute_s.effect(obj$smooth.construct[[j]], get.X = get.X, fit.fun = obj$smooth.construct[[j]]$fit.fun, psamples = psamples[, b, drop = FALSE], FUN = FUN, snames = snames, data = if (!is.null(obj$smooth.construct[[j]]$model.frame)) {
 ...
 rval <- list(model = list(formula = obj$formula, DIC = DIC, pd = pd, N = nrow(mf)), p.effects = p.effects, s.effects = s.effects)
...
fitted.bamlss <- function(object, model = NULL, term = NULL, type = c("link", "parameter"), samples = TRUE, FUN = c95, nsamps = NULL, ...) {
  type <- match.arg(type)
 ...
 return(predict.bamlss(object, model = model, term = term, type = type, FUN = FUN, nsamps = nsamps, ...))
  }
}
grep2 <- function(pattern, x, ...) {
  i <- NULL
  for (p in pattern) i <- c(i, grep(p, x, ...))
  sort(unique(i))
}
samples <- function(object, ...) {
  UseMethod("samples")
}
samples.bamlss <- samples.bamlss.frame <- function(object, model = NULL, term = NULL, combine = TRUE, drop = TRUE, burnin = NULL, thin = NULL, coef.only = FALSE, ...) {
 ...
 stop("no samples to extract!")
  ...
  if (coef.only) {
 cdrop <- c(".accepted", ".alpha", "logLik", "logPost", "AIC", "BIC", "DIC", "pd", ".edf")
 for (d in cdrop) {
 ...
 for (i in seq_along(tx)) {
  ...
  next
   ...
   for (ii in jj) {
  for (k in seq_along(x)) rval[[k]] <- cbind(rval[[k]], x[[k]][, ii, drop = FALSE])
   }
 }
 for (k in seq_along(x)) rval[[k]] <- as.mcmc(rval[[k]], start = start(x[[k]]), end = end(x[[k]]))
 x <- as.mcmc.list(rval)
  }
  if (!is.null(thin)) {
 iterthin <- as.integer(seq(1, nrow(x[[1]]), by = thin))
 for (i in seq_along(x)) {
   x[[i]] <- mcmc(x[[i]][iterthin, , drop = FALSE], start = if (!is.null(burnin)) 
  burnin
   else 1, thin = thin)
 }
  }
...
continue <- function(object, cores = NULL, combine = TRUE, sleep = NULL, results = TRUE, ...) {
  if (is.null(object$samples)) 
 stop("no samples to continue from!")
  ...
  results <- if (results) 
 attr(object, "functions")$results
  else FALSE
  if (is.null(cores)) {
 samples <- sampler(x = object$x, y = object$y, family = object$family, weights = model.weights(object$model.frame), offset = model.offset(object$model.frame), start = start, hessian = object$hessian, ...)
  }
  else {
 parallel_fun <- function(j) {
   if (j > 1 & !is.null(sleep)) 
  Sys.sleep(sleep)
   sampler(x = object$x, y = object$y, family = object$family, weights = model.weights(object$model.frame), offset = model.offset(object$model.frame), start = start, hessian = object$hessian, ...)
 }
...
confint.bamlss <- function(object, parm, level = 0.95, model = NULL, pterms = TRUE, sterms = FALSE, full.names = FALSE, hyper.parameters = FALSE, ...) {
  args <- list(...)
 ...
 quantile(x, probs = probs, na.rm = TRUE)
  ...
  return(.coef.bamlss(object, model = model, term = parm, FUN = FUN, parameters = FALSE, pterms = pterms, sterms = sterms, full.names = full.names, hyper.parameters = hyper.parameters, ...))
}
coef.bamlss <- function(object, model = NULL, term = NULL, FUN = NULL, parameters = NULL, pterms = TRUE, sterms = TRUE, hyper.parameters = TRUE, list = FALSE, full.names = TRUE, rescale = FALSE, ...) {
  .coef.bamlss(object, model = model, term = term, FUN = FUN, parameters = parameters, pterms = pterms, sterms = sterms, s.variances = TRUE, hyper.parameters = hyper.parameters, summary = FALSE, list = list, full.names = full.names, rescale = rescale, ...)
}
.coef.bamlss <- function(object, model = NULL, term = NULL, FUN = NULL, parameters = NULL, pterms = TRUE, sterms = TRUE, s.variances = FALSE, hyper.parameters = FALSE, summary = FALSE, list = FALSE, full.names = TRUE, rescale = FALSE, ...) {
  if (is.null(object$samples) & is.null(object$parameters)) 
 stop("no coefficients to extract!")
  if (is.null(parameters)) 
 parameters <- is.null(object$samples)
  if (hyper.parameters) {
 pterms <- if (s.variances) 
   TRUE
 else FALSE
 if (summary) {
 ...
 drop <- c(".tau2", ".lambda", ".edf", ".accepted", if (!summary) ".alpha" else NULL, "logLik", "logPost", "AIC", "BIC", "DIC", "pd")
 if (is.null(FUN)) 
   FUN <- function(x) {
  mean(x, na.rm = TRUE)
   }
 ...
 if (length(rval$samples)) {
   rval$samples <- apply(rval$samples, 2, function(x, ...) {
  FUN(na.omit(x), ...)
   })
 ...
 if (all(rd < 1)) {
  ...
  if (!length(rval)) 
 return(numeric(0))
  nx <- sapply(strsplit(rownames(rval), ".", fixed = TRUE), function(x) {
 x[1]
  })
  if (list) {
 rval2 <- list()
 ...
 if (!full.names) {
  ...
  for (i in nx) rownames(rval) <- gsub(paste(i, ".", sep = ""), "", rownames(rval), fixed = TRUE)
   }
 }
...
term.labels <- function(x, model = NULL, pterms = TRUE, sterms = TRUE, intercept = TRUE, list = TRUE, ...) {
 ...
 if (intercept & (attr(x[[j]], "intercept") > 0)) {
   rval[[j]]$p <- if (!length(tl)) 
  "(Intercept)"
   else c("(Intercept)", rval[[j]]$p)
 }
  }
  names(rval) <- nx
  if (!list) 
...
term.labels2 <- function(x, model = NULL, pterms = TRUE, sterms = TRUE, intercept = TRUE, list = TRUE, type = 1, rm.by = TRUE, ...) {
  stl <- NULL
  is.bamlss <- FALSE
  if (inherits(x, "bamlss") | inherits(x, "bamlss.frame")) {
...
score <- function(x, limits = NULL, FUN = function(x) {
  mean(x, na.rm = TRUE)
 ...
 nscore <- function(eta) {
  ...
  rval <- try(integrate(integrand, lower = min(y, na.rm = TRUE), upper = max(y, na.rm = TRUE)))
   }
   rval <- if (inherits(rval, "try-error")) 
  NA
   else rval$value
   rval
 }
 ...
 return(data.frame(log = FUN(loglik), quadratic = FUN(quadratic), spherical = FUN(spherical)))
  }
  if (type == "mean") {
 eta <- if (kfitted) {
   kfitted(x, nsamps = nsamps, FUN = function(x) {
  mean(x, na.rm = TRUE)
   }, ...)
 }
 else fitted(x, samples = if (!is.null(h_response(x))) 
   TRUE
 else FALSE)
 ...
 nx <- names(x)
 eta <- if (kfitted) {
   kfitted(x, FUN = function(x) {
  x
   }, nsamps = nsamps, ...)
 }
 else fitted(x, samples = TRUE, FUN = function(x) {
   x
 }, nsamps = nsamps)
 if (!inherits(eta, "list")) {
   eta <- list(eta)
 ...
 for (j in nx) {
   colnames(eta[[j]]) <- paste("i", formatC(1:ncol(eta[[j]]), width = nchar(ncol(eta[[1]])), flag = "0"), sep = ".")
 }
 nc <- ncol(eta[[1]])
 eta <- as.data.frame(eta)
 res <- list()
 for (i in 1:nc) {
   eta2 <- eta[, grep(ni <- paste(".i.", formatC(i, width = nchar(nc), flag = "0"), sep = ""), names(eta)), drop = FALSE]
...
kfitted <- function(x, k = 5, weighted = FALSE, random = FALSE, engine = NULL, verbose = TRUE, FUN = mean, nsamps = NULL, ...) {
  if (!inherits(x, "bamlss")) 
 stop("argument x is not a \"bamlss\" object!")
  if (is.null(engine)) 
 engine <- attr(x, "engine")
  if (is.null(engine)) 
 stop("please choose an engine!")
  mf <- model.frame(x)
 ...
 if (!weighted) {
   take <- mf[i != j, ]
   bcv <- bamlss(f, data = take, family = family, engine = engine, verbose = verbose, ...)
 }
 else {
   w <- 1 * (i != j)
   bcv <- bamlss(f, data = mf, family = family, engine = engine, verbose = verbose, weights = w, ...)
 }
 ...
 if (is.null(rval)) {
   rval <- list()
   for (ii in names(fit)) {
  rval[[ii]] <- matrix(NA, nrow = nrow(mf), ncol = if (is.null(dim(fit[[ii]]))) 
  1
  else ncol(fit[[ii]]))
   }
 }
...
create.dp <- function(family) {
 ...
 names(family$links) <- family$names
  links <- list()
  for (j in names(family$links)) links[[j]] <- make.link2(family$links[j])$linkinv
  d <- function(y, eta, ...) {
 for (j in names(eta)) eta[[j]] <- links[[j]](eta[[j]])
 family$d(y, eta, ...)
  }
  p <- function(y, eta, ...) {
 for (j in names(eta)) eta[[j]] <- links[[j]](eta[[j]])
...
residuals.bamlss <- function(object, type = c("quantile", "response"), nsamps = NULL, ...) {
 ...
 if (type == "quantile") {
   if (is.null(family$p)) {
  type <- "response"
  warning(paste("no $p() function in family '", family$family, "', cannot compute quantile residuals, computing response resdiuals instead!", sep = ""))
   }
   else {
...
plot.bamlss.residuals <- function(x, which = c("hist-resid", "qq-resid", "wp"), spar = TRUE, ...) {
 ...
 for (w in which) {
  ...
  args$xlab <- if (is.null(type)) 
    "Residuals"
  else paste(type, "residuals")
  if (is.null(args$ylab)) 
  args$ylab <- "Density"
  if (is.null(args$main)) 
  args$main <- paste("Histogram and density", if (!is.null(cn[j])) 
    paste(":", cn[j])
  else NULL)
  ...
  ylim <- range(c(as.numeric(mean$y), as.numeric(lower$y), as.numeric(upper$y)), na.rm = TRUE)
  args$plot.it <- TRUE
  if (is.null(args$ylim)) 
    args$ylim <- ylim
  args$y <- x2[, "Mean"]
  ...
  if (add) {
    args <- delete.args("points.default", list(...), package = "graphics", not = c("col", "pch", "cex"))
...
c.bamlss.residuals <- function(...) {
  ...
  Call <- match.call()
  mn <- as.character(Call)[-1]
  names(res) <- if (is.null(names(res))) 
 mn
  else names(res)
...
h_response <- function(x) {
 ...
 for (j in seq_along(x)) rval <- c(rval, h_response(x[[j]]))
  }
  else {
...
XinY <- function(x, y, by = intersect(names(x), names(y)), by.x = by, by.y = by, notin = FALSE, incomparables = NULL, ...) {
...
XnotinY <- function(x, y, by = intersect(names(x), names(y)), by.x = by, by.y = by, notin = TRUE, incomparables = NULL, ...) {
...
stg <- function(x, interp = FALSE, k = -1, ...) {
 ...
 x <- zoo::na.approx(x, rule = 2)
  ...
  xc <- stats::cycle(x)
  mf <- na.omit(data.frame(x = as.numeric(x), trend = 1:NROW(x), season = as.integer(xc), xlag = c(NA, as.numeric(x)[-length(x)])))
 ...
 k[2] <- floor(length(unique(mf$season)) * 0.9)
  if (k[3] < 0) 
 k[3] <- 10
  b <- gam(x ~ s(trend, by = xlag, k = k[1]) + ti(trend, k = k[1], bs = "cr") + ti(season, k = k[2], bs = "cc") + ti(trend, season, bs = c("cr", "cc"), k = k[3]), data = mf, method = "REML")
  plot(b, pages = 1)
  p <- predict(b, type = "terms")
  rval <- cbind(raw = mf[["x"]], fitted = fitted(b), trend = p[, "ti(trend)"] + coef(b)[1], seasonal = p[, "ti(season)"] + p[, "ti(trend,season)"], lag1 = p[, "s(trend):xlag"]/mf$xlag, remainder = stats::residuals(b))
...
smooth.construct.ispline.smooth.spec <- function(object, data, knots, ...) {
  ...
  knots <- object$knots
  knots <- knots[(knots > min(data[[object$term]])) & (knots < max(data[[object$term]]))]
  object$knots <- knots
  object$X <- splines2::iSpline(data[[object$term]], knots = knots, degree = 3, intercept = FALSE, derivs = 0)
  object$S <- list(crossprod(diff(diag(ncol(object$X)), differences = 2)))
  object$derivMat <- splines2::iSpline(data[[object$term]], knots = knots, degree = 3, intercept = FALSE, derivs = 1)
...
smooth.construct.mlt.smooth.spec <- function(object, data, knots, ...) {
 ...
 for (j in 2:length(object$margin)) object$margin[[j]] <- smoothCon(object$margin[[j]], data, knots, absorb.cons = TRUE, scale.penalty = TRUE)[[1]]
  }
  object$X <- tensor.prod.model.matrix(lapply(object$margin, function(x) {
 x$X
  }))
  dX <- list(object$margin[[1]]$derivMat)
  if (length(object$margin) > 1) 
 dX <- c(dX, lapply(object$margin[2:length(object$margin)], function(x) {
   x$X
 }))
  object$dX <- tensor.prod.model.matrix(dX)
  object$S <- tensor.prod.penalties(sapply(object$margin, function(x) {
 x$S
  }))
...
Predict.matrix.ispline.smooth <- function(object, data) {
  stopifnot(requireNamespace("splines2"))
  X <- splines2::iSpline(data[[object$term]], knots = object$knots, degree = 3, intercept = FALSE, derivs = 0)
  attr(X, "deriv") <- splines2::iSpline(data[[object$term]], knots = object$knots, degree = 3, intercept = FALSE, derivs = 1)
  X
}
Predict.matrix.mlt.smooth <- function(object, data) {
  X <- lapply(object$margin, function(x) {
 PredictMat(x, as.data.frame(data))
  })
...
bboost <- function(..., data, type = 1, cores = 1, n = 2, prob = 0.623, fmstop = NULL, trace = TRUE, drop = FALSE, replace = FALSE) {
  if (is.null(fmstop)) {
 fmstop <- function(model, data) {
   y <- response.name(model)
   p <- predict(model, newdata = data, model = "mu")
   mse <- NULL
   for (i in 1:nrow(model$parameters)) mse <- c(mse, sqrt(mean((data[[y]] - p[, i])^2)))
 ...
 foo <- function(j) {
  ...
  cat("... starting bootstrap sample", j, "\n")
   ...
   b <- bamlss(..., data = d0, plot = FALSE, sampler = FALSE, optimizer = opt_boost, boost.light = FALSE, light = TRUE)
 ...
 foo <- function(j) {
  ...
  i <- sample(ind, size = ceiling(nobs * prob), replace = FALSE)
   ...
   d0 <- data[i, , drop = FALSE]
   b <- bamlss(..., data = d0, plot = FALSE, boost.light = TRUE, light = TRUE, sampler = FALSE, optimizer = opt_boost)
   attr(b, "mstop") <- list(logLik = b$model.stats$optimizer$boost_summary$ic, mstop = nrow(b$parameters))
...
bamlss.sl <- function(object, data, ...) {
  ...
  eta <- beta <- list()
  for (j in fam$names) {
 eta[[j]] <- do.call("cbind", lapply(p, function(x) {
   x[, grep(paste0(".", j), colnames(x))]
 }))
 ...
 if (any(beta1 > 0)) 
   beta1 <- beta1/sum(beta1)
 else warning("All models have zero weight!", call. = FALSE)
 beta[paste0(j, 1:k)] <- beta1
 fit[[j]] <- drop(eta[[j]] %*% beta1)
  }
  rval <- list(coefficients = beta, fitted.values = fit, converged = opt$convergence == 0, logLik = -1 * opt$value)
  class(rval) <- "bamlss.sl"
  rval
}
predict.bamlss.sl <- function(object, models, newdata, type = c("link", "parameter"), ...) {
  type <- match.arg(type)
  fam <- models[[1]]$family
  ...
  fit <- list()
  for (j in fam$names) {
 eta <- do.call("cbind", lapply(p, function(x) {
   x[, grep(paste0(".", j), colnames(x))]
 }))
...
smooth.construct.ms.smooth.spec <- function(object, data, knots, ...) {
 ...
 for (j in seq_along(x$S)) S <- S + 1/tau2[j] * x$S0[[j]]
 P <- matrix_inv(XWX + S, index = x$sparse.setup)
 g <- nu * drop(P %*% crossprod(x$X, x$rres))
...
compute_WAIC <- function(object, newdata = NULL) {
 ...
 return(NULL)
  ...
  fam <- family(object)
  par <- predict.bamlss(object, newdata = newdata, type = "parameter", FUN = function(x) {
 return(x)
  }, drop = FALSE)
  y <- if (is.null(newdata)) {
 ...
 if (ncol(y) < 2) 
  ...
  nsamps <- ncol(par[[1]])
  d <- matrix(NA, nrow(par[[1]]), nsamps)
  for (i in 1:nsamps) {
 tpar <- list()
 for (j in names(par)) tpar[[j]] <- par[[j]][, i]
 d[, i] <- fam$d(y, tpar)
  }
  ...
  waic1 <- -2 * (ldm - p1)
  waic2 <- -2 * (ldm - p2)
  rval <- data.frame(WAIC1 = waic1, WAIC2 = waic2, p1 = p1, p2 = p2)
  return(rval)
}
...
.engines <- function(family) {
  ...
  tab <- c(family = family$family, `opt_bfit()` = !optimizer, `opt_boost()` = !optimizer, `opt_bbfit()` = !optimizer, `sam_GMCMC()` = !sampler, `sam_BayesX()` = bayesx, `sam_JAGS()` = jags, `special_opt()` = optimizer, `special_sam()` = sampler)
  return(tab)
}
engines <- function(family, ...) {
  if (missing(family)) {
...
.CRPS <- function(y, par, family, interval = c(-Inf, Inf)) {
 ...
 int1 <- integrate(foo1, lower = interval[1], upper = y[i])$value
 int2 <- integrate(foo2, lower = y[i], upper = interval[2])$value
 crps[i] <- int1 + int2
  }
  return(crps)
}
