sam_GMCMC <- GMCMC <- function(x, y, family, start = NULL, weights = NULL, offset = NULL, n.iter = 1200, burnin = 200, thin = 1, verbose = TRUE, step = 20, propose = "iwlsC_gp", chains = NULL, ...) {
  nx <- family$names
 ...
 for (j in names(x[[i]])) {
  ...
  propose2[[i]][[j]] <- if (is.null(x[[i]][[j]]$propose)) 
  propose
  else x[[i]][[j]]$propose
   }
   else {
 ...
 offset <- as.data.frame(offset)
  samps <- gmcmc(fun = family, theta = theta, fitfun = fitfun, data = x, propose = propose2, logLik = logLik, n.iter = n.iter, burnin = burnin, thin = thin, y = y, simplify = FALSE, zworking = zworking, resids = resids, cores = 1, chains = chains, combine = FALSE, weights = weights, offset = offset, verbose = verbose, step = step, ...)
  samps
}
gmcmc <- function(fun, theta, priors = NULL, propose = NULL, fitfun = NULL, logLik = NULL, data = NULL, attr.copy = "hess", n.iter = 12000, burnin = 2000, thin = 10, verbose = TRUE, step = 20, simplify = TRUE, chains = NULL, cores = NULL, combine = TRUE, sleep = 1, ...) {
  if (!is.list(theta)) {
 theta <- list(theta)
 ...
 for (i in names(theta)) {
  ...
  for (j in names(input[[i]])) {
    ...
    stop(paste("the '", ninput, "' object for block [", i, "][", j, "] is not a ", type, "!", sep = ""), call. = FALSE)
    }
  }
  }
   }
  ...
  if (!inherits(input[[i]], type)) {
    stop(paste("the '", ninput, "' object for block [", i, "] is not a ", type, "!", sep = ""), call. = FALSE)
  }
  }
   }
 }
 ...
 eta[[i]] <- 0
 if (is.list(theta[[i]])) {
   for (j in names(theta[[i]])) eta[[i]] <- eta[[i]] + fitfun[[i]][[j]](data[[i]][[j]], theta[[i]][[j]])
 }
 else {
 ...
 if (is.list(theta[[i]])) {
  ...
  p0 <- propose[[i]][[j]](fun, theta, id = c(i, j), prior = priors[[i]][[j]], data = data[[i]][[j]], eta = eta, iteration = 1, n.iter = n.iter, burnin = burnin, rho = rho, ...)
  if (!is.list(p0)) {
  stop(paste("the propose() function for block [", i, "][", j, "] must return a named list()!", sep = ""))
  }
  if (is.null(p0$alpha)) {
  stop(paste("the propose() function for block [", i, "][", j, "] must return the acceptance probability 'alpha'!", sep = ""))
  }
  if (is.null(p0$parameters)) {
  stop(paste("the propose() function for block [", i, "][", j, "] must return a vector 'parameters'!", sep = ""))
  }
  if (length(p0$parameters) != length(theta[[i]][[j]])) {
  stop(paste("the propose() function for block [", i, "][", j, "] must return a vector 'parameters' with the same length of initial parameters in theta!", sep = ""))
  }
  if (is.null(names(p0$parameters))) 
  names(p0$parameters) <- paste("[", 1:length(p0$parameters), "]", sep = "")
  if (length(p0$parameters) < 2 & FALSE) 
  names(p0$parameters) <- NULL
  theta.save[[i]][[j]] <- list(samples = matrix(NA, nrow = length(iterthin), ncol = length(p0$parameters)), alpha = rep(NA, length = length(iterthin)), accepted = rep(NA, length = length(iterthin)))
  ...
  for (a in attr.copy) attr(theta[[i]][[j]], a) <- attr(p0$parameters, a)
  }
   }
 }
 else {
   p0 <- propose[[i]](fun, theta, id = i, prior = priors[[i]], data = data[[i]], eta = eta, iteration = 1, n.iter = n.iter, burnin = burnin, rho = rho, ...)
   if (!is.list(p0)) {
  stop(paste("the propose() function for block [", i, "] must return a named list()!", sep = ""))
   }
   if (is.null(p0$alpha)) {
  stop(paste("the propose() function for block [", i, "] must return the acceptance probability 'alpha'!", sep = ""))
   }
   if (is.null(p0$parameters)) {
  stop(paste("the propose() function for block [", i, "] must return a vector 'parameters'!", sep = ""))
   }
   if (length(p0$parameters) != length(theta[[i]])) {
  stop(paste("the propose() function for block [", i, "] must return a vector 'parameters' with the same length of initial parameters in theta!", sep = ""))
   }
   if (is.null(names(p0$parameters))) 
  names(p0$parameters) <- paste("[", 1:length(p0$parameters), "]", sep = "")
   if (length(p0$parameters) < 2 & FALSE) 
  names(p0$parameters) <- NULL
   theta.save[[i]] <- list(samples = matrix(NA, nrow = length(iterthin), ncol = length(p0$parameters)), alpha = rep(NA, length = length(iterthin)), accepted = rep(NA, length = length(iterthin)))
  ...
  colnames(theta.save[[i]]$extra) <- names(p0$extra)
   ...
   if (!is.null(attr.copy)) {
  for (a in attr.copy) attr(theta[[i]], a) <- attr(p0$parameters, a)
   }
 }
  }
 ...
 for (iter in 1:n.iter) {
  ...
  if (is.list(theta[[i]])) {
  for (j in names(theta[[i]])) {
    state <- try(propose[[i]][[j]](fun, theta, id = c(i, j), prior = priors[[i]][[j]], data = data[[i]][[j]], eta = eta, iteration = iter, n.iter = n.iter, burnin = burnin, rho = rho, ...), silent = TRUE)
    if (inherits(state, "try-error")) {
    state <- list(alpha = NA)
    }
    accepted <- if (is.na(state$alpha)) 
    FALSE
    else log(runif(1)) <= state$alpha
    if (accepted) {
   ...
   logLik(eta)
    ...
    if (plot) {
   if (js > 1) {
   plot(ll[1:js], type = "l", xlab = "Iterations", ylab = "logLik")
   }
    }
    }
  }
  ...
  state <- try(propose[[i]](fun, theta, id = i, prior = priors[[i]], data = data[[i]], eta = eta, iteration = iter, n.iter = n.iter, burnin = burnin, rho = rho, ...), silent = TRUE)
  if (inherits(state, "try-error")) {
    state <- list(alpha = NA)
  }
  accepted <- if (is.na(state$alpha)) 
 ...
 for (i in ntheta) {
  ...
  lapply(theta.save[[i]], function(x) {
  do.call("cbind", x)
  })
   }
  ...
  for (k in seq_along(cn2)) {
    ...
    paste(if (strsplit(cn2[k], "[", fixed = TRUE)[[1]][1] != "") 
   "."
    else NULL, cn2[k], sep = "")
    }
    else paste(if (cn2[k] != "") 
  ...
  sep <- c(".", "")[as.integer(grepl("[", cn, fixed = TRUE)) + 1]
  for (j in seq_along(cn)) {
  cn[j] <- paste(if (length(theta) > 1) 
    i
  else NULL, if (length(theta[[i]]) > 1) 
    cn[j]
  else NULL, sep = if (length(theta) > 1) 
    sep[j]
  else "")
  }
  ...
  if (is.list(theta[[i]])) 
  do.call("cbind", theta.save[[i]])
  else theta.save[[i]]
   }
 }
 theta.save <- if (length(theta.save) < 2) 
   theta.save[[1]]
 else do.call("cbind", theta.save)
 ...
 if (chains < 2) {
   ...
   samps <- list()
   for (j in 1:chains) samps[[j]] <- sampler(...)
 ...
 if (!inherits(rval, "mcmc.list") & inherits(rval[[1]], "mcmc.list")) 
...
barfun <- function(ptm, n.iter, i, step, nstep, start = TRUE) {
 ...
 p <- i/n.iter
 p <- paste("|", paste(rep("*", round(nstep * p)), collapse = ""), paste(rep(" ", round(nstep * (1 - p))), collapse = ""), "| ", formatC(round(p, 2) * 100, width = 3), "%", sep = "")
 elapsed <- c(proc.time() - ptm)[3]
 rt <- elapsed/i * (n.iter - i)
 rt <- if (rt > 60) {
   paste(formatC(format(round(rt/60, 2), nsmall = 2), width = 5), "min", sep = "")
...
"[.gmcmc.theta" <- function(x, i) {
  if (length(i) < 2) 
 return(x[[i]])
  else return(x[[i[1]]][[i[2]]])
}
"[<-.gmcmc.theta" <- function(x, i, value) {
  if (length(i) < 2) 
 x[[i]] <- value
  else x[[i[1]]][[i[2]]] <- value
  return(x)
}
...
gmcmc_unislice <- function(fun, theta, id, prior, j, ..., w = 1, m = 30, lower = -Inf, upper = +Inf) {
  args <- list(...)
  x0 <- theta[id][j]
  gL <- gR <- theta
  fun2 <- function(theta) {
...
gmcmc_slice <- function(fun, theta, id, prior, ...) {
  for (j in seq_along(theta[id])) theta[id][j] <- gmcmc_unislice(fun, theta, id, prior, j, ...)
...
GMCMC_iwlsC <- function(family, theta, id, eta, y, data, weights = NULL, offset = NULL, zworking, resids, rho, ...) {
  if (!is.null(offset)) {
 for (j in names(offset)) eta[[j]] <- eta[[j]] + offset[[j]]
  }
  W <- if (is.null(weights[[id[1]]])) 
 1
  else weights[[id[1]]]
  rval <- .Call("gmcmc_iwls", family, theta, id, eta, y, data, zworking, resids, id[1], W, rho, PACKAGE = "bamlss")
  if (!data$fixed & !data$fxsp & length(data$S)) {
 if (length(data$S) > 1) {
   i <- grep("tau2", names(rval$parameters))
   for (j in i) {
  rval$parameters <- uni.slice(rval$parameters, data, family, NULL, NULL, id[1], j, logPost = gmcmc_logPost, lower = 0, ll = rval$loglik)
   }
 }
  }
  return(list(parameters = rval$parameters, alpha = rval$alpha, extra = c(edf = rval$edf)))
}
GMCMC_iwlsC_gp <- function(family, theta, id, eta, y, data, weights = NULL, offset = NULL, zworking, resids, rho, ...) {
  if (!is.null(offset)) {
 for (j in names(offset)) eta[[j]] <- eta[[j]] + offset[[j]]
  }
  W <- if (is.null(weights[[id[1]]])) 
 1
  ...
  rval <- .Call("gmcmc_iwls_gp", family, theta, id, eta, y, data, zworking, resids, id[1], W, data$sparse.setup$block.index, data$sparse.setup$is.diagonal, rho)
  if (!data$fixed & !data$fxsp & length(data$S)) {
...
GMCMC_iwlsC_gp_diag_lasso <- function(family, theta, id, eta, y, data, weights = NULL, offset = NULL, zworking, resids, rho, ...) {
  if (!is.null(offset)) {
 for (j in names(offset)) eta[[j]] <- eta[[j]] + offset[[j]]
  }
  W <- if (is.null(weights[[id[1]]])) 
 1
  ...
  rval <- .Call("gmcmc_iwls_gp_diag_lasso", family, theta, id, eta, y, data, zworking, resids, id[1], W, rho, PACKAGE = "bamlss")
  if (!data$fixed & !data$fxsp & length(data$S)) {
 if ((length(data$S) < 2) & (attr(data$prior, "var_prior") == "ig")) {
   g <- get.par(rval$parameters, "b")
   if (is.function(data$S[[1]])) {
...
GMCMC_iwls <- function(family, theta, id, eta, y, data, weights = NULL, offset = NULL, ...) {
 ...
 if ((length(data$S) < 2) & (attr(data$prior, "var_prior") == "ig")) {
  ...
  data$rank <- ncol(K)
   ...
   for (j in i) {
  theta <- uni.slice(theta, data, family, NULL, NULL, id[1], j, logPost = gmcmc_logPost, lower = 0, ll = pibeta)
   }
 }
  }
  p1 <- data$prior(theta)
 ...
 else matrix_inv(XWX + if (!is.null(data$xt[["pS"]])) 
   data$xt[["pS"]]
 else 0, data$sparse.setup)
  }
  else {
 tau2 <- get.par(theta, "tau2")
 for (j in seq_along(data$S)) S <- S + 1/tau2[j] * if (is.function(data$S[[j]])) 
   data$S[[j]](c(theta, data$fixed.hyper))
 else data$S[[j]]
 matrix_inv(XWX + S + if (!is.null(data$xt[["pS"]])) 
   data$xt[["pS"]]
 else 0, data$sparse.setup)
  }
  P[P == Inf] <- 0
...
GMCMC_mvn <- function(family, theta, id, prior, eta, y, data, ...) {
 ...
 if (is.null(attr(theta, "hess"))) {
   ...
   tau2 <- if (!data$fixed) 
  get.par(data$state$parameters, "tau2")
   else NULL
   lp <- function(g) {
  ...
  family$loglik(y, family$map2par(eta)) + data$prior(c(g, tau2))
   ...
   gfun <- hfun <- NULL
   g.grad <- grad(fun = lp, theta = g, id = id[1], prior = NULL, args = list(gradient = gfun, x = data, y = y, eta = eta))
   g.hess <- hess(fun = lp, theta = g, id = id[1], prior = NULL, args = list(gradient = gfun, hessian = hfun, x = data, y = y, eta = eta))
 ...
 if (length(data$S) < 2) {
   ...
   i <- grep("tau2", names(theta))
   for (j in i) {
  theta <- uni.slice(theta, data, family, NULL, NULL, id[1], j, logPost = gmcmc_logPost, lower = 0, ll = ll1)
   }
 }
  }
...
GMCMC_newton <- function(family, theta, id, prior, eta, y, data, ...) {
 ...
 hfun[[id[1]]] <- function(g, y, eta, x, ...) {
  ...
  g.grad2 <- grad(fun = lp, theta = g2, id = id[1], prior = NULL, args = list(gradient = gfun, x = data, y = y, eta = eta))
  g.hess2 <- hess(fun = lp, theta = g2, id = id[1], prior = NULL, args = list(gradient = gfun, hessian = hfun, x = data, y = y, eta = eta))
...
GMCMC_slice <- function(family, theta, id, eta, y, data, ...) {
 ...
 i <- i[-grep("tau2", names(theta))]
  for (j in i) {
 theta <- uni.slice(theta, data, family, y, eta, id[1], j, logPost = gmcmc_logPost)
  }
 ...
 i <- grep("tau2", names(theta))
 for (j in i) {
   theta <- uni.slice(theta, data, family, NULL, NULL, id[1], j, logPost = gmcmc_logPost, lower = 0, ll = ll)
 }
  }
...
gmcmc_mvnorm2 <- function(fun, theta, id, prior, ...) {
 ...
 if (eps > 0.001) {
  ...
  return(-1 * (ll + lp))
   ...
   if (is.null(start)) 
  start <- theta[id]
   opt <- optim(start, fn = objfun, method = "L-BFGS-B", hessian = TRUE, control = list(maxit = 100))
   theta[id] <- opt$par
...
sam_MVNORM <- MVNORM <- function(x, y = NULL, family = NULL, start = NULL, n.samples = 500, hessian = NULL, ...) {
 ...
 if (inherits(x, "bamlss")) {
  ...
  hessian <- opt_optim(x = x$x, y = x$y, family = x$family, start = start, hessian = TRUE, ...)
   }
   else {
  hessian <- x$hessian
   }
 ...
 if (is.null(start)) 
   get.all.par(x, list = FALSE, drop = TRUE)
 else start
  }
  if (length(par) < 1) 
 par <- start
 ...
 for (i in c(1e-05, 1e-04, 0.001, 0.01)) {
   if (ncol(Sigma) != length(par)) {
...
uni.slice <- function(g, x, family, response, eta, id, j, ..., w = 1, m = 30, lower = -Inf, upper = +Inf, logPost) {
  x0 <- g[j]
  gL <- gR <- g
 ...
 repeat {
   if (gL[j] <= lower) 
  break
   if (logPost(gL, x, family, response, eta, id, ...) <= logy) 
  break
   gL[j] <- gL[j] - w
 }
 repeat {
   if (gR[j] >= upper) 
  break
   if (logPost(gR, x, family, response, eta, id, ...) <= logy) 
  break
   gR[j] <- gR[j] + w
 }
  }
...
uni.slice2 <- function(g, x, y, j, ..., w = 1, m = 30, lower = -Inf, upper = +Inf, logPost) {
  x0 <- g[j]
  gL <- gR <- g
  gx0 <- logPost(g, x, y, ...)
  logy <- gx0 - rexp(1)
