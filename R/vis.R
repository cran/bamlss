plot2d <- function(x, residuals = FALSE, rug = FALSE, jitter = TRUE, col.residuals = NULL, col.lines = NULL, col.polygons = NULL, col.rug = NULL, c.select = NULL, fill.select = NULL, data = NULL, sep = "", month = NULL, year = NULL, step = 12, shift = NULL, trans = NULL, scheme = 2, s2.col = NULL, grid = 50, ...) {
  rugp <- attr(x, "rug")
  if (is.null(x)) 
 ...
 if (any(grep("+", as.character(x)[2]))) {
   xch <- as.character(x)
   if (xch[2] %in% names(data)) {
  if (inherits(data[[xch[2]]], "data.frame")) {
  data[[xch[2]]] <- as.matrix(data[[xch[2]]])
  }
   }
   x <- try(model.frame(as.formula(paste("~", xch[2])), data = data), silent = TRUE)
   if (inherits(x, "try-error")) {
  x <- model.frame(as.formula(paste0("~ as.matrix(", xch[2], ")")), data = data)
   }
   x <- as.matrix(x)
   colnames(x) <- gsub(paste0(xch[2], "."), "", colnames(x), fixed = TRUE)
   x <- cbind(model.frame(as.formula(paste("~", xch[3])), data = data), x)
 }
 else x <- model.frame(x, data = data)
 if (ncol(x) < 2) 
   stop("formula is specified wrong!")
  }
  is.bayesx <- grepl(".bayesx", class(x))[1]
  if (is.data.frame(x)) {
 ...
 if (is.null(col.lines)) {
   ...
   col.lines <- c(NA, col.lines[!is.na(col.lines)][1], NA)
 }
  }
  args <- list(...)
  nc <- ncol(x)
  if (is.null(c.select)) {
 if (is.bayesx) 
   c.select <- c(1, 2, 3, 4, 6, 7)
 else c.select <- 1:nc
  }
  if (is.null(c.select)) 
 c.select <- 1:nc
  if (length(c.select) > nc) 
 c.select <- c.select[1:nc]
  if (is.null(fill.select)) {
 if (is.bayesx) 
   fill.select <- c(0, 0, 1, 2, 2, 1)
 if (all(c("Mean", "2.5%", "97.5%") %in% colnames(x)) & (ncol(x) == 4)) 
 ...
 args$col.polygons <- rep(c("grey80", "grey70"), round(nc/2))
  else args$col.polygons <- col.polygons
 ...
 else args$ylab <- attr(x, "specs")$label
  }
 ...
 if (!is.function(trans)) 
   stop("argument trans must be a function!")
 for (j in 2:ncol(x)) x[, j] <- trans(x[, j])
  }
  if (residuals) {
 if (!is.null(shift)) 
   pres[, 2] <- pres[, 2] + shift
 ...
 if (residuals) 
   args$ylim <- range(c(ylim, pres[, 2]), na.rm = TRUE)
 else args$ylim <- range(ylim, na.rm = TRUE)
  }
  if (is.null(args$xlim)) 
 args$xlim <- base::range(x[, 1], na.rm = TRUE)
  if (!(!is.null(args$add) && args$add)) {
 graphics::plot(args$xlim, args$ylim, type = "n", axes = FALSE, xlab = args$xlab, ylab = args$ylab, main = args$main)
  }
  args <- set.plot2d.specs(ncol(x) - 1, args, col.lines, is.bayesx)
  if (!is.null(args$add)) {
 if (args$add) {
 ...
 if (!is.null(month) & !is.null(year)) {
   ...
   if (nrow(x) <= 24) {
  label2 <- month.abb[ifelse(step == 12, 1:12, ifelse(step == 4, c(1, 4, 7, 10), ifelse(step == 2, c(1, 7), FALSE)))]
 ...
 else axis(1, cex.axis = args$cex.axis)
  }
  else {
 if (args$axes) {
   axis(2, cex.axis = args$cex.axis)
   axis(1, cex.axis = args$cex.axis)
 }
  }
  return(invisible(NULL))
}
plot2d.default <- function(x, residuals, range, col.residuals = "black", fill.select = NULL, col.polygons = NULL, col.rug = NULL, pb = FALSE, x.co = NULL, rug = FALSE, jitter = FALSE, specs) {
  if (residuals && !is.null(pres <- attr(x, "residuals"))) 
 residuals <- TRUE
  else residuals <- FALSE
  if (nrow(x) > 1) 
 ...
 e <- attr(x, "residuals")
  ...
  if (pb) {
 nc <- ncol(x)
 if (length(ux <- unique(x[, 2:nc])) < 3) {
   fill.select <- NULL
   if (!is.matrix(ux)) 
  ux <- matrix(ux, nrow = 1)
 }
 else ux <- matrix(unique(x[, 2:nc]), ncol = (nc - 1))
 nux <- nrow(ux)
 if (nux < 2) {
   nux <- 2
   ux <- rbind(ux, ux)
 }
 x.co <- seq(x.co + range[1], x.co - range[2], length = nux)
 x <- cbind(x.co, ux)
 x <- rbind(x, x, x)
  }
 ...
 for (k in 1:nu) {
  ...
  if (specs$scheme == 1) {
  p1 <- poly[, 1, drop = FALSE]
  p2 <- poly[, 2, drop = FALSE]
  y.co <- c(p1, p2[length(p2):1])
  x.co <- x[, 1]
  x.co <- c(x.co, x.co[length(x.co):1])
  graphics::polygon(x = x.co, y = y.co, col = col.polygons[k], lty = specs$poly.lty[k], border = specs$border[k], density = specs$density[k], angle = specs$angle[k], lwd = specs$poly.lwd[k])
  }
  else {
  grid <- if (is.null(specs$grid)) 
    30
  else specs$grid
  ...
  if (length(mx)) {
    ...
    poly <- apply(poly, 1, function(x) {
    x <- as.numeric(x)
    seq(x[1], x[2], length = grid)
    })
  }
  ...
  for (pj in 1:(grid/2)) {
    ...
    x.co <- x[, 1]
    x.co <- c(x.co, x.co[length(x.co):1])
    graphics::polygon(x = x.co, y = y.co, col = specs$s2.col[pj], lty = specs$poly.lty[k], border = specs$border[k], density = specs$density[k], angle = specs$angle[k], lwd = specs$poly.lwd[k])
  }
  }
   }
 }
 ...
 do.call(graphics::points, pargs)
  ...
  for (k in 2:ncol(x)) {
 coll <- specs$col.lines[k - 1]
 if (ncol(x) == 2) 
   coll[is.na(coll)] <- 1
 lines(x[, k] ~ x[, 1], lty = specs$lty[k - 1], lwd = specs$lwd[k - 1], col = coll)
  }
  if (rug) {
 specs$col <- col.rug
 rugp <- if (!is.null(specs$rugp)) 
   specs$rugp
 else x[, 1]
 if (jitter) 
   specs$x <- jitter(rugp)
...
plot3d <- function(x, residuals = FALSE, col.surface = NULL, ncol = 99, swap = FALSE, col.residuals = NULL, col.contour = NULL, c.select = NULL, grid = 30, image = FALSE, contour = FALSE, legend = TRUE, cex.legend = 1, breaks = NULL, range = NULL, digits = 2, d.persp = 1, r.persp = sqrt(3), outscale = 0, data = NULL, sep = "", shift = NULL, trans = NULL, type = "mba", linear = FALSE, extrap = FALSE, k = 40, ...) {
  if (is.null(x)) 
 return(invisible(NULL))
  if (is.character(x)) {
 stopifnot(file.exists(x <- path.expand(x)))
 ...
 if (!is.null(specs) && length(specs$term) > 2) 
 ...
 take <- NULL
 id <- 1:length(nx)
 if (length(c.select) < 2 && c.select == 95) 
   c.select <- as.character(c.select)
 if (length(c.select) < 2 && c.select == 80) 
   c.select <- as.character(c.select)
 is.se <- FALSE
 ...
 if (is.se) {
   take2 <- c("mean", "Mean", "MEAN", "estimate", "Estimate", "ESTIMATE", "mean", "pmode")
   for (k in take2) if (any(nx %in% k)) 
 ...
 if (is.null(take) && !is.character(c.select)) {
   if (min(c.select) < 3) 
  c.select <- c.select + 2
   if (max(c.select) > ncol(x) || min(c.select) < 3) 
  stop("argument c.select is specified wrong!")
   take <- unique(c.select)
 }
 if (is.null(take) && is.character(c.select)) 
   for (k in c.select) for (i in 1:length(nx)) if (!is.na(pmatch(k, nx[i]))) 
  take <- c(take, i)
 if (is.null(take)) 
   stop("argument c.select is specified wrong!")
 for (k in 1:length(take)) {
   fitted[[k]] <- interp2(X, z, x[, take[k]], xo = xn, yo = zn, type = type, linear = linear, extrap = extrap, k = k)
 }
  }
  if (length(fitted[[1]]) == 1 && is.na(fitted[[1]][1])) {
 fitted[[1]] <- interp2(X, z, x[, 3], xo = xn, yo = zn, type = type, linear = linear, extrap = extrap, k = k)
  }
  if (!is.null(range)) {
 ...
 for (k in 1:length(fitted)) {
  ...
  if (residuals) 
 zlimit <- range(c(unlist(fitted), e[, 3]), na.rm = TRUE)
  else zlimit <- range(unlist(fitted), na.rm = TRUE)
  if (is.null(args$xlab)) 
 args$xlab <- names[1]
  if (is.null(args$ylab)) 
 args$ylab <- names[2]
 ...
 if (!is.null(specs) && is.null(specs$label)) 
   args$zlab <- "fitted"
 else args$zlab <- specs$label
  }
  if (is.null(args$zlab)) 
 args$zlab <- try(paste("f(", nx[1], ",", nx[2], ")", sep = ""))
 ...
 par(new = TRUE)
  pmat0 <- NULL
  if (!image && !contour) {
 myfit <- matrix(fitted[[1]], grid, grid)
 if (length(fitted) < 2) {
   args$col <- make_pal(col = col.surface, ncol = ncol, data = myfit, range = range, breaks = breaks, swap = swap, symmetric = args$symmetric)$map(myfit)
 }
 ...
 if (is.null(args$phi)) 
   args$phi <- 40
 if (!is.null(c.select) && length(fitted) > 1) {
   nf <- length(fitted)
  ...
  args$col <- NA
   ...
   args$col <- color[1]
   args$border <- bcol[1]
   pmat <- pmat0 <- do.call(graphics::persp, delete.args("persp.default", args, c("lwd", "lty"), package = "graphics"))
   for (k in 2:length(fitted)) {
  par(new = TRUE)
  args$col <- color[k]
  args$border <- bcol[k]
  if (k > 1) {
  ...
  args$z <- substitute(myfit)
  pmat <- do.call(graphics::persp, delete.args("persp.default", args, c("lwd", "lty"), package = "graphics"))
   }
 }
 else {
   pmat <- pmat0 <- do.call(graphics::persp, delete.args("persp.default", args, c("lwd", "lty"), package = "graphics"))
 ...
 if (residuals && !is.null(e)) {
  ...
  if (image || contour) {
 myfit <- matrix(fitted[[1]], grid[1], grid[1])
 pal <- make_pal(col = col.surface, ncol = ncol, data = myfit, range = range, breaks = breaks, swap = swap, symmetric = args$symmetric)
 args$col <- pal$colors
 ...
 if (image) {
  ...
  layout(matrix(c(1, 2), nrow = 1), widths = c(1, lcm(w)))
   ...
   do.call(graphics::image, delete.args(graphics::image.default, args, c("xlab", "ylab", "main", "axes")))
  ...
  args$col <- "black"
  else args$col <- col.contour
  args$add <- TRUE
  do.call(graphics::contour.default, delete.args(graphics::contour.default, args, c("xlab", "ylab", "main", "axes")))
  contour <- FALSE
   }
   if (legend) {
  if (is.null(args$pos)) {
...
plotblock <- function(x, residuals = FALSE, range = c(0.3, 0.3), col.residuals = "black", col.lines = "black", c.select = NULL, fill.select = NULL, col.polygons = NULL, data = NULL, shift = NULL, trans = NULL, labels = NULL, ...) {
  if (is.null(x)) 
 return(invisible(NULL))
  if (inherits(x, "formula")) {
 if (is.null(data)) 
 ...
 if (any(grep("+", as.character(x)[2]))) {
   xch <- as.character(x)
   x <- model.frame(as.formula(paste("~", xch[2])), data = data)
   x <- cbind(model.frame(as.formula(paste("~", xch[3])), data = data), x)
 }
 else x <- model.frame(x, data = data)
 if (ncol(x) < 2) 
 ...
 for (i in 1:n) {
   effects[[i]] <- x[x[, 1] == xu[i], c.select, drop = FALSE]
   if (!is.matrix(effects[[i]])) 
  effects[[i]] <- matrix(effects[[i]], nrow = 1)
   if (!is.matrix(effects[[i]])) 
  effects[[i]] <- matrix(effects[[i]], nrow = 1)
   if (!is.null(shift)) 
  effects[[i]][, 2:ncol(x[[i]])] <- effects[[i]][, 2:ncol(x[[i]])] + shift
   if (!is.null(trans)) {
  if (!is.function(trans)) 
  stop("argument trans must be a function!")
  for (j in 2:ncol(effects[[i]])) effects[[i]][, j] <- trans(effects[[i]][, j])
   }
   ylim <- c(ylim, effects[[i]][, 2:ncol(effects[[i]])])
   colnames(effects[[i]]) <- rep(paste(xnam, xu[i], sep = ""), ncol(effects[[i]]))
   if (residuals) {
  if (length(pres <- e[e[, 1] == xu[i], ])) {
  if (is.null(dim(pres))) 
    pres <- matrix(pres, nrow = 1)
  if (!is.null(shift)) 
    pres[, 2:ncol(pres)] <- pres[, 2:ncol(pres)] + shift
  attr(effects[[i]], "residuals") <- pres
  ylim <- c(ylim, pres[, 2:ncol(pres)])
  }
   }
 }
 x <- effects
 ...
 for (i in 1:n) {
  ...
  x[[i]] <- matrix(x[[i]], nrow = 1)
   x[[i]] <- x[[i]][x[[i]][, 1] != 0 & x[[i]][, 1] != -1, ]
   if (!is.matrix(x[[i]])) 
  x[[i]] <- matrix(x[[i]], nrow = 1)
   if (nrow(x[[i]]) == 2 && x[[i]][1, 1] == -1) 
  x[[i]] <- matrix(x[[i]][2, ], nrow = 1)
   colnames(x[[i]]) <- cn
   if (residuals) {
  if (!is.null(shift)) {
  if (is.matrix(pres)) 
    pres[, 2:ncol(pres)] <- pres[, 2:ncol(pres)] + shift
  else pres <- pres + shift
  }
  attr(x[[i]], "residuals") <- pres
   }
   if (!is.null(shift)) 
  x[[i]][, 2:ncol(x[[i]])] <- x[[i]][, 2:ncol(x[[i]])] + shift
   if (!is.null(trans)) {
  if (!is.function(trans)) 
  stop("argument trans must be a function!")
  for (j in 2:ncol(x[[i]])) x[[i]][, j] <- trans(x[[i]][, j])
   }
   ylim <- c(ylim, x[[i]][, 2:ncol(x[[i]])])
 }
  }
  if (!is.null(labels)) 
 ...
 par(new = TRUE)
  graphics::plot(args$xlim, args$ylim, type = "n", axes = FALSE, xlab = args$xlab, ylab = args$ylab, main = args$main)
  args <- set.plot2d.specs(ncol(x[[1]]) - 1, args, col.lines, is.bayesx)
  xnames <- NULL
  axn <- rep(NA, n)
  args$specs <- args
  args$residuals <- residuals
 ...
 if (!is.null(attr(args$x, "residuals"))) 
   attr(args$x, "residuals")[, 1] <- i
 do.call(plot2d.default, delete.args(plot2d.default, args))
 axn[i] <- if (is.null(labels)) 
   colnames(x[[i]])[1]
 else labels[i]
  }
  if (is.null(args$type)) 
...
colorlegend <- function(color = NULL, ncol = NULL, x = NULL, breaks = NULL, pos = "center", shift = 0.02, side.legend = 1, side.ticks = 1, range = NULL, lrange = NULL, width = 0.25, height = 0.05, scale = TRUE, xlim = NULL, ylim = NULL, plot = NULL, full = FALSE, add = FALSE, col.border = "black", lty.border = 1, lwd.border = 1, ticks = TRUE, at = NULL, col.ticks = "black", lwd.ticks = 1, lty.ticks = 1, length.ticks = 0.3, labels = NULL, distance.labels = 0, col.labels = "black", cex.labels = 1, 
  digits = 2, swap = FALSE, symmetric = TRUE, xpd = NULL, title = NULL, side.title = 2, shift.title = c(0, 0), cex.title = 1, ...) {
  args <- list(...)
  if (is.null(xlim)) {
 if (add) 
   xlim <- par("usr")[1:2]
 ...
 plot <- TRUE
 graphics::plot.default(xlim, ylim, type = "n", xlab = "", ylab = "", axes = FALSE, xlim = xlim, ylim = ylim, asp = NA)
  }
  else plot <- FALSE
  if (is.null(xpd)) 
 xpd <- FALSE
 ...
 par(xpd = xpd)
  pos2 <- NULL
  postxt <- c("bottomleft", "topleft", "topright", "bottomright", "left", "right", "top", "bottom", "center")
 ...
 if (side.legend < 2) 
  ...
  pos <- opos <- c(min(limits[[1]], na.rm = TRUE) + pos[1] * diff(limits[[1]]), min(limits[[2]], na.rm = TRUE) + pos[2] * diff(limits[[2]]))
  if (side.legend > 1) 
 limits <- rev(limits)
  if (scale) {
 width <- width * diff(limits[[1]])
 height <- height * diff(limits[[2]])
  }
  if (side.legend > 1) {
 wi <- width
 width <- height
 ...
 shift <- 0
  ...
  if (is.null(pos2)) {
 xlim <- range(c(pos[1], pos[1] + width, pos[1] + width, pos[1])) + shift[1] * width
 ylim <- range(c(pos[2], pos[2], pos[2] + height, pos[2] + height)) + shift[2] * height
  }
  else {
 ...
 cl <- c(head(pal$colors, 1), pal$colors, tail(pal$colors, 1))
 obs2legend <- function(x, xr) ((x - lrange[1])/diff(lrange)) * diff(xr) + xr[1]
 if (side.legend < 2) {
   graphics::rect(obs2legend(head(br, -1), xlim), ylim[1], obs2legend(tail(br, -1), xlim), ylim[2], col = cl, border = cl, xpd = xpd, lwd = 0.01)
 }
 else {
   graphics::rect(xlim[1], obs2legend(head(br, -1), ylim), xlim[2], obs2legend(tail(br, -1), ylim), col = cl, border = cl, xpd = xpd, lwd = 0.01)
 }
 graphics::rect(xlim[1], ylim[1], xlim[2], ylim[2], border = col.border, lwd = lwd.border, lty = lty.border, xpd = xpd)
 dl <- TRUE
 if (!is.null(labels)) {
 ...
 if (ticks || dl) {
  ...
  at <- seq(min(lrange), max(lrange), length.out = 3)
   }
   if (is.null(labels)) {
  justify <- "centre"
  ...
  if (side.legend < 2) {
    if (ticks) {
    graphics::lines(c(at[i], at[i]), c(ylim[side.ticks], ylim[side.ticks] - length.ticks), lwd = lwd.ticks[i], lty = lty.ticks[i], col = col.ticks[i])
    }
    if (dl) {
    graphics::text(at[i], ylim[side.ticks] - length.ticks - (distance.labels * length.ticks * 2), labels = labels[i], col = col.labels[i], cex = cex.labels[i], pos = 1, ...)
    }
  }
  else {
    if (ticks) {
    graphics::lines(c(xlim[side.ticks], xlim[side.ticks] - length.ticks), c(at[i], at[i]), lwd = lwd.ticks[i], lty = lty.ticks[i], col = col.ticks[i])
    }
    if (dl) {
    graphics::text(xlim[side.ticks] - length.ticks - (distance.labels * length.ticks * 2), at[i], labels = labels[i], col = col.labels[i], cex = cex.labels[i], pos = if (side.ticks < 2) 
   2
    else 4, ...)
    }
  }
  ...
  axis(where, at = at, labels = labels, col = col.labels, tick = ticks, lty = lty.ticks, col.ticks = col.ticks, lwd.ticks = lwd.ticks, cex.axis = cex.labels[1])
   }
 }
 if (!is.null(title)) {
  ...
  shift.title <- c(shift.title, 0)
   if (!full) {
  xp <- xlim[1] + shift.title[1] * diff(range(xlim)) + diff(range(xlim))/2
  yp <- ylim[2] + shift.title[2] * diff(range(ylim))
  text(if (side.legend < 2) 
  xp
  else yp, if (side.legend < 2) 
  yp
  else xp, title, pos = 3, srt = if (side.legend == 2) 
  270
  else 0, cex = cex.title, xpd = xpd)
   }
   else {
...
make_pal <- function(col, ncol = NULL, data = NULL, range = NULL, breaks = NULL, swap = TRUE, symmetric = TRUE) {
 ...
 if (diff(range) == 0) 
   breaks <- seq(min(range) - 1, min(range) + 1, length.out = ncol + 1)
 else breaks <- seq(range[1], range[2], length.out = ncol + 1)
  }
  else stopifnot(length(breaks) == ncol + 1)
  if (is.matrix(data)) {
 obs2col <- function(x) {
   hgt <- (x[-1, -1] + x[-1, -(ncol(x) - 1)] + x[-(nrow(x) - 1), -1] + x[-(nrow(x) - 1), -(ncol(x) - 1)])/4
   c(col[1], col, col[ncol])[cut(hgt, c(-Inf, breaks, Inf))]
 }
  }
  else {
 obs2col <- function(x) c(col[1], col, col[ncol])[cut(x, c(-Inf, breaks, Inf))]
  }
...
dopos <- function(pos, limits, width, height, side.legend, shift) {
 ...
 limits <- rev(limits)
  ...
  if (pos == "bottomleft") {
 xlim <- c(min(limits[[1]], na.rm = TRUE), min(limits[[1]], na.rm = TRUE) + width) + shift[1]
 ylim <- c(min(limits[[2]], na.rm = TRUE), min(limits[[2]], na.rm = TRUE) + height) + shift[2]
  }
  if (pos == "topleft") {
 xlim <- c(min(limits[[1]], na.rm = TRUE), min(limits[[1]], na.rm = TRUE) + width) + shift[1]
 ylim <- c(max(limits[[2]], na.rm = TRUE) - height, max(limits[[2]], na.rm = TRUE)) - shift[2]
  }
  if (pos == "topright") {
 xlim <- c(max(limits[[1]], na.rm = TRUE) - width, max(limits[[1]], na.rm = TRUE)) - shift[1]
 ylim <- c(max(limits[[2]], na.rm = TRUE) - height, max(limits[[2]], na.rm = TRUE)) - shift[2]
  }
  if (pos == "bottomright") {
 xlim <- c(max(limits[[1]], na.rm = TRUE) - width, max(limits[[1]], na.rm = TRUE)) - shift[1]
 ...
 xlim <- c(min(limits[[1]], na.rm = TRUE), min(limits[[1]], na.rm = TRUE) + width) + m
 ylim <- c(min(limits[[2]], na.rm = TRUE), min(limits[[2]], na.rm = TRUE) + height) + shift[2]
  }
  if (pos == "top") {
 m <- mean(limits[[1]] - min(limits[[1]], na.rm = TRUE), na.rm = TRUE) - 0.5 * width
 ...
 xlim <- c(min(limits[[1]], na.rm = TRUE), min(limits[[1]], na.rm = TRUE) + width) + shift[1]
 ylim <- c(min(limits[[2]], na.rm = TRUE), min(limits[[2]], na.rm = TRUE) + height) + m
  }
  if (pos == "right") {
 m <- mean(limits[[2]] - min(limits[[2]], na.rm = TRUE), na.rm = TRUE) - 0.5 * height
 xlim <- c(max(limits[[1]], na.rm = TRUE) - width, max(limits[[1]], na.rm = TRUE)) - shift[1]
 ...
 xlim <- c(min(limits[[1]], na.rm = TRUE), min(limits[[1]], na.rm = TRUE) + width) + mx
 ylim <- c(min(limits[[2]], na.rm = TRUE), min(limits[[2]], na.rm = TRUE) + height) + my
  }
...
list2sp <- function(x) {
 ...
 ret[[id]] <- sp::Polygons(srl = idPolygons, ID = id)
  ...
  whichAreInner <- which(sapply(surrounding, length) > 0)
...
plotmap <- function(map, x = NA, id = NULL, select = NULL, legend = TRUE, names = FALSE, values = FALSE, ...) {
 ...
 if (missing) {
  ...
  args$mdensity <- 20
   plot(map, density = args$mdensity, border = args$border, xlim = args$xlim, ylim = args$ylim, xpd = args$xpd, angle = args$angle, pbg = args$pbg, axes = args$axes, lty = args$lty, lwd = args$lwd, xlab = args$xlab, ylab = args$ylab, main = args$main)
 }
 plot(map, col = pal$map(map@data[[select[1]]]), border = args$border, add = args$add | missing, xlim = args$xlim, ylim = args$ylim, xpd = args$xpd, density = args$density, angle = args$angle, pbg = args$pbg, axes = args$axes, lty = args$lty, lwd = args$lwd, xlab = args$xlab, ylab = args$ylab, main = args$main)
 if (legend) {
   args$add <- TRUE
   pal <- do.call("colorlegend", delete.args("colorlegend", args))
 }
 if (values) {
   co <- coordinates(map)
   text(co[, 1], co[, 2], as.character(round(map@data[[select[1]]], digits = args$digits)), cex = args$cex)
 }
  }
  if (names) {
 co <- coordinates(map)
...
.plotmap <- function(map, x = NULL, id = NULL, c.select = NULL, legend = TRUE, missing = TRUE, swap = FALSE, range = NULL, names = FALSE, values = FALSE, col = NULL, ncol = 100, breaks = NULL, cex.legend = 1, cex.names = 1, cex.values = cex.names, digits = 2, mar.min = 2, add = FALSE, interp = FALSE, grid = 200, land.only = FALSE, extrap = FALSE, outside = FALSE, type = "interp", linear = FALSE, k = 40, p.pch = 15, p.cex = 1, shift = NULL, trans = NULL, scheme = 1, ...) {
  if (missing(map)) 
 ...
 for (i in upn) {
   j <- poly.names == i
   poly.names[j] <- paste(poly.names[j], if (sum(j) > 1) 
  paste(".", 1:sum(j), sep = "")
   else NULL, sep = "")
 }
 names(map) <- poly.names
  ...
  inner.id <- which(sapply(surrounding, length) > 0)
  if (length(inner.id)) {
 ...
 map_fun <- make_pal(col = col, ncol = ncol, data = as.numeric(x$x), range = range, breaks = breaks, swap = swap, symmetric = symmetric)$map
 ...
 if (is.null(col)) 
  ...
  if (is.null(args$pos)) 
 args$pos <- "right"
  if (legend && !is.null(args$pos) && args$pos[1] == "right") {
 ...
 ico <- interp2(x = cdata[["x"]], y = cdata[["y"]], z = cdata[["z"]], xo = xo, yo = yo, type = type, linear = linear, extrap = extrap, k = if (is.null(k)) 
   ceiling(length(map) * 0.8)
 else as.integer(k))
 ...
 if (!outside) {
  ...
  pip <- cbind(pip, point.in.polygon(xco, yco, oco[, 1], oco[, 2], mode.checked = FALSE) < 1)
   }
 ...
 if (length(res)) {
   rect(pp[, 1] - res[1]/2, pp[, 2] - res[2]/2, pp[, 1] + res[1]/2, pp[, 2] + res[2]/2, col = icolors, border = NA, lwd = 0)
 }
 else {
 ...
 colors <- rep(NA, length = length(colors))
  ...
  args$type <- NULL
  args$axes <- NULL
  lwd.p <- if (!is.null(args$lwd)) 
 rep(args$lwd, length.out = n)
  else NULL
  if (is.null(lwd.p)) 
 lwd.p <- rep(1, length.out = n)
  lty.p <- if (!is.null(args$lty)) 
 rep(args$lty, length.out = n)
  else NULL
  if (is.null(lty.p)) 
 lty.p <- rep(1, length.out = n)
  border.p <- if (!is.null(args$border)) 
 rep(args$border, length.out = n)
  else NULL
  if (is.null(border.p)) 
 border.p <- rep(if (scheme != 1) "white" else "black", length.out = n)
  density.p <- if (!is.null(args$density)) 
 rep(args$density, length.out = n)
  else NULL
  angle.p <- if (!is.null(args$angle)) 
 rep(args$angle, length.out = n)
  else NULL
  if (is.null(angle.p)) 
 ...
 for (i in which(poly.names.orig == poly)) {
  ...
  if (is.null(args$density)) 
   ...
   do.call(graphics::polygon, delete.args(graphics::polygon, args, c("lwd", "cex", "lty")))
   if (names && !values) {
  args$polygon <- map[[i]]
 ...
 do.call(colorlegend, delete.args(colorlegend, args, c("font")))
  ...
  if (!is.null(args$xlab)) 
 mtext(args$xlab, side = 1)
  if (!is.null(args$ylab)) 
 mtext(args$ylab, side = 2)
  return(invisible(NULL))
}
plot.bnd <- function(x, ...) {
  plotmap(x, ...)
...
compute.x.id <- function(x, id = NULL, c.select = NULL, range = NULL, symmetric = TRUE) {
 ...
 if (is.matrix(x)) {
  ...
  nx <- names(x)
  if (all(nx %in% paste("V", 1:length(nx), sep = ""))) {
  nx[1:2] <- c("id", "x")
  c.select <- "x"
  }
   }
 ...
 if (is.null(c.select)) {
   take <- c("mean", "Mean", "MEAN", "estimate", "Estimate", "ESTIMATE", "mean", "pmode", "pmean_tot")
   did.take <- FALSE
   for (k in take) {
...
df2m <- function(x) {
 ...
 for (k in 1:length(nxa)) if (all(nxa[k] != c("dim", "dimnames", "class", "names", "row.names"))) {
...
find.limits <- function(map, mar.min = 2, ...) {
 ...
 stop("argument map must be a list() of matrix polygons!")
  n <- length(map)
  myrange <- function(x, c.select = 1, ...) {
 if (!is.null(dim(x))) {
   return(na.omit(x[, c.select], ...))
 }
 else return(na.omit(x))
  }
  xlim <- range(unlist(lapply(map, myrange, c.select = 1, ...)))
  ylim <- range(unlist(lapply(map, myrange, c.select = 2, ...)))
  mar <- NULL
  asp <- attr(map, "asp")
  if (is.null(asp)) 
...
set.plot2d.specs <- function(nc, args, col.lines, is.bayesx) {
 ...
 lwd <- rep(1, nc)
  else lwd <- rep(lwd, length.out = nc)
  lty <- args$lty
  if ((is.null(lty) || any(is.na(lty))) && !is.bayesx[1]) 
 lty <- rep(1, nc)
  if ((is.null(lty) || any(is.na(lty))) && is.bayesx[1]) 
 lty <- c(1, 0, 0, 0, 0)
 ...
 col.lines <- rep("black", nc)
  else col.lines <- rep(col.lines, length.out = nc)
  args$lty <- lty
  args$lwd <- lwd
  args$col.lines <- col.lines
  return(args)
}
interp2 <- function(x, y, z, xo = NULL, yo = NULL, grid = 30, type = c("mba", "interp", "mgcv", "gam", "raw"), linear = FALSE, extrap = FALSE, k = 40) {
  type <- tolower(type)
  type <- match.arg(type)
  if (is.null(xo)) 
 ...
 fit <- MBA::mba.surf(data.frame(x = x, y = y, z = z), xgrid, ygrid)$xyz.est$z
  ...
  if (type == "interp") {
 fit <- try(interp::interp(x, y, z, xo = xo, yo = yo, duplicate = "mean", linear = linear, extrap = extrap)$z, silent = TRUE)
 if (inherits(fit, "try-error") | all(is.na(fit))) {
   ...
   fit <- try(interp::interp(jitter(x, amount = .Machine$double.eps), jitter(y, amount = .Machine$double.eps), z, xo = xo, yo = yo, duplicate = "mean", linear = linear, extrap = extrap)$z, silent = TRUE)
 }
  }
...
sliceplot <- function(x, y = NULL, z = NULL, view = 1, c.select = NULL, values = NULL, probs = c(0.1, 0.5, 0.9), grid = 100, legend = TRUE, pos = "topright", digits = 2, data = NULL, rawdata = FALSE, type = "mba", linear = FALSE, extrap = FALSE, k = 40, rug = TRUE, rug.col = NULL, jitter = TRUE, ...) {
  if (is.vector(x) & is.vector(y) & is.vector(z)) {
 nx <- c(deparse(substitute(x), backtick = TRUE, width.cutoff = 500), deparse(substitute(y), backtick = TRUE, width.cutoff = 500), deparse(substitute(z), backtick = TRUE, width.cutoff = 500))
 x <- cbind(x, y, z)
 colnames(x) <- nx
  }
  else {
 ...
 zi <- interp2(x[, view], x[, noview], x[, c.select], xo = xo, yo = yo, type = type, linear = linear, extrap = extrap, k = k)
 yg <- rep(yo, each = grid)
 zg <- as.vector(zi)
 slices <- xo
  }
 ...
 slices <- cbind(slices, zg[yg == val])
  ...
  k <- ncol(slices)
  args <- l.args <- list(...)
  args$lty <- if (is.null(args$lty)) 
 1:k
  else rep(args$lty, length.out = k)
  args$col <- if (is.null(args$col)) 
 "black"
  else rep(args$col, length.out = k)
  args$lwd <- if (is.null(args$lwd)) 
 1
  else rep(args$lwd, length.out = k)
  if (is.null(args$xlab)) 
 args$xlab <- nx[view]
  if (is.null(args$ylab)) 
