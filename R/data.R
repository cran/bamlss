...
data_Germany <- function(dir = NULL) {
 ...
 dir <- "~/svn/bayesr/pkg/bamlss/data"
  ...
  on.exit(unlink(tf))
  download.file("http://biogeo.ucdavis.edu/data/gadm2/shp/DEU_adm.zip", zf <- file.path(tf, "germany.zip"))
  unzip(zf, exdir = gsub("\\.zip$", "", zf))
  g <- sf::st_read(file.path(tf, "germany", "DEU_adm3.shp"))
  g <- sf::as_Spatial(g)
  rn <- as.character(d$NAME_3)
  Encoding(rn) <- "latin1"
  GermanyBnd <- BayesX::sp2bnd(g, regionNames = rn)
  d <- slot(g, "data")
  d <- data.frame(name = as.character(d$NAME_3), id = as.character(d$ID_3), stringsAsFactors = FALSE)
...
data_spam <- function(...) {
  ...
  i <- grep("continuous.", dnames, fixed = TRUE)[1]
 ...
 for (ch in seq_along(chars)) dnames <- gsub(chars[ch], rchars[ch], dnames, fixed = TRUE)
  }
