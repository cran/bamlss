cox_bamlss <- function(...) {
  rval <- list(family = "cox", names = c("lambda", "gamma"), links = c(lambda = "log", gamma = "log"), transform = function(x, ...) {
 surv_transform(x = x$x, y = x$y, data = model.frame(x), family = x$family, is.cox = TRUE, ...)
  }, optimizer = opt_Cox, sampler = sam_Cox, predict = cox_predict)
  class(rval) <- "family.bamlss"
  rval
}
opt_Cox <- cox_mode <- function(x, y, start, weights, offset, criterion = c("AICc", "BIC", "AIC"), nu = 0.1, update.nu = TRUE, eps = .Machine$double.eps^0.25, maxit = 400, verbose = TRUE, digits = 4, ...) {
  if (nu < 0 | nu > 1) 
 ...
 if (length(x[[i]]$smooth.construct)) {
   for (j in names(x[[i]]$smooth.construct)) x[[i]]$smooth.construct[[j]]$state$edf <- 0
 }
  }
  edf <- 0
  y <- y[[1]]
 ...
 eta0 <- eta
 for (sj in seq_along(x$lambda$smooth.construct)) {
   state <- update_surv_tv(x$lambda$smooth.construct[[sj]], y, eta, eta_timegrid, width, sub, update.nu, criterion = criterion, edf = edf, ...)
 ...
 for (sj in seq_along(x$gamma$smooth.construct)) {
   state <- update_surv_tc(x$gamma$smooth.construct[[sj]], y, eta, eeta, int, criterion = criterion, edf = edf, ...)
 ...
 if (verbose) {
  ...
  "\r"
   ...
   vtxt <- paste(criterion, " ", fmt(IC, width = 8, digits = digits), " logLik ", fmt(logLik, width = 8, digits = digits), " edf ", fmt(edf, width = 6, digits = digits), " eps ", fmt(eps0, width = 6, digits = digits + 2), " iteration ", formatC(iter, width = nchar(maxit[1])), sep = "")
   cat(vtxt)
 ...
 cat("\nelapsed time: ", et, "\n", sep = "")
  ...
  rval <- list(fitted.values = eta, parameters = get.all.par(x), edf = get.edf(x, type = 2), logLik = logLik, logPost = logPost, hessian = get.hessian(x), converged = iter < maxit[1], time = elapsed)
...
update_surv_tv <- function(x, y, eta, eta_timegrid, width, sub, update.nu, criterion, edf, ...) {
 ...
 objfun1 <- function(tau2) {
  ...
  par[x$pid$b] <- g2
  opt_state <- list(parameters = par, fitted.values = fit, fitted_timegrid = fit_timegrid, edf = edf1, hessian = xhess, nu = nu, do.optim = x$state$do.optim)
...
sam_Cox <- cox_mcmc <- function(x, y, family, start, weights, offset, n.iter = 1200, burnin = 200, thin = 1, verbose = TRUE, digits = 4, step = 20, ...) {
  nu <- 1
  if (!is.null(start)) 
 x <- set.starting.values(x, start)
  nx <- names(x)
...
propose_surv_tv <- function(x, y, eta, eta_timegrid, width, sub, nu) {
 ...
 for (j in i) {
   x$state$parameters <- uni.slice(x$state$parameters, x, NULL, NULL, NULL, "lambda", j, logPost = gmcmc_logPost, lower = 0, ll = pibeta)
 }
  }
...
propose_surv_tc <- function(x, y, eta, int) {
 ...
 i <- grep("tau2", names(x$state$parameters))
 for (j in i) {
   x$state$parameters <- uni.slice(x$state$parameters, x, NULL, NULL, NULL, "gamma", j, logPost = gmcmc_logPost, lower = 0, ll = pibeta)
 }
  }
  x$state$parameters <- set.par(x$state$parameters, g, "b")
  x$state$alpha <- drop((pibetaprop + qbeta + p2) - (pibeta + qbetaprop + p1))
...
rSurvTime2 <- function(lambda, x, cens_fct, upper = 1000, ..., file = NULL, subdivisions = 1000) {
  if (!is.matrix(x)) 
 x <- cbind(x)
  time <- rep(NA, nrow(x))
...
surv_transform <- function(x, y, data, family, subdivisions = 100, timedependent = "lambda", timevar = NULL, idvar = NULL, is.cox = FALSE, alpha = 0.1, ...) {
  rn <- names(y)
  y <- y[[rn]]
  ntd <- timedependent
 ...
 if (!is.null(x$lambda$smooth.construct$model.matrix)) {
  ...
  x$lambda$smooth.construct$model.matrix$term <- gsub("(Intercept)+", "", x$lambda$smooth.construct$model.matrix$term, fixed = TRUE)
  x$lambda$smooth.construct$model.matrix$state$parameters <- x$lambda$smooth.construct$model.matrix$state$parameters[-1]
  x$lambda$terms <- drop.terms.bamlss(x$lambda$terms, pterms = TRUE, sterms = TRUE, keep.intercept = FALSE)
   }
 }
  }
 ...
 if (has_pterms(x[[ntd[i]]]$terms)) {
   x[[ntd[i]]]$smooth.construct$model.matrix <- param_time_transform(x[[ntd[i]]]$smooth.construct$model.matrix, drop.terms.bamlss(x[[ntd[i]]]$terms, sterms = FALSE, keep.response = FALSE), data, grid, yname, timevar, take)
 }
 if (length(x[[ntd[i]]]$smooth.construct)) {
  ...
  x[[ntd[i]]]$smooth.construct[[j]] <- sm_time_transform(x[[ntd[i]]]$smooth.construct[[j]], data[, unique(c(xterm, yname, by, timevar, idvar)), drop = FALSE], grid, yname, timevar, take)
  }
   }
 }
  }
...
param_time_transform <- function(x, formula, data, grid, yname, timevar, take, derivMat = FALSE, eps = 1e-07) {
  if (timevar != yname) {
...
sm_time_transform <- function(x, data, grid, yname, timevar, take, derivMat = FALSE, eps = 1e-07) {
  if (derivMat) 
 ddata <- data
  if (!is.null(take)) 
 data <- data[take, , drop = FALSE]
...
cox_predict <- function(object, newdata, type = c("link", "parameter", "probabilities"), FUN = function(x) {
  mean(x, na.rm = TRUE)
}, time = NULL, subdivisions = 100, cores = NULL, chunks = 1, verbose = FALSE, ...) {
  if (is.null(newdata)) 
 ...
 width <- sapply(timegrid, function(x) {
   x[2]
 })
 pred.setup <- predict.bamlss(object, data, type = "link", get.bamlss.predict.setup = TRUE, ...)
 enames <- pred.setup$enames
 pred_tc <- with(pred.setup, .predict.bamlss("gamma", object$x$gamma, samps, enames$gamma, intercept, nsamps, data))
 pred_tv <- with(pred.setup, .predict.bamlss.surv.td("lambda", object$x$lambda$smooth.construct, samps, enames$lambda, intercept, nsamps, data, yname, timegrid, drop.terms.bamlss(object$x$lambda$terms, sterms = FALSE, keep.response = FALSE)))
 probs <- NULL
...
sm_Xtimegrid <- function(x, data, grid, yname, derivMat = FALSE) {
  ff <- sm_time_transform(x, data, grid, yname, timevar = yname, take = NULL, derivMat = derivMat)$fit.fun_timegrid
  ff(NULL)
}
param_Xtimegrid <- function(formula, data, grid, yname, type = 1, derivMat = FALSE) {
  ff <- if (type < 2) {
 param_time_transform(list(), formula, data, grid, yname, timevar = yname, take = NULL, derivMat = derivMat)$fit.fun_timegrid
  }
  else {
 param_time_transform2(list(), formula, data, grid, yname, timevar = yname, take = NULL, derivMat = derivMat)$fit.fun_timegrid
  }
  ff(NULL)
}
.predict.bamlss.surv.td <- function(id, x, samps, enames, intercept, nsamps, newdata, yname, grid, formula, type = 1, derivMat = FALSE) {
 ...
 for (j in enames2[i]) {
  ...
  if (has_intercept) {
  sn <- snames[grep2(paste(id, "p", j, sep = "."), snames, fixed = TRUE)]
  eta <- eta + as.numeric(fitted_matrix(matrix(1, nrow = length(grid[[1]]) * nrow(newdata), ncol = 1), samps[, sn, drop = FALSE]))
  }
   }
 }
  }
