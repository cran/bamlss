...
split_t2_smooth <- function(object) {
 ...
 sm[[i]] <- list(S = list(object$S[[i]][indi, indi]), first.para = min(ind.para[indi]), last.para = max(ind.para[indi]), fx = object$fx[i], fixed = object$fx[i], sp = object$sp[i], null.space.dim = 0, df = length(indi), rank = object$rank[i], label = label, S.scale = object$S.scale[i])
 class(sm[[i]]) <- "t2.frag"
 St <- St + object$S[[i]]
  }
  i <- length(object$S) + 1
  ...
  if (length(indi)) {
 label <- paste(object$label, ".frag", i, sep = "")
 sm[[i]] <- list(S = NULL, first.para = min(ind.para[indi]), last.para = max(ind.para[indi]), fx = TRUE, fixed = TRUE, null.space.dim = 0, label = label, df = length(indi))
 class(sm[[i]]) <- "t2.frag"
  }
  sm
}
...
smooth2random.t2.smooth <- function(object, vnames, type = 1) {
 ...
 Xf <- object$X[, fixed, drop = FALSE]
  ...
  list(rand = random, trans.D = diagU, Xf = Xf, fixed = FALSE, rind = 1:n.para, rinc = rep(n.para, n.para), pen.ind = pen.ind)
}
smooth2random.mgcv.smooth <- function(object, vnames, type = 1) {
 ...
 attr(random[[1]], "s.label") <- object$label
  ...
  list(rand = random, Xf = Xf, trans.U = U, trans.D = D, fixed = FALSE, rind = rind, rinc = rinc, pen.ind = pen.ind)
}
smooth2random.tensor.smooth <- function(object, vnames, type = 1) {
 ...
 object$S[[l]] <- (object$S[[l]] + t(object$S[[l]]))/2
  ...
  rind <- 1:p.rank
  rinc <- rep(p.rank, p.rank)
  list(rand = random, Xf = Xf, trans.U = U, trans.D = rep(1, ncol(U)), fixed = FALSE, rind = rind, rinc = rinc)
}
...
plot.density2 <- function(x, main = NULL, xlab = NULL, ylab = "Density", type = "l", zero.line = TRUE, ...) {
  if (is.null(xlab)) 
