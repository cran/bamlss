BayesX.control <- function(n.iter = 1200, thin = 1, burnin = 200, seed = NULL, predict = "light", model.name = "bamlss", data.name = "d", prg.name = NULL, dir = NULL, verbose = FALSE, show.prg = TRUE, modeonly = FALSE, ...) {
  if (is.null(seed)) 
 seed <- "##seed##"
 ...
 dir.create(dir)
  cores <- 1
  cvals <- list(prg = list(iterations = n.iter, burnin = burnin, step = thin, setseed = seed, predict = predict, modeonly = modeonly), setup = list(main = c(rep(FALSE, 3), rep(TRUE, 2)), model.name = model.name, data.name = data.name, prg.name = prg.name, dir = dir, verbose = verbose, show.prg = show.prg, cores = cores))
  cvals
}
sam_BayesX <- BayesX <- function(x, y, family, start = NULL, weights = NULL, offset = NULL, data = NULL, control = BayesX.control(...), ...) {
 ...
 if (ncol(y) < 2) {
  ...
  cny <- colnames(y)
  for (j in seq_along(cny)) cny[j] <- all.vars(as.formula(paste("~", cny[j])))
  colnames(y) <- cny
  yname <- colnames(y)[1]
  for (j in names(y)) {
 if (is.factor(y[[j]])) {
   if (nlevels(y[[j]]) < 3) {
  y[[j]] <- as.integer(as.integer(y[[j]]) - 1)
   }
 }
  }
  is.user <- function(x) {
 ...
 if (!is.null(x$smooth.construct)) {
  ...
  if (is.factor(data[[x$smooth.construct[[j]]$by]])) {
    ...
    for (tlj in colnames(mm)) sdata[[tlj]] <- mm[, tlj]
  }
  else {
 ...
 if (!is.null(sdata)) {
  ...
  sdata[[j]] <- as.integer(as.character(sdata[[j]]))
   ...
   if (nrow(sdata) == (if (is.null(dim(y))) 
  length(y)
   else nrow(y))) 
  sdata <- cbind(sdata, y)
   rval$dname <- paste(paste(id, collapse = "_"), data.name, sep = "_")
   write.table(sdata, file = file.path(dir, paste(rval$dname, ".raw", sep = "")), quote = FALSE, row.names = FALSE)
   rval$prgex <- c(paste("dataset", rval$dname), paste(rval$dname, ".infile using ", file.path(dir, paste(rval$dname, ".raw", sep = "")), sep = ""), rval$prgex)
 }
 rval
  }
  eqn <- list()
  prgex <- NULL
  n <- 1
  main <- if (is.null(family$bayesx$main)) 
 c(TRUE, rep(FALSE, length(x) - 1))
  else family$bayesx$main
 ...
 if (!all(c("fake.formula", "formula") %in% names(x[[i]]))) {
  ...
  teqn <- paste(model.name, ".hregress ", msp$eqn, ", family=", if (k < 2) 
  fbx[[i]][1]
  else "gaussian_re", " equationtype=", fbx[[i]][2], if (n == length(x) & k < 2) {
  paste(" ", paste(names(control$prg), "=", control$prg, sep = "", collapse = " "))
  }
  else NULL, if (main[n]) {
  paste(" predict=", pcmd, sep = "")
  }
  else NULL, if (!is.null(msp$dname)) 
  paste(" using", msp$dname)
  else NULL, sep = "")
  eqn[[i]][[j]] <- teqn
  ...
  k <- k + 1
   ...
   teqn <- paste(model.name, ".hregress ", msp$eqn, ", family=", fbx[[i]][1], if (!is.null(nrcat)) 
  paste0(if (n == 1) 
  " hlevel=1 "
  else NULL, " nrcat=", nrcat)
   else NULL, " equationtype=", if (!main[n]) 
  fbx[[i]][length(fbx[[i]])]
   else fbx[[i]][2], if (n == length(x)) {
  paste(paste(" ", paste(names(control$prg), "=", control$prg, sep = "", collapse = " ")), if (modeonly) 
  " modeonly"
  else "", sep = "")
   }
   else NULL, if (main[n]) {
 ...
 n <- n + 1
  ...
  for (i in unlist(rev(eqn))) prg <- c(prg, i, "")
  if (!modeonly) 
 prg <- c(prg, paste(model.name, "getsample", sep = "."))
  prg <- c(paste("%% BayesX program created by bamlss: ", as.character(Sys.time()), sep = ""), paste("%% usefile ", file.path(dir, prg.name), sep = ""), "", prg)
  prg <- unique(prg)
  if (any(grepl("##seed##", prg, fixed = TRUE))) 
 prg <- gsub("##seed##", round(runif(1) * .Machine$integer.max), prg, fixed = TRUE)
 ...
 writeLines(prg)
  ...
  warn <- getOption("warn")
  options(warn = -1)
  cmd <- paste0("Bayes", "X", "src", ":", ":", "run.bayesx(prg = prgf, verbose = control$setup$verbose)")
 ...
 if (!all(c("fake.formula", "formula") %in% names(x[[i]]))) {
  ...
  if ((length(tl) > 1) & is.user(x[[i]]$smooth.construct[[j]]) & !is.tx(x[[i]]$smooth.construct[[j]])) 
    tl <- paste(tl, collapse = "")
  term <- paste(if (is.user(x[[i]]$smooth.construct[[j]])) 
    "of"
  else NULL, "_", paste(tl, collapse = "_"), "_", "sample", sep = "")
  ...
  if (length(tj)) 
    sf <- if (is.tx(x[[i]]$smooth.construct[[j]]) & (length(x[[i]]$smooth.construct[[j]]$term) > 1)) 
    sf[tj]
    else sf[-tj]
  if ((x[[i]]$smooth.construct[[j]]$by != "NA") & !is.user(x[[i]]$smooth.construct[[j]])) 
  ...
  if (length(cn) != ncol(samps)) {
    ...
    warning(paste("number of returned parameters from BayesX is different from number of columns in the design matrix for term ", names(x[[i]]$smooth.construct)[j], "!", sep = ""))
  }
...
sx <- function(x, z = NULL, bs = "ps", by = NA, ...) {
  ...
  call <- match.call()
  available.terms <- c("rw1", "rw2", "season", "ps", "psplinerw1", "psplinerw2", "pspline", "te", "pspline2dimrw2", "te1", "pspline2dimrw1", "kr", "kriging", "gk", "geokriging", "gs", "geospline", "mrf", "spatial", "bl", "baseline", "factor", "ridge", "lasso", "nigmix", "re", "ra", "random", "cs", "catspecific", "offset", "generic", "rps", "hrandom_pspline")
  if (!bs %in% available.terms) 
 stop(paste("basis type", sQuote(bs), "not supported by BayesX"))
 ...
 if (length(grep("~", term <- deparse(call$x))) && bs %in% c("re", "ra", "random")) {
  ...
  xt <- xt$xt
   warn <- getOption("warn")
   options(warn = -1)
   if (by != "NA" && is.vector(by) && length(by) < 2 && !is.na(as.numeric(by))) {
  xt["b"] <- by
  by <- "NA"
   }
   options(warn = warn)
   if (bs %in% c("pspline2dimrw1", "pspline2dimrw2", "te", "gs", "geospline", "kr", "gk", "kriging", "geokriging")) {
  if (by != "NA") 
  ...
  bs <- "pspline2dimrw1"
   if (!is.null(xt$knots)) 
  xt$nrknots <- xt$knots
   if (bs %in% c("ps", "te", "psplinerw1", "psplinerw2", "pspline", "pspline2dimrw2", "pspline2dimrw1", "gs", "geospline")) {
  if (!is.null(xt$degree)) 
  m <- xt$degree
  if (!is.null(xt$order)) {
  if (is.na(m)) 
    m <- c(3, xt$order)
  else m <- c(m[1], xt$order)
  }
  if (length(m) < 2 && (bs %in% c("gs", "geospline"))) 
  m <- c(3, 1)
  if (length(m) < 2 && is.na(m)) 
  m <- c(3, 2)
  if (is.null(xt$order) && length(m) < 2) 
  m <- c(m, 2)
  if (is.null(xt$order) && length(m) < 2) 
  m <- c(m, 1)
  m[1] <- m[1] - 1
  ...
  rval$margin[[2]]$term <- term[1]
   ...
   rval$label <- paste("sx(", paste(term, collapse = ",", sep = ""), if (by != "NA") 
  paste(",by=", by, sep = "")
   else NULL, ")", sep = "")
 }
  }
 ...
 rval$xt$theta <- switch(rval$xt$prior, sd = 0.00877812, hc = 0.01034553, hn = 0.1457644, u = 0.2723532)
  }
  if (is.null(rval$xt$scaletau2)) 
 rval$xt$scaletau2 <- rval$xt$theta
  if (is.null(rval$xt$hyperprior)) {
 rval$xt$hyperprior <- switch(rval$xt$prior, ig = "invgamma", hn = "hnormal", sd = "scaledep", hc = "hcauchy", u = "aunif")
  }
  rval$xt$prior <- NULL
  rval$xt$theta <- NULL
  rval$special <- TRUE
...
sx.construct.default <- function(object, data, ...) {
 ...
 info <- if (cl == paste(bs, "smooth.spec", sep = ".")) {
  ...
  stop(paste("BayesX does not support smooth terms ", info, ", it is recommended to use sx() for specifying smooth terms", sep = ""))
}
do.xt <- function(term, object, not = NULL, noco = FALSE) {
 ...
 for (name in names.xt) {
  ...
  co <- ","
   if (!name %in% not) {
  if (name %in% c("full", "catspecific", "center", "derivative", "nofixed") || is.logical(object$xt[[name]])) {
...
sx.construct.userdefined.smooth.spec <- sx.construct.tensorX.smooth <- function(object, data, id = NULL, dir = NULL, ...) {
 ...
 object$xt$prior <- switch(object$xt$hyperprior, invgamma = "ig", hnormal = "hn", scaledep = "sd", hcauchy = "hc", aunif = "u")
  }
  if (is.null(object$xt$scaletau2)) {
 if (is.null(object$xt$theta)) {
   object$xt$theta <- switch(object$xt$prior, sd = 0.00877812, hc = 0.01034553, hn = 0.1457644, u = 0.2723532)
 }
  }
  else {
 object$xt$theta <- object$xt$scaletau2
  ...
  object$xt$hyperprior <- switch(object$xt$prior, ig = "invgamma", hn = "hnormal", sd = "scaledep", hc = "hcauchy", u = "aunif")
 ...
 S <- 0
 for (j in seq_along(object$S)) S <- S + object$S[[j]]
 object$S <- list(S)
  }
 ...
 paste(if (is.null(object$tx.term)) 
   object$term
 else object$tx.term, collapse = if (!is.tx) 
   ""
 else "*")
  }
  else object$term
 ...
 object$rank <- sapply(object$S, function(x) {
   qr(x)$rank
 })
 ...
 object$xt$nocenter <- NULL
  ...
  term <- paste(term, if (is.tx & (length(object$S) > 1)) 
 "(tensor,"
  else "(userdefined,", sep = "")
  for (j in seq_along(object$S)) term <- paste(term, paste("penmatdata", if (j < 2) 
 ""
  else j, "=", sep = ""), Sn[j], ",", sep = "")
 ...
 for (j in seq_along(object$S)) {
   term <- paste(term, paste("designmatdata", if (j < 2) 
  ""
   else j, "=", sep = ""), Xn[j], if (j == length(object$S)) {
  if (noc_and_cm) 
  ","
  else ""
   }
 ...
 term <- paste(term, "designmatdata=", Xn, if (noc_and_cm) 
   ","
 else "", sep = "")
  }
  if (!is.null(object$C)) 
 ...
 term <- paste(term, ",rankK=", sum(object$rank), sep = "")
  term <- paste(do.xt(term, object, c("center", "before", "penalty", "polys", "map", "map.name", "nb", "gra", "ft", "prior", "theta", "pmean", "pSigma", "doC", "constraint", "binning")), ")", sep = "")
  write <- function(dir) {
 exists <- NULL
 for (j in seq_along(object$S)) {
   if (!file.exists(file.path(dir, paste(Sn[j], ".raw", sep = "")))) {
  colnames(object$S[[j]]) <- rownames(object$S[[j]]) <- NULL
  write.table(round(object$S[[j]], 5), file = file.path(dir, paste(Sn[j], ".raw", sep = "")), quote = FALSE, row.names = FALSE)
   }
 ...
 if (is.tx) {
  ...
  write.table(round(object$margin[[j]]$X, 5), file = file.path(dir, paste(Xn[j], ".raw", sep = "")), quote = FALSE, row.names = FALSE)
  }
  ...
  write.table(round(object$X, 5), file = file.path(dir, paste(Xn, ".raw", sep = "")), quote = FALSE, row.names = FALSE)
   }
   else exists <- c(exists, file.path(dir, paste(Xn, ".raw", sep = "")))
 }
 if (!is.null(object$C)) {
   if (!file.exists(file.path(dir, paste(Cn, ".raw", sep = "")))) {
  write.table(object$C, file = file.path(dir, paste(Cn, ".raw", sep = "")), quote = FALSE, row.names = FALSE)
   }
 ...
 if (!is.null(object$state$parameters)) {
  ...
  write.table(spar, file = file.path(dir, paste(Pn, ".raw", sep = "")), quote = FALSE, row.names = FALSE)
   }
 ...
 if (!is.null(object$xt[["pmean"]])) {
  ...
  write.table(spar, file = file.path(dir, paste(Pm, ".raw", sep = "")), quote = FALSE, row.names = FALSE)
   }
 ...
 if (is.tx) {
  ...
  cmd <- c(cmd, paste("dataset", Sn[j]), paste(Sn[j], ".infile using ", file.path(dir, paste(Sn[j], ".raw", sep = "")), sep = ""))
  }
  if (!(file.path(dir, paste(Xn[j], ".raw", sep = "")) %in% exists)) {
  cmd <- c(cmd, paste("dataset", Xn[j]), paste(Xn[j], ".infile using ", file.path(dir, paste(Xn[j], ".raw", sep = "")), sep = ""))
  }
   }
 }
 else {
   if (!(file.path(dir, paste(Sn, ".raw", sep = "")) %in% exists)) {
  cmd <- c(cmd, paste("dataset", Sn), paste(Sn, ".infile using ", file.path(dir, paste(Sn, ".raw", sep = "")), sep = ""))
   }
   if (!(file.path(dir, paste(Xn, ".raw", sep = "")) %in% exists)) {
  cmd <- c(cmd, paste("dataset", Xn), paste(Xn, ".infile using ", file.path(dir, paste(Xn, ".raw", sep = "")), sep = ""))
   }
 }
 if (!is.null(object$C)) {
   if (!(file.path(dir, paste(Cn, ".raw", sep = "")) %in% exists)) {
  cmd <- c(cmd, paste("dataset", Cn), paste(Cn, ".infile using ", file.path(dir, paste(Cn, ".raw", sep = "")), sep = ""))
   }
 }
 if (!is.null(object$state$parameters)) {
   if (!(file.path(dir, paste(Pn, ".raw", sep = "")) %in% exists)) {
  cmd <- c(cmd, paste("dataset", Pn), paste(Pn, ".infile using ", file.path(dir, paste(Pn, ".raw", sep = "")), sep = ""))
   }
 }
 if (!is.null(object$xt[["pmean"]])) {
   if (!(file.path(dir, paste(Pm, ".raw", sep = "")) %in% exists)) {
  cmd <- c(cmd, paste("dataset", Pm), paste(Pm, ".infile using ", file.path(dir, paste(Pm, ".raw", sep = "")), sep = ""))
   }
 }
 return(cmd)
  }
...
sx.construct.pspline.smooth <- sx.construct.ps.smooth.spec <- sx.construct.psplinerw1.smooth.spec <- sx.construct.psplinerw2.smooth.spec <- sx.construct.pspline.smooth.spec <- function(object, data, mcmcreg = FALSE, ...) {
 ...
 if (object$p.order[2] > 2) {
  ...
  if (nrknots < 5) {
 warning("number of inner knots smaller than 5 not supported by BayesX, set to 5!", call. = FALSE)
 nrknots <- 5
  }
  termo <- object$term
  term <- if (mcmcreg) {
 paste(termo, "(pspline,nrknots=", nrknots, ",degree=", object$p.order[1], ",difforder=", object$p.order[2], sep = "")
  }
  else {
 paste(termo, "(psplinerw", object$p.order[2], ",nrknots=", nrknots, ",degree=", object$p.order[1], sep = "")
  }
...
sx.construct.tensor.smooth <- sx.construct.tensor.smooth.spec <- sx.construct.t2.smooth.spec <- function(object, data, dir, prg, mcmcreg = FALSE, ...) {
  by <- object$term[1]
  term <- object$term[2]
  object <- object$margin[[1]]
...
sx.construct.ra.smooth.spec <- sx.construct.re.smooth.spec <- sx.construct.random.smooth.spec <- sx.construct.random.effect <- function(object, data, mcmcreg = FALSE, ...) {
  term <- object$term
  if (is.null(object$ins)) 
 term <- paste(term, "(random", sep = "")
  else term <- paste(term, "(hrandom", sep = "")
...
sx.construct.kr.smooth.spec <- sx.construct.kriging.smooth.spec <- function(object, data, ...) {
 ...
 object$bs.dim <- 10
  ...
  xt <- object$xt
  if (is.null(xt$full)) 
 term <- paste(termo[1], "*", termo[2], "(kriging,nrknots=", nrknots, sep = "")
  else {
 term <- paste(termo[1], "*", termo[2], "(kriging,full", sep = "")
 object$xt$full <- NULL
  }
  term <- paste(do.xt(term, object, NULL), ")", sep = "")
  if (object$by != "NA") 
...
sx.construct.mrf.smooth <- sx.construct.mrf.smooth.spec <- sx.construct.spatial.smooth.spec <- function(object, data, ...) {
 ...
 stop("need to supply a map object in argument xt!")
  map.name <- help.map.name(deparse(substitute(object, env = .GlobalEnv), backtick = TRUE, width.cutoff = 500))
 ...
 if (!is.list(object$xt[[1]])) {
   if (inherits(object$xt[[1]], "gra")) 
  map <- object$xt[[1]]
   else map <- object$xt
 }
 else map <- object$xt[[1]]
 if (is.null(map)) {
   map <- object$xt
 ...
 if (inherits(map, "bnd")) {
  ...
  fext <- ifelse(pos > -1, substring(map, pos + 1), "")
  if (fext == "gra") 
  prg <- c(prg, paste(map.name, ".infile, graph using ", path.expand(map), sep = ""))
  else prg <- c(prg, paste(map.name, ".infile using ", path.expand(map), sep = ""))
   }
 }
 prg
  }
...
make_by <- function(term, object, data) {
 ...
 if (is.factor(by) && nlevels(by) > 1) {
   ...
   if ((k <- length(term)) > 1) 
  for (j in 1:k) if (!grepl("center", term[j]) && is.null(object$xt$center)) 
...
help.map.name <- function(x) {
 ...
 return("")
  x <- splitme(x)
  if (resplit(x[1:2]) == "s(") {
 x <- resplit(c("sfoofun", x[2:length(x)]))
...
sfoofun <- function(x, xt = NULL, ...) {
 ...
 return(NULL)
  x <- rval <- deparse(substitute(xt), backtick = TRUE, width.cutoff = 500)
  if (is.list(xt) && length(xt) > 1) 
 for (i in 1:length(xt)) if (inherits(xt[[i]], "bnd") || inherits(xt[[i]], "gra") || inherits(xt[[i]], "list")) {
   rval <- strsplit(x, ",", " ")[[1]]
   if (length(rval) > 1) 
  rval <- rval[i]
 }
  rval <- splitme(rval)
  if (length(rval) > 5) 
 if (resplit(rval[1:5]) == "list(") 
   rval <- rval[6:length(rval)]
  if (rval[length(rval)] == ")") 
 rval <- rval[1:(length(rval) - 1)]
  if (any(grepl("=", rval))) 
 rval <- rval[(grep("=", rval) + 2):length(rval)]
  rval <- resplit(rval)
  return(rval)
}
splitme <- function(x) {
...
te2 <- function(..., k = NA, bs = "cr", m = NA, d = NA, by = NA, fx = FALSE, mp = TRUE, np = TRUE, xt = NULL, id = NULL, sp = NULL, pc = NULL) {
 ...
 for (i in 2:dim) term[i] <- deparse(vars[[i]], backtick = TRUE)
  for (i in 1:dim) term[i] <- attr(terms(stats::reformulate(term[i])), "term.labels")
 ...
 stxt <- paste(stxt, "k=", deparse(k[i], backtick = TRUE), ",bs=", deparse(bs[i], backtick = TRUE), ",m=", deparse(m[[i]], backtick = TRUE), ",xt=xt1", ")")
 ...
 id <- as.character(id)
  ...
  ret <- list(margin = margin, term = term, by = by.var, fx = fx, label = label, dim = dim, mp = mp, np = np, id = id, sp = sp, inter = FALSE)
  if (!is.null(pc)) {
 if (length(pc) < d) 
...
tx <- function(..., bs = "ps", k = -1, ctr = c("center", "main", "both", "both1", "both2", "none", "meanf", "meanfd", "meansimple", "nullspace"), xt = NULL, special = TRUE) {
  if (length(k) < 2) {
 if (k < 0) 
   k <- 10
  }
...
tx3 <- function(..., bs = "ps", k = c(10, 5), ctr = c("main", "center"), xt = NULL, special = TRUE) {
 ...
 stop("3 variables are necessary for the space-time interaction term!")
  ...
  object <- list()
  object$margin <- list(eval(parse(text = m1)), eval(parse(text = m2)))
  object$term <- vars
  object$by <- "NA"
  object$fx <- FALSE
...
smooth.construct.tensorX3.smooth.spec <- function(object, data, knots, ...) {
  ...
  object$margin[[2]]$S <- list(kronecker(diag(1, ncol(object$margin[[2]]$margin[[1]]$S[[1]])), object$margin[[2]]$margin[[1]]$S[[1]]) + kronecker(object$margin[[2]]$margin[[2]]$S[[1]], diag(1, ncol(object$margin[[2]]$margin[[2]]$S[[1]]))))
 ...
 if (object$constraint == "none") 
   object$xt$nocenter <- TRUE
 else object$xt$centermethod <- object$constraint
  }
  else {
 ...
 if (object$xt[["ft"]]) {
  ...
  nraniso <- if (is.null(object$xt$nraniso)) 
  11
  else object$xt$nraniso
  minaniso <- if (is.null(object$xt$minaniso)) 
  0.05
  else object$xt$minaniso
  omegaseq <- seq(from = minaniso, to = 1 - minaniso, length = nraniso)
  omegaseq[5] <- 0.4099
  omegaprob <- rep(1/nraniso, nraniso)
  object$xt$theta <- try(hyperpar_mod2(object$X, object$margin[[1]]$S[[1]], object$margin[[2]]$S[[1]], A = object$C, c = 3, alpha = 0.1, omegaseq = omegaseq, omegaprob = omegaprob, R = 1000, type = toupper(object$xt$prior), lowrank = if (ncol(object$X) > 100) 
  TRUE
  else FALSE, k = min(c(50, ceiling(0.5 * ncol(object$X))))), silent = TRUE)
   }
   else {
  object$xt$theta <- try(hyperpar_mod2(object$X, object$S[[1]], NULL, A = object$C, c = 3, alpha = 0.1, omegaseq = 1, omegaprob = 1, R = 1000, type = toupper(object$xt$prior), lowrank = if (ncol(object$X) > 100) 
  TRUE
  else FALSE, k = min(c(50, ceiling(0.5 * ncol(object$X))))), silent = TRUE)
   }
  ...
  object$sx.S <- lapply(object$margin, function(x) {
 x$S[[1]]
  })
 ...
 object$sx.rank <- object$sx.rank - nrow(object$C)
  object$side.constrain <- if (object$special) 
 FALSE
  else TRUE
...
Predict.matrix.tensorX3.smooth <- function(object, data) {
  tensor.prod.model.matrix(list(Predict.matrix(object$margin[[1]], data), Predict.matrix(object$margin[[2]]$margin[[1]], data), Predict.matrix(object$margin[[2]]$margin[[2]], data)))
}
...
smooth.construct.tensorX.smooth.spec <- function(object, data, knots, ...) {
 ...
 stop("more than two variables in tx() currently not supported!")
  side.constrain <- if (object$special) 
 FALSE
  else TRUE
  object$np <- FALSE
  object$by.done <- TRUE
 ...
 object$label <- paste(object$label, object$by, sep = ":")
  ...
  ref <- sapply(object$margin, function(x) {
 ...
 if (length(object$margin) < 2) {
  ...
  object$C <- t(cbind(1, 1:p))
   ...
   p1 <- ncol(object$margin[[2]]$X)
   p2 <- ncol(object$margin[[1]]$X)
   I1 <- diag(p1)
   I2 <- diag(p2)
  ...
  A1 <- if (ref[1]) 
    rep(0, p1)
  else cbind(rep(1, p1), 1:p1)
  A2 <- if (ref[2]) 
    rep(0, p2)
  else cbind(rep(1, p2), 1:p2)
  }
 ...
 if (object$xt[["ft"]]) {
  ...
  omegaseq[5] <- 0.4099
  omegaprob <- rep(1/nraniso, nraniso)
  object$xt$theta <- try(hyperpar_mod2(unique(object$X), object$margin[[1]]$S[[1]], object$margin[[2]]$S[[1]], A = object$C, c = 3, alpha = 0.1, omegaseq = omegaseq, omegaprob = omegaprob, R = 1000, type = toupper(object$xt$prior), lowrank = if (ncol(object$X) > 100) 
  TRUE
  else FALSE, k = min(c(50, ceiling(0.5 * ncol(object$X))))), silent = TRUE)
   }
   else {
  object$xt$theta <- try(hyperpar_mod2(unique(object$X), object$S[[1]], NULL, A = object$C, c = 3, alpha = 0.1, omegaseq = 1, omegaprob = 1, R = 1000, type = toupper(object$xt$prior), lowrank = if (ncol(object$X) > 100) 
  TRUE
  else FALSE, k = min(c(50, ceiling(0.5 * ncol(object$X))))), silent = TRUE)
   }
   if (inherits(object$xt$theta, "try-error")) {
...
Cmat <- function(x) {
 ...
 if (is.null(x$xt$constraint)) 
  ...
  ref <- sapply(x$margin, function(z) {
 ...
 p <- if (is.null(x$margin)) 
   ncol(x$X)
 else ncol(x$margin[[1]]$X)
 C <- matrix(1, ncol = p)
 if (x$xt$constraint == "main") 
   C <- t(cbind(1, 1:p))
  }
  else {
 p1 <- ncol(x$margin[[2]]$X)
 p2 <- ncol(x$margin[[1]]$X)
...
get_BayesXsrc <- function(dir = NULL, install = TRUE) {
 ...
 on.exit(unlink(dir))
  ...
  devel <- c("REPOS=http://svn.gwdg.de/svn/bayesx/trunk", "USER=guest", "PASSWD=guest", "DIRS=\"adaptiv alex andrea bib dag graph leyre mcmc psplines samson structadd\"", "FILES=\"export_type.h main.cpp values.h\"", "mkdir -p src/bayesxsrc", "cd src/bayesxsrc", "for i in $DIRS ; do", "  svn checkout --username \"${USER}\" --password \"${PASSWD}\" $REPOS/$i $i", "done", "for i in $FILES ; do", "  svn export --username \"${USER}\" --password \"${PASSWD}\" $REPOS/$i $i", "done", "cd ..", "cp dev-Makefile Makefile", 
 "cp dev-Makefile.win Makefile.win")
  writeLines(devel, file.path("BayesXsrc", "bootstrap-devel.sh"))
  sh <- c("cd BayesXsrc", "sh ./bootstrap-devel.sh", "cd ..", "R CMD build BayesXsrc", if (install) "R CMD INSTALL BayesXsrc_*.tar.gz" else NULL)
