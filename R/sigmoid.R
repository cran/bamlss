...
sigmoid.fit.w <- function(X, y, weights = NULL, j = 2, ...) {
 ...
 beta <- chol2inv(chol(XX + diag(0.001, 2))) %*% t(X * weights) %*% y
 fit <- drop(X %*% beta)
 sum((y2 - fit)^2)
  }
  ...
  X <- cbind(wL, wR)
  XX <- crossprod(X * weights)
  beta <- drop(chol2inv(chol(XX + diag(0.001, 2))) %*% t(X * weights) %*% y)
  fit <- drop(X %*% beta)
  rval <- list(fitted.values = fit, residuals = y - fit, coefficients = w, w = list(L = wL, R = wR))
...
sigmoid.fit <- function(X, y, weights = NULL, ...) {
 ...
 colSums(g * weights)
  ...
  w <- opt$par
  rval <- list(fitted.values = drop(w[1] + w[2]/(1 + exp(-(X %*% w[-c(1:2)])))), fit.fun = function(X) drop(w[1] + w[2]/(1 + exp(-(X %*% w[-c(1:2)])))), coefficients = w[-c(1:2)], weights = weights)
  rval$rank <- 2 + ncol(X)
  rval$residuals <- y - rval$fitted.values
  class(rval) <- "sigmoid"
  rval
...
build_net_w <- function(X, y, k = 10, weights = NULL, wts = NULL, maxit = 10, ...) {
  ...
  m <- nnet::nnet(X[, -1, drop = FALSE], y, weights = weights, linout = TRUE, maxit = maxit, size = k, Wts = wts, MaxNWts = 20000, trace = FALSE, decay = 1e-04)
...
extract_nnet_weights <- function(x) {
 ...
 x <- coef(x)
  nw <- nw0 <- names(x)
  nw0 <- sapply(strsplit(nw0, "->", fixed = TRUE), function(x) {
 x[2]
  })
  nw0 <- paste0("->", nw0)
  nw <- grep("->h", nw, fixed = TRUE, value = TRUE)
  nw <- sapply(strsplit(nw, "->", fixed = TRUE), function(x) {
 x[2]
  })
  nw <- unique(nw)
  wts <- NULL
