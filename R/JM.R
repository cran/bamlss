jm_bamlss <- function(...) {
  links = c(lambda = "log", gamma = "log", mu = "identity", sigma = "log", alpha = "identity", dalpha = "identity")
  rval <- list(family = "jm", names = c("lambda", "gamma", "mu", "sigma", "alpha", "dalpha"), links = links, transform = function(x, jm.start = NULL, timevar = NULL, idvar = NULL, init.models = FALSE, plot = FALSE, nonlinear = FALSE, edf_alt = FALSE, start_mu = NULL, k_mu = 6, ...) {
 rval <- jm_transform(x = x$x, y = x$y, terms = x$terms, knots = x$knots, formula = x$formula, family = x$family, data = x$model.frame, jm.start = jm.start, timevar = timevar, idvar = idvar, nonlinear = nonlinear, edf_alt = edf_alt, start_mu = start_mu, k_mu = k_mu, ...)
 if (init.models) {
   x2 <- rval$x[c("mu", "sigma")]
   for (i in names(x2)) {
  for (j in seq_along(x2[[i]]$smooth.construct)) x2[[i]]$smooth.construct[[j]]$update <- bfit_iwls
   }
   attr(x2, "bamlss.engine.setup") <- TRUE
   cat2("generating starting model for longitudinal part...\n")
   gpar <- opt_bfit(x = x2, y = rval$y[[1]][, "obs", drop = FALSE], family = gF2(gaussian), start = NULL, maxit = 100)$parameters
   if (plot) {
  ...
  plot(results.bamlss.default(b))
   ...
   spar <- opt_Cox(x = x2, y = list(y), family = gF2("cox"), start = NULL, maxit = 100)$parameters
   if (plot) {
 ...
 return(rval)
  }, optimizer = opt_JM, sampler = sam_JM, predict = jm_predict)
  class(rval) <- "family.bamlss"
  rval
}
jm_transform <- function(x, y, data, terms, knots, formula, family, subdivisions = 25, timedependent = c("lambda", "mu", "alpha", "dalpha"), timevar = NULL, idvar = NULL, alpha = .Machine$double.eps, mu = NULL, sigma = NULL, sparse = TRUE, nonlinear = FALSE, edf_alt = FALSE, start_mu = NULL, k_mu = 6, ...) {
  rn <- names(y)
  y0 <- y
  y <- y[[rn]]
  ntd <- timedependent
 ...
 data[[idvar]] <- did
  ...
  width <- rep(NA, nobs)
  for (i in 1:nobs) width[i] <- grid[[i]][2]
 ...
 if (!tp & !fac) {
   ...
   alpha_mu$C <- C
   x$alpha$smooth.construct <- smoothCon(alpha_mu, data = data, knots = list(mu = k), absorb.cons = TRUE, n = nrow(data), scale.penalty = FALSE)
 }
 if (fac) {
 ...
 if (tp) {
   ...
   f_alpha_mu <- paste0("te(mu, ", term, ", bs = c(\"", bs_mu, "\",\"", bs_alpha, "\"), k = c(", k_mu, ",", k_alpha, "))")
   alpha_mu <- eval(parse(text = f_alpha_mu))
   alpha_mu$C <- C
   x$alpha$smooth.construct <- smoothCon(alpha_mu, data = data, knots = list(mu = k), absorb.cons = FALSE, n = nrow(data), scale.penalty = FALSE)
 }
 ...
 if (nonlinear & (j == "alpha")) {
   x[[j]] <- design.construct(terms, data = data[take_last, , drop = FALSE], knots = list(mu = k), model.matrix = TRUE, smooth.construct = TRUE, model = j, scale.x = FALSE, absorb.cons = TRUE, C = C)[[j]]
 }
 else {
   x[[j]] <- design.construct(terms, data = data[take, , drop = FALSE], knots = knots, model.matrix = TRUE, smooth.construct = TRUE, model = j, scale.x = FALSE)[[j]]
 }
  }
  for (j in c("lambda", "gamma", "alpha", if (dalpha) "dalpha" else NULL)) {
 ...
 x$dmu <- design.construct(terms, data = data, knots = knots, model.matrix = TRUE, smooth.construct = TRUE, model = "mu", scale.x = FALSE)$mu
 ...
 if (!is.null(x[[j]]$smooth.construct$model.matrix)) {
  ...
  x[[j]]$smooth.construct$model.matrix$term <- gsub("(Intercept)+", "", x[[j]]$smooth.construct$model.matrix$term, fixed = TRUE)
  x[[j]]$smooth.construct$model.matrix$state$parameters <- x[[j]]$smooth.construct$model.matrix$state$parameters[-1]
  x[[j]]$terms <- drop.terms.bamlss(x[[j]]$terms, keep.intercept = FALSE)
  x[[j]]$smooth.construct$model.matrix$label <- gsub("(Intercept)+", "", x[[j]]$smooth.construct$model.matrix$label, fixed = TRUE)
  x[[j]]$smooth.construct$model.matrix$bs.dim <- as.integer(x[[j]]$smooth.construct$model.matrix$bs.dim - 1)
  pid <- !grepl("tau", names(x[[j]]$smooth.construct$model.matrix$state$parameters)) & !grepl("edf", names(x[[j]]$smooth.construct$model.matrix$state$parameters))
 ...
 if (has_pterms(x[[ntd[i]]]$terms)) {
  ...
  x[[ntd[i]]]$smooth.construct$model.matrix <- param_time_transform2(x[[ntd[i]]]$smooth.construct$model.matrix, drop.terms.bamlss(x[[ntd[i]]]$terms, sterms = FALSE, keep.response = FALSE), data, grid, yname, timevar_mu, take, timevar2 = timevar_mu, idvar = idvar, delete.intercept = TRUE)
   }
   else {
  x[[ntd[i]]]$smooth.construct$model.matrix <- param_time_transform(x[[ntd[i]]]$smooth.construct$model.matrix, drop.terms.bamlss(x[[ntd[i]]]$terms, sterms = FALSE, keep.response = FALSE), data, grid, yname, if (ntd[i] != "mu" & ntd[i] != "dmu") 
  timevar
  else timevar_mu, take, derivMat = (ntd[i] == "dmu"))
   }
 }
 if (length(x[[ntd[i]]]$smooth.construct)) {
  ...
  by <- if (x[[ntd[i]]]$smooth.construct[[j]]$by != "NA") 
    x[[ntd[i]]]$smooth.construct[[j]]$by
  else NULL
  x[[ntd[i]]]$smooth.construct[[j]] <- sm_time_transform2(x[[ntd[i]]]$smooth.construct[[j]], data[, unique(c(xterm, yname, by, timevar, timevar_mu, idvar)), drop = FALSE], grid, yname, if (ntd[i] != "mu" & ntd[i] != "dmu") 
    timevar
  else timevar_mu, take_last, derivMat = (ntd[i] == "dmu"))
  }
   }
 }
  }
 ...
 by <- NULL
 x[["alpha"]]$smooth.construct[[1]]$margin[[1]] <- sm_time_transform2(x[["alpha"]]$smooth.construct[[1]]$margin[[1]], data[, unique(c(xterm, yname, by, timevar, timevar_mu, idvar)), drop = FALSE], grid, yname, timevar, take_last, derivMat = FALSE)
 xterm <- x[["alpha"]]$smooth.construct[[1]]$margin[[2]]$term
 by <- NULL
 x[["alpha"]]$smooth.construct[[1]]$margin[[2]] <- sm_time_transform2(x[["alpha"]]$smooth.construct[[1]]$margin[[2]], data[, unique(c(xterm, yname, by, timevar, timevar_mu, idvar)), drop = FALSE], grid, yname, timevar, take_last, derivMat = FALSE)
  }
 ...
 x$mu$smooth.construct$model.matrix$state$parameters[1] <- if (is.null(mu)) 
   mean(y[, "obs"], na.rm = TRUE)
 else mu
 ...
 x$sigma$smooth.construct$model.matrix$state$parameters[1] <- if (is.null(sigma)) 
   log(sd(y[, "obs"], na.rm = TRUE))
 else sigma
 ...
 logLik <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * y[attr(y, "take"), "status"], na.rm = TRUE) - exp(eta$gamma) %*% int + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
 if (logPost) 
...
sparse_Matrix_setup <- function(x, sparse = TRUE, force = FALSE, take, nonlinear) {
 ...
 for (j in seq_along(x$S)) x$S[[j]] <- Matrix(x$S[[j]], sparse = TRUE)
 if (nonlinear) {
...
opt_JM <- jm_mode <- function(x, y, start = NULL, weights = NULL, offset = NULL, criterion = c("AICc", "BIC", "AIC"), maxit = c(100, 1), nu = c(lambda = 0.1, gamma = 0.1, mu = 1, sigma = 1, alpha = 1, dalpha = 1), update.nu = FALSE, eps = 1e-04, alpha.eps = 0.001, ic.eps = 1e-08, nback = 40, verbose = TRUE, digits = 4, ...) {
  fix.lambda <- FALSE
  fix.gamma <- FALSE
  fix.alpha <- FALSE
  fix.mu <- FALSE
 ...
 for (j in names(x$alpha$smooth.construct)) {
  ...
  fit_timegrid <- matrix(drop(X %*% b), nrow = nrow(eta_timegrid_mu), ncol = ncol(eta_timegrid_mu), byrow = TRUE)
 ...
 eta_timegrid <- eta_timegrid_lambda + eta_timegrid_alpha * eta_timegrid_mu + eta_timegrid_dalpha * eta_timegrid_dmu
  ...
  knots <- if (nonlinear) {
 if (tp) {
   x$alpha$smooth.construct[[1]]$margin[[1]]$knots
 }
 else {
   x$alpha$smooth.construct[[1]]$knots
 }
  }
  else {
 NULL
 ...
 logLik_surv <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - drop(exp(eta$gamma)) %*% int0
 ...
 attr(logPost, "logLik") <- c(surv = logLik_surv, long = logLik_long, prio = log.priors)
 return(logPost)
  }
 ...
 eta0_alpha <- if (is.null(dalpha)) 
   eta$alpha
 else do.call("cbind", eta[c("alpha", "dalpha")])
 ...
 if (!fix.alpha & ok.alpha) {
  ...
  if (nonlinear) {
    state <- update_jm_alpha_nonlin(x$alpha$smooth.construct[[sj]], eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, eps0_alpha < edf.eps, edf = edf, edf_alt, tp = tp, ...)
  }
  else {
    state <- update_jm_alpha(x$alpha$smooth.construct[[sj]], eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, eps0_alpha < edf.eps, edf = edf, ...)
  }
  ...
  LogPrioUP <- LogPrioOld - x$alpha$smooth.construct[[sj]]$prior(x$alpha$smooth.construct[[sj]]$state$parameters) + x$alpha$smooth.construct[[sj]]$prior(state$parameters)
  ...
  if (myplots) {
    plot(eta_timegrid_mu, state$fitted_timegrid, main = paste0("alpha: iteration ", iter, ", edf ", round(state$edf, 2), ", term: ", names(x$alpha$smooth.construct)[sj]))
    abline(h = 0, col = 2)
  ...
  if (mydiag) {
    if (min(eta_timegrid_alpha) < -10 | max(eta_timegrid_alpha) > 10) 
    browser()
  }
  }
  if (maxit[2] > 1) {
  eps00 <- mean(abs((eta$alpha - eta00)/eta$alpha), na.rm = TRUE)
  if (is.na(eps00) | !is.finite(eps00)) 
    eps00 <- eps[2] + 1
  }
  iter00 <- iter00 + 1
   }
 ...
 if (!fix.dalpha & dalpha & ok.alpha) {
  ...
  state <- update_jm_dalpha(x$dalpha$smooth.construct[[sj]], eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, eps0_alpha < edf.eps, edf = edf, ...)
  ...
  LogPrioUP <- LogPrioOld - x$dalpha$smooth.construct[[sj]]$prior(x$dalpha$smooth.construct[[sj]]$state$parameters) + x$dalpha$smooth.construct[[sj]]$prior(state$parameters)
  LogPostUP <- get_LogPost(eta_timegridUP, etaUP, LogPrioUP)
  if (LogPostUP > LogPostOld || iter == 0) {
    LogPrioOld <- LogPrioUP
    LogPostOld <- LogPostUP
 ...
 while (eps00 > eps[2] & iter00 < maxit[2]) {
  ...
  state <- x$mu$smooth.construct[[sj]]$update(x$mu$smooth.construct[[sj]], y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, eps0_long < edf.eps, edf = edf, dx = if (dalpha) 
    x$dmu$smooth.construct[[sj]]
  else NULL, xsmalpha = if (nonlinear) 
    x$alpha$smooth.construct
  else NULL, knots = knots, tp = if (nonlinear) 
    tp
  else FALSE, fac = if (nonlinear) 
    fac
  else FALSE, ...)
  ...
  LogPrioUP <- LogPrioOld - x$mu$smooth.construct[[sj]]$prior(x$mu$smooth.construct[[sj]]$state$parameters) + x$mu$smooth.construct[[sj]]$prior(state$parameters)
  LogPostUP <- get_LogPost(eta_timegridUP, etaUP, LogPrioUP)
  if (LogPostUP > LogPostOld || iter == 0) {
    LogPrioOld <- LogPrioUP
    LogPostOld <- LogPostUP
  ...
  if (mydiag) {
    if (min(eta_timegrid_alpha) < -10 | max(eta_timegrid_alpha) > 10) 
    browser()
    if (min(eta_timegrid_mu) < -10 | max(eta_timegrid_alpha) > 20) 
    browser()
  }
  if (dalpha & (sj %in% names(x$dmu$smooth.construct))) {
    ...
    LogPrioUP <- LogPrioOld - x$sigma$smooth.construct[[sj]]$prior(x$sigma$smooth.construct[[sj]]$state$parameters) + x$sigma$smooth.construct[[sj]]$prior(state$parameters)
    LogPostUP <- get_LogPost(eta_timegridUP, etaUP, LogPrioUP)
    if (LogPostUP > LogPostOld || iter == 0) {
    LogPrioOld <- LogPrioUP
    LogPostOld <- LogPostUP
  ...
  state <- update_jm_sigma(x$sigma$smooth.construct[[sj]], y, eta, eta_timegrid, update.nu, criterion, get_LogPost, nobs, eps0_long < edf.eps, edf = edf, ...)
 ...
 while (eps00 > eps[2] & iter00 < maxit[2]) {
  ...
  state <- update_jm_lambda(x$lambda$smooth.construct[[sj]], eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, eps0_surv < edf.eps, edf = edf, nonlinear, ...)
  ...
  LogPrioUP <- LogPrioOld - x$lambda$smooth.construct[[sj]]$prior(x$lambda$smooth.construct[[sj]]$state$parameters) + x$lambda$smooth.construct[[sj]]$prior(state$parameters)
  LogPostUP <- get_LogPost(eta_timegridUP, etaUP, LogPrioUP)
  if (LogPostUP > LogPostOld || iter == 0) {
    LogPrioOld <- LogPrioUP
    LogPostOld <- LogPostUP
   ...
   if (mydiag) {
  if (min(eta_timegrid) < -100 | max(eta_timegrid) > 100) 
  browser()
   }
   if (!fix.gamma) {
  if (length(x$gamma$smooth.construct)) {
  ...
  for (sj in seq_along(x$gamma$smooth.construct)) {
    state <- update_jm_gamma(x$gamma$smooth.construct[[sj]], eta, eta_timegrid, int0, status, update.nu, criterion, get_LogPost, nobs, eps0_surv < edf.eps, edf = edf, ...)
    etaUP$gamma <- eta$gamma - fitted(x$gamma$smooth.construct[[sj]]$state) + fitted(state)
    LogPrioUP <- LogPrioOld - x$gamma$smooth.construct[[sj]]$prior(x$gamma$smooth.construct[[sj]]$state$parameters) + x$gamma$smooth.construct[[sj]]$prior(state$parameters)
    LogPostUP <- get_LogPost(eta_timegrid, etaUP, LogPrioUP)
    if (LogPostUP > LogPostOld || iter == 0) {
    LogPrioOld <- LogPrioUP
    LogPostOld <- LogPostUP
 ...
 eeta <- exp(eta_timegrid)
 int0 <- width * (0.5 * (eeta[, 1] + eeta[, sub]) + apply(eeta[, 2:(sub - 1)], 1, sum))
 logLik <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int0 + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
 ...
 if (verbose) {
  ...
  "\r"
   ...
   vtxt <- paste(criterion, " ", fmt(IC, width = 8, digits = digits), " logLik ", fmt(logLik, width = 8, digits = digits), " edf ", fmt(edf, width = 6, digits = digits), " eps ", paste(fmt(eps0, width = 6, digits = digits + 2), "|S", fmt(eps0_surv, width = 6, digits = digits + 2), "|L", fmt(eps0_long, width = 6, digits = digits + 2), "|A", fmt(eps0_alpha, width = 6, digits = digits + 2), sep = ""), " iteration ", formatC(iter, width = nchar(maxit[1])), sep = "")
   cat(vtxt)
 ...
 warning("the backfitting algorithm did not converge, please check argument eps and maxit!")
  return(list(fitted.values = eta, parameters = get.all.par(x), logLik = logLik, logPost = logPost, hessian = get.hessian(x), converged = iter < maxit[1], time = elapsed))
}
update_jm_gamma <- function(x, eta, eta_timegrid, int, status, update.nu, criterion, get_LogPost, nobs, do.optim2, edf, ...) {
  tint <- xhess0 <- 0
  for (i in 1:nobs) {
 ...
 objfun1 <- function(tau2) {
  ...
  par[x$pid$b] <- g2
  opt_state <- list(parameters = par, fitted.values = fit, edf = edf1, hessian = xhess, nu = nu, do.optim = x$state$do.optim)
...
update_jm_lambda <- function(x, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, nonlinear, ...) {
 ...
 objfun1 <- function(tau2) {
  ...
  eta_timegrid <- eta_timegrid_lambda + eta_timegrid_alpha * eta_timegrid_mu + eta_timegrid_dalpha * eta_timegrid_dmu
   ...
   if (!is.null(env$ic_val)) {
  if ((ic < env$ic_val) & (ic < env$ic00_val)) {
  opt_state <- list(parameters = set.par(par, g2, "b"), fitted.values = fit, fitted_timegrid = fit_timegrid, edf = edf1, hessian = xhess, nu = nu, do.optim = x$state$do.optim)
  assign("state", opt_state, envir = env)
  assign("ic_val", ic, envir = env)
  }
   }
...
update_jm_mu <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, dx = NULL, xsmalpha = NULL, knots = NULL, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
  eeta <- exp(eta_timegrid)
  int <- survint(X, eeta * eta_timegrid_alpha, width, exp(eta$gamma), eeta * eta_timegrid_alpha^2, index = x$sparse.setup[["mu.matrix"]], dX, if (!is.null(dX)) 
 eeta * eta_timegrid_dalpha
  else NULL, if (!is.null(dX)) 
 eeta * eta_timegrid_dalpha^2
  else NULL)
  xgrad <- drop(t(x$X) %*% drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2) + t(x$XT) %*% drop(eta$alpha * status) - int$grad)
  if (!is.null(dx)) 
 ...
 objfun1 <- function(tau2) {
  ...
  eta_timegrid_dmu <- eta_timegrid_dmu - dx$state$fitted_timegrid + dx$fit.fun_timegrid(g2)
   ...
   if (!is.null(env$ic_val)) {
  if ((ic < env$ic_val) & (ic < env$ic00_val)) {
  opt_state <- list(parameters = set.par(par, g2, "b"), fitted.values = fit, fitted_timegrid = fit_timegrid, edf = edf1, hessian = -1 * xhess, nu = nu, do.optim = x$state$do.optim)
  assign("state", opt_state, envir = env)
  assign("ic_val", ic, envir = env)
  }
   }
...
update_jm_mu_Matrix <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
 ...
 eeta * eta_timegrid_dalpha^2
  ...
  xgrad <- crossprod(x$X, drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2)) + crossprod(x$XT, drop(eta$alpha * status)) - int$grad
  if (!is.null(dx)) 
 ...
 objfun1 <- function(tau2) {
  ...
  if ((ic < env$ic_val) & (ic < env$ic00_val)) {
  par[x$pid$b] <- g2
  opt_state <- list(parameters = par, fitted.values = fit, fitted_timegrid = fit_timegrid, edf = edf1, hessian = -1 * xhess, nu = nu, do.optim = x$state$do.optim)
  assign("state", opt_state, envir = env)
  assign("ic_val", ic, envir = env)
  }
   }
...
update_jm_sigma <- function(x, y, eta, eta_timegrid, update.nu, criterion, get_LogPost, nobs, do.optim2, edf, ...) {
  xgrad <- crossprod(x$X, -1 + (y[, "obs"] - eta$mu)^2/exp(eta$sigma)^2)
  xhess0 <- -2 * crossprod(x$X * drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2), x$X * drop(y[, "obs"] - eta$mu))
  if ((!(!x$state$do.optim | x$fixed | x$fxsp)) & do.optim2) {
 par <- x$state$parameters
 edf0 <- if (is.null(edf)) {
   0
 ...
 objfun1 <- function(tau2) {
  ...
  if ((ic < env$ic_val) & (ic < env$ic00_val)) {
  par[x$pid$b] <- g2
  opt_state <- list(parameters = par, fitted.values = fit, edf = edf1, hessian = -1 * xhess, nu = nu, do.optim = x$state$do.optim)
  assign("state", opt_state, envir = env)
  assign("ic_val", ic, envir = env)
  }
   }
...
update_jm_alpha <- function(x, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, ...) {
  X <- x$fit.fun_timegrid(NULL)
  eeta <- exp(eta_timegrid)
  int <- survint(X, eeta * eta_timegrid_mu, width, exp(eta$gamma), eeta * (eta_timegrid_mu^2), index = x$sparse.setup$matrix)
...
update_jm_dalpha <- function(x, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, ...) {
  X <- x$fit.fun_timegrid(NULL)
  eeta <- exp(eta_timegrid)
  int <- survint(X, eeta * eta_timegrid_dmu, width, exp(eta$gamma), eeta * (eta_timegrid_dmu^2), index = x$sparse.setup$matrix)
...
sam_JM <- jm_mcmc <- function(x, y, family, start = NULL, weights = NULL, offset = NULL, n.iter = 1200, burnin = 200, thin = 1, verbose = TRUE, digits = 4, step = 20, ...) {
  fixed <- NULL
  slice <- NULL
  dalpha <- has_pterms(x$dalpha$terms) | (length(x$dalpha$smooth.construct) > 0)
  nu_surv <- 0.1
 ...
 for (j in names(x[[i]]$smooth.construct)) {
   samps[[i]][[j]] <- list(samples = matrix(NA, nrow = length(iterthin), ncol = length(x[[i]]$smooth.construct[[j]]$state$parameters)), edf = rep(NA, length = length(iterthin)), alpha = rep(NA, length = length(iterthin)), accepted = rep(NA, length = length(iterthin)))
 ...
 x
  ...
  int0 <- width * (0.5 * (eeta[, 1] + eeta[, sub]) + apply(eeta[, 2:(sub - 1)], 1, sum))
  if (is.null(slice)) {
 slice <- c(lambda = FALSE, gamma = FALSE, mu = FALSE, sigma = FALSE, alpha = FALSE, dmu = FALSE, dalpha = FALSE)
  }
  else {
 ...
 for (i in nx2) {
  ...
  p.state <- prop_fun(x[[i]]$smooth.construct[[sj]], y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id = i, int0, nobs, dx = if (dalpha & (i == "mu")) 
  x[["dmu"]]$smooth.construct[[sj]]
  else NULL, xsmalpha = if (nonlinear & (i == "mu")) 
  x$alpha$smooth.construct
  else NULL, knots = if (nonlinear & (i == "mu")) 
  knots
  else NULL, tp = tp, fac = fac, ...)
  accepted <- if (is.na(p.state$alpha)) 
  FALSE
  else log(runif(1)) <= p.state$alpha
  if (i %in% fixed) 
  accepted <- FALSE
  if (accepted) {
  ...
  if (i == "alpha") {
    ...
    plot(eta_timegrid_mu, p.state$fitted_timegrid, main = paste0("alpha: iteration ", iter, ", edf ", round(p.state$edf, 2), ", term: ", sj))
    abline(h = 0, col = 2)
    abline(v = median(y[, 3]), col = 2)
 ...
 if (save) {
   logLik.samps[js] <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int0 + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
 ...
 samps[[i]] <- do.call("cbind", samps[[i]])
  ...
  pd <- mean(dev) - mdev
  DIC <- mdev + 2 * pd
  samps <- cbind(samps, DIC = rep(DIC, length.out = nrow(samps)), pd = rep(pd, length.out = nrow(samps)))
...
uni.slice_beta_logPost <- function(g, x, family, y = NULL, eta = NULL, id, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, status, dx = NULL, nonlinear = FALSE, ...) {
 ...
 if (nonlinear) {
  ...
  ll <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
  lp <- x$prior(g)
  return(ll + lp)
}
...
propose_jm_slice <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, dx = NULL, nonlinear = FALSE, ...) {
  for (j in seq_along(get.par(x$state$parameters, "b"))) {
 x$state$parameters <- uni.slice(x$state$parameters, x, family, y, eta, id = id, j, logPost = uni.slice_beta_logPost, eta_timegrid = eta_timegrid, eta_timegrid_lambda = eta_timegrid_lambda, eta_timegrid_mu = eta_timegrid_mu, eta_timegrid_alpha = eta_timegrid_alpha, eta_timegrid_dmu = eta_timegrid_dmu, eta_timegrid_dalpha = eta_timegrid_dalpha, width = width, sub = sub, status = status, dx = dx, nonlinear = nonlinear)
  }
 ...
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   ...
   for (j in i) {
  x$state$parameters <- uni.slice(x$state$parameters, x, NULL, NULL, NULL, id = "mu", j, logPost = uni.slice_tau2_logPost, lower = 0, ll = 0)
   }
 }
  }
  x$state$alpha <- 0
...
propose_jm_lambda <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, ...) {
  if (!x$fixed & is.null(x$sp) & length(x$S)) {
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   g <- get.par(x$state$parameters, "b")
   a <- x$rank/2 + x$a
  ...
  eeta <- exp(eta_timegrid)
  int0 <- width * (0.5 * (eeta[, 1] + eeta[, sub]) + apply(eeta[, 2:(sub - 1)], 1, sum))
  pibeta <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int0 + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
 ...
 eta_timegrid <- eta_timegrid_lambda + eta_timegrid_alpha * eta_timegrid_mu + eta_timegrid_dalpha * eta_timegrid_dmu
  ...
  eeta <- exp(eta_timegrid)
  int0 <- width * (0.5 * (eeta[, 1] + eeta[, sub]) + apply(eeta[, 2:(sub - 1)], 1, sum))
  pibetaprop <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int0 + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
  ...
  x$state$newtraphs <- list(xgrad0 = xgrad0, xgrad1 = xgrad1, pgrad0 = pgrad0, pgrad1 = pgrad1, xhess0 = intOLD$hess, xhess1 = int$hess, phess0 = phess0, phess1 = phess1, g0 = g0, g1 = g, mu0 = mu, mu1 = mu2, pibetaprop = pibetaprop, pibeta = pibeta, qbetaprop = qbetaprop, qbeta = qbeta, p0 = p1, p1 = p2)
  return(x$state)
}
propose_jm_mu <- function(x, ...) {
  return(x$propose(x, ...))
}
propose_jm_mu_simple <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, dx = NULL, ...) {
  if (!x$fixed & is.null(x$sp) & length(x$S)) {
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   g <- get.par(x$state$parameters, "b")
   a <- x$rank/2 + x$a
 ...
 eeta * eta_timegrid_dalpha^2
  ...
  xgrad0 <- drop(t(x$X) %*% drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2) + t(x$XT) %*% drop(eta$alpha * status) - int$grad)
  if (!is.null(dx)) 
 ...
 eeta * eta_timegrid_dalpha^2
  ...
  xgrad1 <- drop(t(x$X) %*% drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2) + t(x$XT) %*% drop(eta$alpha * status) - int$grad)
  if (!is.null(dx)) 
 ...
 crossprod(x$X * (1/exp(eta$sigma)^2), x$X)
  ...
  x$state$alpha <- drop((pibetaprop + qbeta + p2) - (pibeta + qbetaprop + p1))
  x$state$newtraphs <- list(xgrad0 = xgrad0, xgrad1 = xgrad1, pgrad0 = pgrad0, pgrad1 = pgrad1, xhess0 = -xhess0, xhess1 = -xhess1, phess0 = phess0, phess1 = phess1, g0 = g0, g1 = g, mu0 = mu, mu1 = mu2, pibetaprop = pibetaprop, pibeta = pibeta, qbetaprop = qbetaprop, qbeta = qbeta, p0 = p1, p1 = p2)
  return(x$state)
}
propose_jm_mu_Matrix <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, dx = NULL, ...) {
  if (!is.null(x$xt$tau2)) {
 ...
 eeta * eta_timegrid_dalpha^2
  ...
  xgrad0 <- crossprod(x$X, drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2)) + crossprod(x$XT, drop(eta$alpha * status)) - int$grad
  if (!is.null(dx)) 
 ...
 eeta * eta_timegrid_dalpha^2
  ...
  xgrad1 <- crossprod(x$X, drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2)) + crossprod(x$XT, drop(eta$alpha * status)) - int$grad
  if (!is.null(dx)) 
 ...
 lqbeta <- lapply(1:length(x$sparse.setup$block.index), function(i) {
   ...
   if (x$sparse.setup$is.diagonal) {
  if (Sigma2[tmp, tmp] < 0) 
  Sigma2[tmp, tmp] <- -1 * Sigma2[tmp, tmp]
...
propose_jm_alpha <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, ...) {
  if (!x$fixed & is.null(x$sp) & length(x$S)) {
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   g <- get.par(x$state$parameters, "b")
   a <- x$rank/2 + x$a
   ...
   i <- grep("tau2", names(x$state$parameters))
   for (j in i) {
  x$state$parameters <- uni.slice(x$state$parameters, x, NULL, NULL, NULL, id = "alpha", j, logPost = uni.slice_tau2_logPost, lower = 0, ll = 0)
   }
 }
  }
  X <- x$fit.fun_timegrid(NULL)
  ...
  pibeta <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int0 + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
  p1 <- x$prior(x$state$parameters)
  intOLD <- survint(X, eeta * eta_timegrid_mu, width, exp(eta$gamma), eeta * eta_timegrid_mu^2, index = x$sparse.setup$matrix)
...
propose_jm_dalpha <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, ...) {
  if (!x$fixed & is.null(x$sp) & length(x$S)) {
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   g <- get.par(x$state$parameters, "b")
   a <- x$rank/2 + x$a
...
propose_jm_gamma <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, int, nobs, ...) {
  if (!x$fixed & is.null(x$sp) & length(x$S)) {
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   g <- get.par(x$state$parameters, "b")
   a <- x$rank/2 + x$a
   ...
   i <- grep("tau2", names(x$state$parameters))
   for (j in i) {
  x$state$parameters <- uni.slice(x$state$parameters, x, NULL, NULL, NULL, id = "gamma", j, logPost = uni.slice_tau2_logPost, lower = 0, ll = 0)
   }
 }
  }
  eeta <- exp(eta_timegrid)
  pibeta <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
  p1 <- x$prior(x$state$parameters)
  tint <- xhess0 <- 0
  for (i in 1:nobs) {
 tint <- tint + exp(eta$gamma[i]) * x$X[i, , drop = FALSE] * int[i]
 xhess0 <- xhess0 + exp(eta$gamma[i]) * x$X[i, ] %*% t(x$X[i, ]) * int[i]
  ...
  eta$gamma <- eta$gamma - fitted(x$state) + fit
  x$state$fitted.values <- fit
  pibetaprop <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta$gamma) * status, na.rm = TRUE) - exp(eta$gamma) %*% int + sum(dnorm(y[, "obs"], mean = eta$mu, sd = exp(eta$sigma), log = TRUE))
  tint <- xhess1 <- 0
  for (i in 1:nobs) {
 tint <- tint + exp(eta$gamma[i]) * x$X[i, , drop = FALSE] * int[i]
...
propose_jm_sigma <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, int, ...) {
  if (!x$fixed & is.null(x$sp) & length(x$S)) {
 if ((length(x$S) < 2) & (attr(x$prior, "var_prior") == "ig")) {
   g <- get.par(x$state$parameters, "b")
   a <- x$rank/2 + x$a
   ...
   i <- grep("tau2", names(x$state$parameters))
   for (j in i) {
  x$state$parameters <- uni.slice(x$state$parameters, x, NULL, NULL, NULL, id = "sigma", j, logPost = uni.slice_tau2_logPost, lower = 0, ll = 0)
   }
 }
  }
  eeta <- exp(eta_timegrid)
  ...
  pgrad1 <- x$grad(score = NULL, x$state$parameters, full = FALSE)
  xgrad <- xgrad1 + pgrad1
  xhess1 <- -2 * crossprod(x$X * drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2), x$X * drop(y[, "obs"] - eta$mu))
  phess1 <- x$hess(score = NULL, x$state$parameters, full = FALSE)
  xhess <- xhess1 - phess1
  Sigma2 <- matrix_inv(-1 * xhess, index = NULL)
  mu2 <- drop(g + nu * Sigma2 %*% xgrad)
...
smooth.construct.Re.smooth.spec <- function(object, data, knots, ...) {
  ...
  if (object$bs.dim < 0) 
 object$bs.dim <- 5
  xobj <- eval(as.call(c(as.symbol("s"), as.symbol(object$term[!isf]), k = object$bs.dim, xt = list(object$xt), bs = "ps")))
 ...
 object$X[[j]] <- xobj$X[id == xl[j], , drop = FALSE]
  ...
  object$zeros <- apply(object$X, 2, function(x) {
 all(x == 0)
  })
...
Predict.matrix.Random.effect <- function(object, data) {
  ...
  X <- list()
  xl <- levels(id)
  for (j in seq_along(xl)) X[[j]] <- Xd[id == xl[j], , drop = FALSE]
...
param_time_transform2 <- function(x, formula, data, grid, yname, timevar, take, derivMat = FALSE, eps = 1e-07, timevar2 = NULL, idvar = NULL, delete.intercept = FALSE) {
  if (derivMat) 
 ddata <- data
  id <- data[[idvar]]
  X <- Xn <- tvar <- NULL
 ...
 if ((!grepl("Surv(", j, fixed = TRUE) & !grepl("Surv2(", j, fixed = TRUE)) & (j != yname) & (j != timevar)) {
  ...
  length(unique(idata[[i]])) > 1
   ...
   if (any(unlist(temp))) {
  tvar <- c(tvar, j)
  values <- lapply(1:length(idata), function(i) {
  unique(idata[[i]])
  })
 ...
 data <- data[take, , drop = FALSE]
  for (j in names(data)) {
 if ((!grepl("Surv(", j, fixed = TRUE) & !grepl("Surv2(", j, fixed = TRUE)) & (j != yname) & (j != timevar) & !(j %in% tvar)) {
...
simJM <- function(nsub = 300, times = seq(0, 120, 1), probmiss = 0.75, long_setting = "functional", alpha_setting = if (nonlinear) "linear" else "nonlinear", dalpha_setting = "zero", sigma = 0.3, long_df = 6, tmax = NULL, seed = NULL, full = FALSE, file = NULL, nonlinear = FALSE, fac = FALSE) {
  if (is.null(tmax)) {
 tmax <- max(times)
  }
 ...
 b_set <- list(knots = attr(bt, "knots"), Boundary.knots = attr(bt, "Boundary.knots"), degree = attr(bt, "degree"), intercept = attr(bt, "intercept"))
 ...
 switch(long_setting, linear = (1.25 + r[, 1] + 0.6 * sin(x) + (-0.01) * time + r[, 2] * 0.02 * time), nonlinear = (0.5 + r[, 1] + 0.6 * sin(x) + 0.1 * (time + 2) * exp(-0.075 * time)), functional = (0.5 + r[, 1] + 0.6 * sin(x) + 0.1 * (time + 2) * exp(-0.075 * time) + apply(bs(time, long_df, b_set$knots, b_set$degree, b_set$intercept, b_set$Boundary.knots) * beta, 1, sum)))
  }
 ...
 switch(long_setting, simple = (-0.02) + 0 * time, linear = (-0.01 + r[, 2] * 0.02) + 0 * time, nonlinear = (0.085 - 0.0075 * time) * exp(-0.075 * time), functional = (0.085 - 0.0075 * time) * exp(-0.075 * time) + apply(dbs(time, long_df, b_set$knots, b_set$degree, b_set$intercept, b_set$Boundary.knots) * beta, 1, sum))
  }
  alpha <- function(time, alpha_setting) {
 switch(alpha_setting, zero = 0 * time, constant = 0 * time + 1, linear = 1 - 0.015 * time, nonlinear = cos((time - 60)/20) + 1, nonlinear2 = cos((time - 33)/33))
  }
  alpha_nonlin <- function(time, alpha_setting, x2, x3, r, long_df, b_set, long_setting) {
 switch(alpha_setting, zero = 0 * time, constant = 0 * time + 1, linear = 1 * mu(time, x2, r, long_df, b_set, long_setting), nonlinear = -0.1 * (mu(time, x2, r, long_df, b_set, long_setting) + 3)^2 + mu(time, x2, r, long_df, b_set, long_setting) + 1.8, nonlinear2 = x3 * (-0.1 * (mu(time, x2, r, long_df, b_set, long_setting) + 3)^2 + mu(time, x2, r, long_df, b_set, long_setting) + 1.8) + (1 - x3) * (0.1 * (mu(time, x2, r, long_df, b_set, long_setting) - 3)^2 + 0.75 * mu(time, x2, r, long_df, 
   b_set, long_setting) - 0.8))
  }
  alpha_nonlin_simple <- function(alpha_setting, x3, mu) {
 switch(alpha_setting, zero = 0, constant = 1, linear = 1 * mu, nonlinear = -0.1 * (mu + 3)^2 + mu + 1.8, nonlinear2 = x3 * (-0.1 * (mu + 3)^2 + mu + 1.8) + (1 - x3) * (0.1 * (mu - 3)^2 + 0.75 * mu - 0.8))
  }
  dalpha <- function(time, dalpha_setting) {
 switch(dalpha_setting, zero = 0 * time, constant = 0 * time + 10, linear = 6 - 0.015 * time, nonlinear = 50 + 55 * sin((time)/20), nonlinear2 = 50 + 55 * sin((time)/20))
  }
 ...
 if (nonlinear) {
   exp(lambda(time) + gamma(x[1], nonlinear) + alpha_nonlin(time, alpha_setting, x[2], x[3], r, long_df, b_set, long_setting) + dalpha(time, dalpha_setting) * dmu(time, r, long_df, b_set, long_setting))
 }
 else {
   exp(lambda(time) + gamma(x[1]) + alpha(time, alpha_setting) * mu(time, x[2], r, long_df, b_set, long_setting) + dalpha(time, dalpha_setting) * dmu(time, r, long_df, b_set, long_setting))
 }
  }
 ...
 x[, 3] <- rep(1, nsub)
  ...
  data_grid <- data.frame(survtime = times, mu = seq(-0.5, 2.5, length.out = length(times)))
...
rJM <- function(hazard, censoring, x, r, subdivisions = 1000, tmin = 0, tmax, file = NULL, ...) {
  nsub <- nrow(x)
  time <- rep(NA, nsub)
  Hazard <- function(hazard, time, x, r) {
 integrate(hazard, 0, time, x = x, r = r, subdivisions = subdivisions)$value
  }
 ...
 survprob[i] <- exp((-1) * cumhaz[i])
  ...
  data_short <- data.frame(survtime = time_event[, 1], event = time_event[, 2], x, r, cumhaz = cumhaz)
  names(data_short) <- gsub(".", "", names(data_short), fixed = TRUE)
  return(data_short)
}
jm_predict <- function(object, newdata, type = c("link", "parameter", "probabilities", "cumhaz", "loglik"), dt, steps, id, FUN = function(x) {
  mean(x, na.rm = TRUE)
}, subdivisions = 100, cores = NULL, chunks = 1, verbose = FALSE, ...) {
 ...
 newdata <- NULL
  ...
  calculate_mu <- FALSE
  if (attr(object$y[[1]], "nonlinear") & (("alpha" %in% list(...)$model) | is.null(list(...)$model)) & (is.null(newdata) | is.null(newdata$mu))) {
 calculate_mu <- TRUE
  }
  if (is.null(newdata)) {
 ...
 mu <- predict.bamlss(object, model = "mu", newdata = tempdata, type = type, cores = cores, chunks = chunks, verbose = verbose)
 newdata$mu <- mu
  }
  if (!(type %in% c("probabilities", "cumhaz"))) {
 object$family$predict <- NULL
 return(predict.bamlss(object, newdata = newdata, type = type, FUN = FUN, cores = cores, chunks = chunks, verbose = verbose, ...))
  }
 ...
 if (dt == 0) {
   ...
   timegrid <- lapply(dsurv[[timevar["lambda"]]], function(x) {
  grid(0, x, subdivisions)
   })
 }
 else {
   take <- !duplicated(data[, c(timevar["lambda"], idvar)], fromLast = TRUE)
   dsurv <- subset(data, take)
   timegrid <- lapply(dsurv[[timevar["mu"]]], function(x) {
  grid(x, x + dt, subdivisions)
   })
 }
 nobs <- nrow(dsurv)
 gdim <- c(length(timegrid), length(timegrid[[1]]))
 width <- rep(NA, nobs)
 for (i in 1:nobs) width[i] <- timegrid[[i]][2] - timegrid[[i]][1]
 pred.setup <- predict.bamlss(object, data, type = "link", get.bamlss.predict.setup = TRUE, ...)
 enames <- pred.setup$enames
 pred_gamma <- with(pred.setup, .predict.bamlss("gamma", object$x$gamma, samps, enames$gamma, intercept, nsamps, dsurv))
 pred_lambda <- with(pred.setup, .predict.bamlss.surv.td("lambda", object$x$lambda$smooth.construct, samps, enames$lambda, intercept, nsamps, dsurv, timevar["lambda"], timegrid, drop.terms.bamlss(object$x$lambda$terms, sterms = FALSE, keep.response = FALSE), type = 2))
 pred_mu <- with(pred.setup, .predict.bamlss.surv.td("mu", object$x$mu$smooth.construct, samps, enames$mu, intercept, nsamps, dsurv, timevar["mu"], timegrid, drop.terms.bamlss(object$x$mu$terms, sterms = FALSE, keep.response = FALSE)))
 pred_alpha <- with(pred.setup, .predict.bamlss.surv.td("alpha", object$x$alpha$smooth.construct, samps, enames$alpha, intercept, nsamps, dsurv, timevar["lambda"], timegrid, drop.terms.bamlss(object$x$alpha$terms, sterms = FALSE, keep.response = FALSE)))
 if (dalpha) {
   pred_dalpha <- with(pred.setup, .predict.bamlss.surv.td("dalpha", object$x$dalpha$smooth.construct, samps, enames$dalpha, intercept, nsamps, dsurv, timevar["lambda"], timegrid, drop.terms.bamlss(object$x$dalpha$terms, sterms = FALSE, keep.response = FALSE)))
   pred_dmu <- with(pred.setup, .predict.bamlss.surv.td("dmu", object$x$dmu$smooth.construct, samps, enames$dmu, intercept, nsamps, dsurv, timevar["mu"], timegrid, drop.terms.bamlss(object$x$dmu$terms, sterms = FALSE, keep.response = FALSE), derivMat = TRUE))
 }
 ...
 if (loglik) {
   ...
   eeta <- exp(eta_timegrid)
   int <- width * (0.5 * (eeta[, 1] + eeta[, subdivisions]) + apply(eeta[, 2:(subdivisions - 1)], 1, sum))
   ll <- sum((eta_timegrid[, ncol(eta_timegrid)] + eta_gamma) * y[take, "status"], na.rm = TRUE) - exp(eta_gamma) %*% int + sum(dnorm(y[, "obs"], mean = eta_mu, sd = exp(eta_sigma), log = TRUE))
   return(drop(ll))
 }
 if (dt == 0) {
   probs <- NULL
...
jm_survplot <- function(object, id = 1, dt = NULL, steps = 10, points = TRUE, rug = !points) {
 ...
 if (!is.factor(mf[[j]])) {
  ...
  plot2d(p_surv ~ time, fill.select = c(0, 1, 0, 1), scheme = 2, axes = FALSE, ylim = c(0, 1), xlim = c(0, maxtime), s2.col = s2.col, xlab = "", ylab = "")
  abline(v = tmax, lty = 2)
  axis(2)
  box()
  mtext("Prob(T > t + dt |T > t)", side = 2, line = 2.5)
  plot2d(p_long[time2 <= tmax, ] ~ time2[time2 <= tmax], fill.select = c(0, 1, 0, 1), scheme = 2, axes = FALSE, xlim = c(0, maxtime), xlab = "", ylab = "", ylim = if (points) 
 range(c(p_long, object$y[[1]][ii, "obs"]))
  else range(p_long))
  plot2d(p_long[time2 >= tmax, ] ~ time2[time2 >= tmax], fill.select = c(0, 1, 0, 1), scheme = 2, axes = FALSE, add = TRUE, s2.col = s2.col, xlab = "", ylab = "")
  abline(v = tmax, lty = 2)
  if (points) 
...
.predict.bamlss.jm.td <- function(id, x, samps, enames, intercept, nsamps, newdata, env, yname, grid, formula, type = 1, derivMat = FALSE) {
 ...
 for (j in enames2[i]) {
  ...
  if (!length(sn)) 
    sn <- snames[grep2(paste(id, "p.model.matrix", j, sep = "."), snames, fixed = TRUE)]
  eta <- eta + fitted_matrix(matrix(1, nrow = length(grid[[1]]) * nrow(newdata), ncol = 1), samps[, sn, drop = FALSE])
  }
   }
 }
  }
...
sm_time_transform2 <- function(x, data, grid, yname, timevar, take, derivMat = FALSE, eps = 1e-07, update_data = NULL) {
  if (derivMat) 
 ddata <- data
  if (!is.null(take)) 
 data <- data[take, , drop = FALSE]
...
update_jm_alpha_nonlin <- function(x, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, edf_alt = FALSE, tp = FALSE, ...) {
...
propose_jm_alpha_nonlin <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, tp = FALSE, fac = FALSE, ...) {
  if ("model.matrix" %in% attr(x, "class")) {
 X <- x$fit.fun_timegrid(NULL)
  }
  else {
...
update_jm_mu_nonlin <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, fac = FALSE, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
 ...
 if (i != "model.matrix") {
  ...
  int <- survint(X, eeta * eta_timegrid_alpha1, width, exp(eta$gamma), eeta * ((eta_timegrid_alpha1)^2 + eta_timegrid_alpha2), index = x$sparse.setup[["mu.matrix"]], dX, if (!is.null(dX)) 
 eeta * eta_timegrid_dalpha
  else NULL, if (!is.null(dX)) 
 eeta * eta_timegrid_dalpha^2
  else NULL)
  xgrad <- drop(t(x$X) %*% drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2) + t(x$XT) %*% drop(eta_alpha1 * status) - int$grad)
  XWX <- if (is.null(x$sparse.setup$matrix)) {
 crossprod(x$X * (1/exp(eta$sigma)^2), x$X)
  }
  else do.XWX(x$X, exp(eta$sigma)^2, index = x$sparse.setup$matrix)
...
update_jm_mu_nonlin_Matrix <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_alpha, eta_timegrid_mu, eta_timegrid_dalpha, eta_timegrid_dmu, status, update.nu, width, criterion, get_LogPost, nobs, do.optim2, edf, dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, fac = FALSE, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
  gdim <- c(nrow(eta_timegrid), ncol(eta_timegrid))
  if (tp) 
 Xalpha2 <- xsmalpha$margin[[2]]$fit.fun_timegrid(NULL)
  eeta <- exp(eta_timegrid)
  eta_timegrid_alpha1 <- eta_timegrid_alpha2 <- 0
  for (i in names(xsmalpha)) {
 if (i != "model.matrix") {
 ...
 eeta * eta_timegrid_dalpha^2
  ...
  xgrad <- crossprod(x$X, drop((y[, "obs"] - eta$mu)/exp(eta$sigma)^2)) + crossprod(x$XT, drop(eta_alpha1 * status)) - int$grad
  XWX <- crossprod(Diagonal(x = 1/exp(eta$sigma)^2) %*% x$X, x$X)
  XAX <- crossprod(x$XT * drop(eta_alpha2 * status), x$XT)
  xhess0 <- -1 * XWX + XAX - int$hess
  env <- new.env()
...
rowTensorProduct <- function(X1, X2) {
  do.call("rbind", lapply(1:nrow(X1), function(i) rep(X1[i, ], each = ncol(X2)) * rep(X2[i, ], ncol(X1))))
}
propose_jm_mu_nonlin <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, fac = FALSE, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
...
propose_jm_mu_nonlin_Matrix <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, fac = FALSE, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
...
propose_jm_mu_nonlin_Matrix_block <- function(x, y, eta, eta_timegrid, eta_timegrid_lambda, eta_timegrid_mu, eta_timegrid_alpha, eta_timegrid_dmu, eta_timegrid_dalpha, width, sub, nu, status, id, dx = NULL, xsmalpha = NULL, knots = NULL, tp = FALSE, fac = FALSE, ...) {
  X <- x$fit.fun_timegrid(NULL)
  dX <- if (!is.null(dx)) 
 dx$fit.fun_timegrid(NULL)
  else NULL
